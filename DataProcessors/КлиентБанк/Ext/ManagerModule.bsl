...
Процедура ЗаполнитьРеквизитыСписания(ДокументыКЗагрузке, СтрокаДокумента, ДокументОбъект, СоздаватьКонтрагентов, ПроводитьДокументы, Префикс)
	...
	//8/2/2018 5:43:36 PM
	//получим статью ддс по контрагенту и ХозяйственнаяОперация
	ЗаполнитьСтатьюДДСПоКонтрагентуИХозОперации(СтрокаДокумента, ХозяйственнаяОперация);
	//}Кожемякин А.Г.
...
Процедура ЗаполнитьСтатьюДДСПоКонтрагентуИХозОперации(Знач СтрокаДокумента, ХозяйственнаяОперация)
	
	Перем СтатьяПоКонтрагенту;
	
	Если ЗначениеЗаполнено(СтрокаДокумента.Контрагент) и ЗначениеЗаполнено(ХозяйственнаяОперация) Тогда
		СтатьяПоКонтрагенту = дп_ДополнительныеПроцедуры.ПолучитьСтатьюДДСПоКонтрагентуИХозОперации(СтрокаДокумента.Контрагент,ХозяйственнаяОперация);
		Если ЗначениеЗаполнено(СтатьяПоКонтрагенту) Тогда
			СтрокаДокумента.СтатьяДвиженияДенежныхСредств = СтатьяПоКонтрагенту;
		КонецЕсли; 
	КонецЕсли;
