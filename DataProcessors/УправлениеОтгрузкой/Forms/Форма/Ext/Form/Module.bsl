...
Процедура ПриОткрытии(Отказ)
	...
	//30.11.2017 14:23:32
	//
	ПодключитьОбработчикОжидания("ОбновитьСпискиОтгрузкиФрагмент",60);
	//}Кожемякин А.Г.
КонецПроцедуры
...
Процедура ЗаполнитьРаспоряженияНаОтгрузку(ПроверятьИзменениеПараметров = Ложь)
	...
	дп_ДополнительныеПроцедуры.ДополнитьЗапросДопДаннымиАРМОтгрузки(Запрос);
	
	ВремТабл = Запрос.Выполнить().Выгрузить();
	
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//02.06.2016 20:30:19
	//проверим новые распоряжения
	ЗафиксироватьНовыеРапоряжения(ВремТабл);
	//}Кожемякин А.Г.
	
	РаспоряженияНаОтгрузку.Загрузить(ВремТабл);
	...
	//ПРФТ++
	Для Каждого ТекСтрока ИЗ РаспоряженияНаОтгрузку Цикл
		Попытка
			ТекСтрока.ТребуетПодтвержденияСклада = ТекСтрока.Распоряжение.СостояниеПодтвержденияЗаказа;
		Исключение
		КонецПопытки;
	КонецЦикла;
	//ПРФТ--
	...
	//ПРФТ ++  20190721
		Для Каждого ТекСтрока ИЗ РаспоряженияНаОтгрузку Цикл
		Попытка
			ТекСтрока.Контрагент = ТекСтрока.Распоряжение.Контрагент;
		Исключение
		КонецПопытки;
	КонецЦикла;
...
//ПРФТ ++  20190729
		...
		КоличествоСтрокВОчереди = 0;
		КоличествоШтукВОчереди = 0;

		Для Каждого ТекСтрока ИЗ РаспоряженияНаОтгрузку Цикл
			Если ТекСтрока.СтатусОтгрузки = ПредопределенноеЗначение("Перечисление.СтатусыРасходныхОрдеров.КОтбору") Тогда
				КоличествоКОтбору = КоличествоКОтбору +1;
				КоличествоСтрок = КоличествоСтрок +ТекСтрока.СтрокВРаспоряжении;
				КоличествоШтук = КоличествоШтук +ТекСтрока.КоличествоВРаспоряжении;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ТекСтрока.СтатусОтгрузки) Тогда
				КоличествоВОчереди = КоличествоВОчереди +1;
				КоличествоСтрокВОчереди = КоличествоСтрокВОчереди +ТекСтрока.СтрокВРаспоряжении;
				КоличествоШтукВОчереди = КоличествоШтукВОчереди +ТекСтрока.КоличествоВРаспоряжении;
			КонецЕсли;
	...
	Для Каждого ТекСтрока ИЗ РаспоряженияНаОтгрузку Цикл
		Попытка
			Для Каждого ТСтр Из ТекСтрока.Распоряжение.ДополнительныеРеквизиты Цикл
					Если ТСтр.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Желаемая менеджером дата отгрузки") ТОгда
						ТекСтрока.ЖелаемаяМенеджеромДата = ТСтр.Значение;
					КонецЕсли;
			КонецЦикла;
		Исключение
		КонецПопытки;
...
Процедура ПечатьПакетаДокументов(Команда)
	...
	Для Каждого СтрИД из Элементы.РаспоряженияНаОтгрузку.ВыделенныеСтроки Цикл
		ТекРаспоряжение = РаспоряженияНаОтгрузку.НайтиПоИдентификатору(СтрИД).Распоряжение;
		Если ТипЗнч(ТекРаспоряжение) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
			МассивРаспоряжений.Добавить(ТекРаспоряжение);
		Иначе
			Отказ = Истина;
		КонецЕсли;
		Прервать;
	...
	//- по всем ли отгружаемым позициям по выделенному распоряжению созданы ли РТиУ, 
	//	если нет - уведомлять пользователя: "По выделенному распоряжению не созданы требуемые документы продажи". 
	//	Проверку производить, если организация в Заказе клиента не равна ЧЛ.
	//- если орагнизация в заказе клиента = юридическое лицо, то проверять: на основании всех ли РТиУ созданы Счет-фактуры,
	//	если нет - уведомлять пользователя: "Не оформлены требуемые счет-фактуры!" 
	ТекстОшибки = "";
	Если Не дп_ДополнительныеПроцедуры.ЕстьНаличиеВсехДокументов(ТекРаспоряжение, ТекстОшибки, Отказ) Тогда
	...
	Новый Структура("Объекты", МассивРаспоряжений));
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ЗафиксироватьНовыеРапоряжения(ТекРаспоряженияНаОтгрузку)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТекРаспоряжения.Распоряжение
		|ПОМЕСТИТЬ ТекРаспоряжения
		|ИЗ
		|	&ТаблРаспоряжения КАК ТекРаспоряжения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ТекРаспоряжения КАК Распоряжения
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.дпВремяРаспоряженияВАРМ КАК дпВремяРаспоряженияВАРМ
		|		ПО (дпВремяРаспоряженияВАРМ.Распоряжение = Распоряжения.Распоряжение)
		|			И (дпВремяРаспоряженияВАРМ.Пользователь = &ТекПользователь)
		|ГДЕ
		|	дпВремяРаспоряженияВАРМ.Распоряжение ЕСТЬ NULL ";
		//}Кожемякин А.Г.

	Запрос.УстановитьПараметр("ТаблРаспоряжения", ТекРаспоряженияНаОтгрузку);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		МенеджерЗаписи 				= РегистрыСведений.дпВремяРаспоряженияВАРМ.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Распоряжение = ВыборкаДетальныеЗаписи.Распоряжение;
...
&НаКлиенте
Процедура дп_ПечатьТранспортнойЭтикетки(Команда)
	СтруктураПараметры = Новый Структура("СсылкаНаОбъект");
	СтруктураПараметры.СсылкаНаОбъект   = Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.Распоряжение;
	
	ОткрытьФорму(
	"Обработка.дп_ТранспортнаяЭтикетка.Форма.Форма",
...
Процедура ПечатьЭтикеток(Команда)
	...
	ПараметрКоманды.Добавить(Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.Распоряжение);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПроверитьПроведенностьДокументовЗавершение", ЭтотОбъект);
	
	УправлениеПечатьюКлиент.ПроверитьПроведенностьДокументов(ОписаниеОповещения, ПараметрКоманды);
	
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьПроведенностьДокументовЗавершение(ПараметрКоманды, ДополнительныеПараметры) Экспорт
	
	Перем АдресВХранилище, СтруктураПараметры;
...
Процедура ОбновитьСпискиОтгрузки(Команда)
	ОбновитьСпискиОтгрузкиФрагмент();
КонецПроцедуры

&НаКлиенте
Процедура ОбновитьСпискиОтгрузкиФрагмент()
...
Процедура СоздатьРеализацию(Команда)
	...
	ЗаполнитьРеализациюНаСервере(ДанныеФормы, ТекущиеДанные.Распоряжение);
	КопироватьДанныеФормы(ДанныеФормы, Форма.Объект);
	
	ТемпПараметры = ЭтапыОплатыКлиентСервер.СтруктураПолученияНадписиЭтаповОплатыДляРеализациииТоваровУслуг();
	ЗаполнитьЗначенияСвойств(ТемпПараметры, Форма.Объект);
	
	ОбщаяСуммаПредоплаты = Форма.СуммаПредоплатыВВалютеДокумента + Форма.СуммаПредоплатыЗаТаруВВалютеДокумента;
	
	ТемпПараметры.Вставить("ПорядокРасчетов", Форма.Объект.ПорядокРасчетов);
	ТемпПараметры.Вставить("СуммаВсего",      Форма.СуммаВсего);
	ТемпПараметры.Вставить("СуммаПредоплаты", ОбщаяСуммаПредоплаты);
	ТемпПараметры.Вставить("ИспользоватьСтатусыРеализацийТоваровУслуг", Форма.ИспользоватьСтатусыРеализацийТоваровУслуг);
	ТемпПараметры.Вставить("ГрафикИсполненияВДоговоре", Форма.ГрафикИсполненияВДоговоре);
	
	Форма.НадписьЭтапыОплаты = ЭтапыОплатыКлиентСервер.НадписьЭтапыОплатыДляРеализациииТоваровУслуг(ТемпПараметры);
...
&НаСервере
Процедура ЗаполнитьРеализациюНаСервере(ДанныеФормы, ТекРаспоряжение)

	Док = ДанныеФормыВЗначение(ДанныеФормы, Тип("ДокументОбъект.РеализацияТоваровУслуг"));
	
	//Док = Документы.РеализацияТоваровУслуг.СоздатьДокумент();
	
	Док.Заполнить(ПолучитьСтруктуруЗаполнения(ТекРаспоряжение));
...
&НаСервере
Функция ПолучитьСтруктуруЗаполнения(Знач ТекРаспоряжение)
	
	Возврат Новый Структура("ДокументОснование,ВариантОформленияПродажи,СкладОтгрузки",ТекРаспоряжение,Перечисления.ВариантыОформленияПродажи.РеализацияТоваровУслуг,Неопределено);

КонецФункции

&НаКлиенте
Процедура ПодтвердитьЗаказ(Команда)
	
	ПодтвердитьЗаказНаСервере(Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.Распоряжение,  Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.КоличествоВРаспоряжении, Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.СтрокВРаспоряжении);
КонецПроцедуры

&НаСервере
Процедура ПодтвердитьЗаказНаСервере(ДокЗаказ, кВо, стро)
	Если НЕ РольДоступна("ПРФТ_ПодтверждениеЗаказовСкладом") Тогда
		Сообщить("Пользователю запрещено подтверждать/отклонять заказы");
		Возврат;
	КонецЕсли;

	Если ДокЗаказ.СостояниеПодтвержденияЗаказа = Перечисления.СостоянияПодтвержденияЗаказа.ТребуетПодтверждения ТОгда
		Докк = ДОкЗаказ.ПолучитьОбъект();
		Докк.СостояниеПодтвержденияЗаказа =  Перечисления.СостоянияПодтвержденияЗаказа.Подтвержден;
		Докк.ЖелаемаяДатаОтгрузки = Докк.ЖелаемаяДатаОтгрузки - 86400;
		
		
		//Строки
		СтрокиЕсть = Ложь;
		Для Каждого ТекСтрока Из Докк.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Строк") ТОгда
				СтрокиЕсть = Истина;
				ТекСтрока.Значение = стро;
			КонецЕсли;
		КонецЦикла;
		Если Не СтрокиЕсть ТОгда
			НоваяСтркоа = Докк.ДополнительныеРеквизиты.Добавить();
			НоваяСтркоа.Значение = стро;
			НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Строк");
		КонецЕсли;
		
		//Штуки
		ШтукиЕсть = Ложь;
		Для Каждого ТекСтрока Из Докк.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Штук") ТОгда
				ШтукиЕсть = Истина;
				ТекСтрока.Значение = кВо;
			КонецЕсли;
		КонецЦикла;
		Если Не ШтукиЕсть ТОгда
			НоваяСтркоа = Докк.ДополнительныеРеквизиты.Добавить();
			НоваяСтркоа.Значение = кВо;
			НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Штук");
		КонецЕсли;
		
		//КОтборуПриПодтвержденииОтказе
		КОтборуЕсть = Ложь;
		Для Каждого ТекСтрока Из Докк.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("КОтборуПриПодтвержденииОтказе") ТОгда
				КОтборуЕсть = Истина;
				ТекСтрока.Значение = КоличествоСтрок;
			КонецЕсли;
		КонецЦикла;
		Если Не КОтборуЕсть ТОгда
			НоваяСтркоа = Докк.ДополнительныеРеквизиты.Добавить();
			НоваяСтркоа.Значение = КоличествоСтрок;
			НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("КОтборуПриПодтвержденииОтказе");
		КонецЕсли;
		
		//ВОчередиПриПодтвержденииОтказе
		ВОчередиЕсть = Ложь;
		Для Каждого ТекСтрока Из Докк.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("ВОчередиПриПодтвержденииОтказе") ТОгда
				ВОчередиЕсть = Истина;
				ТекСтрока.Значение = КоличествоСтрокВОчереди;
			КонецЕсли;
		КонецЦикла;
		Если Не ВОчередиЕсть ТОгда
			НоваяСтркоа = Докк.ДополнительныеРеквизиты.Добавить();
			НоваяСтркоа.Значение = КоличествоСтрокВОчереди;
			НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("ВОчередиПриПодтвержденииОтказе");
		КонецЕсли;

		//ДатаПодтвержденияОтказаСклада
		ДатаПодтвЕсть = Ложь;
		Для Каждого ТекСтрока Из Докк.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("ДатаПодтвержденияОтказаСклада") ТОгда
				ДатаПодтвЕсть = Истина;
				ТекСтрока.Значение = ТекущаяДата();
			КонецЕсли;
		КонецЦикла;
		Если Не ДатаПодтвЕсть ТОгда
			НоваяСтркоа = Докк.ДополнительныеРеквизиты.Добавить();
			НоваяСтркоа.Значение = ТекущаяДата();
			НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("ДатаПодтвержденияОтказаСклада");
		КонецЕсли;
	...
	Задача.Наименование                  = "Подтверждение переноса даты заказа";
	Задача.Описание                      = "Заказу " + ДокЗаказ.Ссылка + " перенесёна дата отгрузки на: " + Формат(ДокЗаказ.ЖелаемаяДатаОтгрузки,"ДФ=dd.MM.yyyy");
	Задача.Важность                      = Перечисления.ВариантыВажностиЗадачи.Высокая;
	Задача.СрокИсполнения                = ТекущаяДата();
	Задача.Исполнитель                   = ДокЗаказ.Менеджер;
...
&НаКлиенте
Процедура ОтклонитьЗаказ(Команда)
	Перем ВыбЗнач;
	Массив = Новый Массив;
	Массив.Добавить(Тип("Строка"));
	КС = Новый КвалификаторыСтроки(20);
	ОписаниеТипов = Новый ОписаниеТипов(Массив,  КС);
	Если ВвестиЗначение(ВыбЗнач, "Введите причину отклонения заказа", ОписаниеТипов) Тогда
		Причина = ВыбЗнач;
		ОтклонитьЗаказНаСервере(Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.Распоряжение, Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.КоличествоВРаспоряжении, Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.СтрокВРаспоряжении);

		
	Иначе
		Сообщить("Причину ввести обязательно. Заказ не отклонён");
	КонецЕсли;
...
&НаСервере
Процедура ОтклонитьЗаказНаСервере(ДокЗаказ, кВо, стро)
Если НЕ РольДоступна("ПРФТ_ПодтверждениеЗаказовСкладом") Тогда
		Сообщить("Пользователю запрещено подтверждать/отклонять заказы");
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ДокЗаказ.СостояниеПодтвержденияЗаказа)  ТОгда
		Докк = ДОкЗаказ.ПолучитьОбъект();		
		ПричинаЕсть = Ложь;
		Для Каждого ТекСтрока Из Докк.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Причина отклонения") ТОгда
				ПричинаЕсть = Истина;
				ТекСтрока.Значение = Причина;
			КонецЕсли;
		КонецЦикла;
		Если Не ПричинаЕсть ТОгда
			НоваяСтркоа = Докк.ДополнительныеРеквизиты.Добавить();
			НоваяСтркоа.Значение = Причина;
			НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Причина отклонения");
		КонецЕсли;
		...
		Докк.СостояниеПодтвержденияЗаказа =  Перечисления.СостоянияПодтвержденияЗаказа.Отклонен;
		Докк.Записать();
	...
	Задача.Дата                          = ТекущаяДата();
	Задача.Автор                         =  ПараметрыСеанса.ТекущийПользователь;
	Задача.Наименование                  = "Отказ подтверждения переноса даты заказа";
	Задача.Описание                      = "Заказу " + ДокЗаказ.Ссылка + " ОТКАЗАНО перененос дата отгрузки";
	Задача.Важность                      = Перечисления.ВариантыВажностиЗадачи.Высокая;
	Задача.СрокИсполнения                = ТекущаяДата();
	Задача.Исполнитель                   = ДокЗаказ.Менеджер;
	Задача.Выполнена = Истина;
...
&НаКлиенте
Процедура ПечатьУпаковкиКороба(Команда)
	//КоллекцияПечатныхФорм = УправлениеПечатьюКлиент.НоваяКоллекцияПечатныхФорм("УпаковкиКоробы");
	//
	//АдресКоллекцияПечатныхФорм = Команда1НаСервере(Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.Распоряжение);
	//
...
Команда1НаСервере(Элементы.РаспоряженияНаОтгрузку.ТекущиеДанные.Распоряжение).Показать();
КонецПроцедуры


&НаСервере
Функция Команда1НаСервере(ДокССылка)
	//ДокССылка  = Объект.СсылкаНаОбъект;
	
	Табдок = Новый ТабличныйДокумент;	
	Если ДокССылка.Товары.Количество() = ДокССылка.ШтрихкодыКоробов.Количество() Тогда
	Иначе
		Сообщить("Количество строк товаров не равно количеству строк  ШК коробов!!!!!");
		Возврат  Табдок;
	КонецЕсли;
	...
	Область = макет.ПолучитьОбласть("ОбластьГоризонтальная|ОбластьВертикальная");
	ОбщиеПараметры = Новый Структура;
	ОбщиеПараметры.Вставить("ДатаПоставки", Формат(ДокССылка.ЖелаемаяДатаОтгрузки,"ДФ=dd.MM.yyyy"));
	ОбщиеПараметры.Вставить("Юрлицо", ДокССылка.Организация);
	...
	|	И ДополнительныеСведения.Свойство.Наименование ПОДОБНО ""%"" + ""Номер поставки"" + ""%""";	
	Запрос.УстановитьПараметр("Объект", ДокССылка);	
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	Если РезультатЗапроса.Следующий() Тогда
		ОбщиеПараметры.Вставить("НомерПоставки", РезультатЗапроса.Значение);	
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст = 	"ВЫБРАТЬ
	|	ДополнительныеСведения.Значение КАК Значение,
	...
	|ГДЕ
	|	ДополнительныеСведения.Объект = &Объект
	|	И ДополнительныеСведения.Свойство.Наименование ПОДОБНО ""%"" + ""Склад Вб"" + ""%""";	
	Запрос.УстановитьПараметр("Объект", ДокССылка);	
	РезультатЗапроса = Запрос.Выполнить().Выбрать();
	Если РезультатЗапроса.Следующий() Тогда
		ОбщиеПараметры.Вставить("СкладВБ", РезультатЗапроса.Значение);	
	КонецЕсли;

	
	Счетчик = 0;
	Для каждого ТекСтрока Из ДокССылка.Товары Цикл
		Область = Макет.ПолучитьОбласть("ОбластьГоризонтальная|ОбластьВертикальная");
		ЗаполнитьЗначенияСвойств(Область.Параметры,ОбщиеПараметры);
		Область.Параметры.Номенклатура = ТекСтрока.Номенклатура;
		Область.Параметры.Характеристка = ТекСтрока.Характеристика;
		Область.Параметры.Количество = ТекСтрока.КоличествоУпаковок;
		...
		выборкаНов = Запрос.Выполнить().Выбрать();
		Если выборкаНов.Следующий() Тогда
			Область.Параметры.Баркод =выборкаНов.Штрихкод;		
		КонецЕсли;
		 КоличествоМиллиметровВПикселе = 0.3;
		ПараметрыШтрихкода = Новый Структура;
		ПараметрыШтрихкода.Вставить("Ширина",          Окр(Область.Рисунки.КартинкаШтрихкода.Ширина / КоличествоМиллиметровВПикселе));
		ПараметрыШтрихкода.Вставить("Высота",          Окр(Область.Рисунки.КартинкаШтрихкода.Высота / КоличествоМиллиметровВПикселе));
		ПараметрыШтрихкода.Вставить("Штрихкод",       ДокССылка.ШтрихкодыКоробов[Счетчик].Штрихкод);
		...
		//скю++
		Запрос = Новый Запрос("ВЫБРАТЬ
		|	КодыЦентральногоСправочника.КодЦС КАК КодЦС
		|ИЗ
		...
		Если ВЫб.Следующий() Тогда
			Область.Параметры.SKU =ВЫб.КодЦС;		
		КонецЕсли;
		//скю--
		...
		Если НЕ Цел(Счетчик/2) = Счетчик/2 Тогда
