...
Процедура ЗаполнитьАдресатов(Команда)
	...
	Если Объект[ИмяТабличнойЧасти].Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьАдресатовЗавершение", ЭтотОбъект), НСтр("ru = 'Табличная возможных адресатов будет очищена. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
	КонецЕсли;
...
Процедура СформироватьЗаголовок()
	...
	Если Объект.ПредназначенДляЭлектронныхПисем Тогда
		Заголовок = НСтр("ru = 'Письмо по шаблону'");
	ИначеЕсли Объект.ПредназначенДляSMS Тогда
		Заголовок = НСтр("ru = 'Сообщение SMS по шаблону'");
	КонецЕсли;
		...
		Если ЭлементСоответствия.Ключ = "Произвольные" Тогда
			...
			Для каждого ЭлементМассиваПараметров Из ЭлементСоответствия.Значение Цикл
				...
				Исключение
					...
					НСтр("ru = 'Шаблоны сообщений'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция УсловияПереходаПоСтраницамНеВыполнены(Команда)
	...
	Если Элементы.СтраницыОсновные.ТекущаяСтраница = Элементы.СтраницаВыборШаблона Тогда
		...
		Если ШаблоныТекущиеДанные = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран шаблон сообщения'"),, "ДоступныеШаблоны");
			Возврат Истина;
		КонецЕсли;
...
Процедура ОбработатьПереданныеПараметры()
	...
	Если ТипЗнч(Параметры.Основание) = Тип("СправочникСсылка.ШаблоныСообщений") И ЗначениеЗаполнено(Параметры.Основание) Тогда
		...
		Если НЕ Реквизиты.ПредназначенДляЭлектронныхПисем  И Параметры.ПредназначенДляЭлектронныхПисем Тогда
			ВызватьИсключение НСтр("ru='Данный шаблон не предназначен для создания электронных писем.'");
		КонецЕсли;
		Если НЕ Реквизиты.ПредназначенДляSMS И НЕ Параметры.ПредназначенДляЭлектронныхПисем Тогда
			ВызватьИсключение НСтр("ru='Данный шаблон не предназначен для создания сообщений SMS.'");
		КонецЕсли;
	КонецЕсли;
...
Функция СообщениеSMSЗаполненоСОшибками()
	...
	Если ПустаяСтрока(Объект.ТекстСообщенияSMS) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Пустой текст сообщения SMS'"),,"Объект.ТекстСообщенияSMS");
		Отказ = Истина;
	КонецЕсли;
	...
	Если Адресаты.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан ни один получатель SMS'"),,"Объект.АдресатыSMS");
		Отказ = Истина;
...
Функция ЭлектронноеПисьмоЗаполненоСОшибками()
	...
	Если Объект.УчетнаяЗапись.Пустая() Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указана учетная запись для отправки'"),,
	...
	Если Получатели.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан ни один получатель письма'"),,
		...
		Для каждого Получатель Из Получатели Цикл
			Если НЕ ОбщегоНазначенияКлиентСервер.АдресЭлектроннойПочтыСоответствуетТребованиям(Получатель.Адрес) Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				                    НСтр("ru = 'Указан некорректный адрес электронной почты %1'"),
				                    Получатель.Адрес);
...
Функция КомментарийПоНаименованиюШаблона()
	...
	Возврат НСтр("ru = 'Создано и отправлено по шаблону'") + " - " + Объект.НаименованиеШаблона;
