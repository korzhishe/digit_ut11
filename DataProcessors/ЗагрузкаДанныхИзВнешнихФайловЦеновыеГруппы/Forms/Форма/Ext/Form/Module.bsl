...
Процедура ЗаполнитьТаблицуНоменклатурыИШтрихкодов()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Справочник.ЦеновыеГруппы КАК Номенклатура
		|ГДЕ
		...
		|	ВЫБОР
		|		КОГДА НомХар.Номенклатура ССЫЛКА Справочник.Номенклатура
		|			ТОГДА НомХар.Номенклатура.Артикул
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК Артикул,
		|	"""" КАК АртикулПоиск,
		...
		|	ВЫБОР
		|		КОГДА НомХар.Номенклатура ССЫЛКА Справочник.Номенклатура
		|			ТОГДА НомХар.Номенклатура.НаименованиеПолное
		|		ИНАЧЕ """"
	...
	//ТекстУсловияОтбор = "Номенклатура.НаименованиеПолное = ""Кабель Baseus Aces Combo Data Cable 1M""";
