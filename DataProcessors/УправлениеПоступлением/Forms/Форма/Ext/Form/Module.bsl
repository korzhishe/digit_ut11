...
Процедура ГруппаСтраницыПриСменеСтраницыСервер(ИмяТекущейСтраницы)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.РаботаСЗаявкамиНаВозврат Тогда
		ОбновитьНаСервере();
	КонецЕсли;
...
Процедура ЗаполнитьРаспоряженияНаПоступление()
	...
	Для Каждого СтрокаРаспоряжение из РаспоряженияНаПоступление Цикл
		Если ТипЗнч(СтрокаРаспоряжение.Распоряжение) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
			СтрокаРаспоряжение.ЖелаемаяДатаПоступления = СтрокаРаспоряжение.Распоряжение.ЖелаемаяДатаПоступления;
		КонецЕсли;	
	КонецЦикла;	
...
Функция ДанныеДляПечатиЭтикетокПоРаспоряжению(Распоряжение)
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		Запрос.УстановитьПараметр("Импортер", Распоряжение.дпИмпортер);
	Иначе
		Запрос.УстановитьПараметр("Импортер", "");
	КонецЕсли;
...
//
&НаСервере
Процедура доп_СоздатьПоступлениеНаСервере(Знач ТекРаспоряжение, Отказ, Знач Ордер)
	Если ТипЗнч(ТекРаспоряжение) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		//создаем документ постпление товаров услуг
		ПоступлениеОбъект = Документы.ПриобретениеТоваровУслуг.СоздатьДокумент();
		//выполняем процедуру заполнить, передаем туда распоряжение (заказ поставщику)
		//ПоступлениеОбъект.Заполнить(Новый Структура("ДокументОснование",Ордер));
		ПоступлениеОбъект.Заполнить(Ордер);
		//проводим поступление
		Если ПоступлениеОбъект.Товары.Количество() Тогда
...
&НаКлиенте
Процедура доп_СоздатьПоступление(Команда)
	ПоказатьВопрос(Новый ОписаниеОповещения("доп_СоздатьПоступлениеЗавершение", ЭтотОбъект), "Будет создан документ поступления. Продолжить?",РежимДиалогаВопрос.ДаНет);	
КонецПроцедуры

&НаКлиенте
Процедура доп_СоздатьПоступлениеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		//получаем текущую строку
		ТекущиеДанные = Элементы.ОрдераВРаботе.ТекущиеДанные;
		//получаем рапоряжение ордера
		Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
			ТекРаспоряжение = ПолучитьРаспоряжение(ТекущиеДанные);
			Если ТипЗнч(ТекРаспоряжение) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
				Отказ = Ложь;
				доп_СоздатьПоступлениеНаСервере(ТекРаспоряжение, Отказ, ТекущиеДанные.Ссылка);
				Если Отказ Тогда
					ПоказатьПредупреждение(Неопределено,"Не удалось создать документ.");
				Иначе	
					ПоказатьПредупреждение(Неопределено,"Документ создан.");
				КонецЕсли;	
			Иначе
				ПоказатьПредупреждение(Неопределено,"Создается только по распоряжениям ""Заказ поставщику"".");
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьРаспоряжение(Знач ТекущиеДанные)
	
	Возврат ТекущиеДанные.Ссылка.Распоряжение;
...
Процедура СоздатьВозврат(Команда)
	...
	Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
		ТекРаспоряжение = ПолучитьРаспоряжение(ТекущиеДанные);
		Если ТипЗнч(ТекРаспоряжение) = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
			//
			//СоздатьВозвратНаСервере(ТекущиеДанные);
			СоздатьВозвратНаСервере(ТекРаспоряжение);
		Иначе
			ПоказатьПредупреждение(Неопределено,"Создается только по распоряжениям ""Заявка на возврат товаров"".");
		КонецЕсли;
		Элементы.ОрдераВРаботе.Обновить();	
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура СоздатьВозвратНаСервере(ТекРаспоряжение)
	ВозвратОбъект = Документы.ВозвратТоваровОтКлиента.СоздатьДокумент();
	//выполняем процедуру заполнить, передаем туда распоряжение (заказ поставщику)
	//ПоступлениеОбъект.Заполнить(Новый Структура("ДокументОснование",Ордер));
	ВозвратОбъект.Заполнить(ТекРаспоряжение);
	Если НЕ ЗначениеЗаполнено(ВозвратОбъект.Дата) ТОгда
		ВозвратОбъект.Дата = ТекущаяДата();
	КонецЕсли;
	Если ВозвратОбъект.Товары.Количество() Тогда
		//ВозвратОбъект.Записать(РежимЗаписиДокумента.Запись);
		Попытка
			ВозвратОбъект.Записать(РежимЗаписиДокумента.Проведение);
			Сообщить("Успешно создан: "+ ВозвратОбъект.Ссылка);
		Исключение
		КонецПопытки;
	Иначе
		//Отказ = Истина;
	...
	//ОткрытьЗначение(ВозвратОбъект.ССылка);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаявкиНаВозвратПриАктивизацииСтроки(Элемент)
	Если Элемент.ТекущиеДанные = Неопределено ТОгда Возврат; КонецЕсли;
	ЗаполнитьОчиститьОрдераВозвраты(Элемент.ТекущиеДанные.Заявка);
КонецПроцедуры

Процедура ЗаполнитьОчиститьОрдераВозвраты(ДокЗаявка)
	Ордера.Очистить();
	Возвраты.Очистить();
	...
	|ГДЕ
	|	ПриходныйОрдерНаТовары.Распоряжение = &Распоряжение");
	Запрос.УстановитьПараметр("Распоряжение", ДокЗаявка);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = Ордера.Добавить();
		НоваяСтрока.докССылка = Выборка.ССылка;
		НоваяСтрока.докЗаявка = ДокЗаявка;
		НоваяСтрока.КоличествоТоваров = Выборка.ССылка.ТОвары.Итог("КоличествоУпаковок");
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("ЗаявкаНаВозвратТоваровОтКлиента", ДокЗаявка);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = Возвраты.Добавить();
		НоваяСтрока.докССылка = Выборка.ССылка;
		НоваяСтрока.докЗаявка = ДокЗаявка;
		НоваяСтрока.КоличествоТоваров = Выборка.ССылка.ТОвары.Итог("КоличествоУпаковок");
	КонецЦикла;
...
Процедура ОбновитьНаСервере()
	ЗаявкиНаВозврат.Очистить();
	Ордера.Очистить();
	Возвраты.Очистить();
	
	Если ЗначениеЗаполнено(СкладОтбор) ТОгда
		//ЗАпрос = Новый ЗАпрос("ВЫБРАТЬ
		//                      |	СУММА(ЕСТЬNULL(ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.КоличествоУпаковок, 0)) КАК КоличествоЗаявка,
		//                      |	СУММА(ЕСТЬNULL(ПриходныйОрдерНаТоварыТовары.КоличествоУпаковок, 0)) КАК КоличествоПриходныйОрдер,
		//                      |	СУММА(ЕСТЬNULL(ВозвратТоваровОтКлиентаТовары.КоличествоУпаковок, 0)) КАК КоличествоВозврат,
		...
		//                      |		ПО ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка = ВозвратТоваровОтКлиентаТовары.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента
		//                      |			И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Номенклатура = ВозвратТоваровОтКлиентаТовары.Номенклатура
		//                      |			И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Характеристика = ВозвратТоваровОтКлиентаТовары.Характеристика
		//                      |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдерНаТовары.Товары КАК ПриходныйОрдерНаТоварыТовары
		//                      |		ПО ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка = ПриходныйОрдерНаТоварыТовары.Ссылка.Распоряжение
		//                      |			И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Номенклатура = ПриходныйОрдерНаТоварыТовары.Номенклатура
		//                      |			И ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Характеристика = ПриходныйОрдерНаТоварыТовары.Характеристика
		//                      |ГДЕ
		//                      |	ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка.Склад = &Склад
		//                      |
		//                      |СГРУППИРОВАТЬ ПО
		//                      |	ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка");	
		ЗАпрос = Новый ЗАпрос("ВЫБРАТЬ
		                      |	СУММА(ЕСТЬNULL(ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.КоличествоУпаковок, 0)) КАК КоличествоЗаявка,
		                      |	ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка КАК Ссылка
		                      |ПОМЕСТИТЬ ВТ_Заявка
		                      ...
		                      |	И НЕ ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Отменено
		                      |
		                      |СГРУППИРОВАТЬ ПО
		                      |	ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка
		                      |;
		                      ...
		                      |////////////////////////////////////////////////////////////////////////////////
		                      |ВЫБРАТЬ
		                      |	СУММА(ПриходныйОрдерНаТоварыТовары.КоличествоУпаковок) КАК КоличествоПриходныйОрдер,
		                      |	ПриходныйОрдерНаТоварыТовары.Ссылка.Распоряжение КАК Распоряжение
		                      |ПОМЕСТИТЬ ВТ_Ордер
		                      |ИЗ
		                      |	ВТ_Заявка КАК ВТ_Заявка
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриходныйОрдерНаТовары.Товары КАК ПриходныйОрдерНаТоварыТовары
		                      |		ПО ВТ_Заявка.Ссылка = ПриходныйОрдерНаТоварыТовары.Ссылка.Распоряжение
		                      |
		                      |СГРУППИРОВАТЬ ПО
		                      ...
		                      |ВЫБРАТЬ
		                      |	СУММА(ЕСТЬNULL(ВозвратТоваровОтКлиентаТовары.КоличествоУпаковок, 0)) КАК КоличествоВозврат,
		                      |	ВозвратТоваровОтКлиентаТовары.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента КАК ЗаявкаНаВозвратТоваровОтКлиента
		                      |ПОМЕСТИТЬ ВТ_Возврат
		                      |ИЗ
		                      |	ВТ_Заявка КАК ВТ_Заявка
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента.Товары КАК ВозвратТоваровОтКлиентаТовары
		                      |		ПО ВТ_Заявка.Ссылка = ВозвратТоваровОтКлиентаТовары.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента
		                      |
		                      |СГРУППИРОВАТЬ ПО
		                      |	ВозвратТоваровОтКлиентаТовары.Ссылка.ЗаявкаНаВозвратТоваровОтКлиента
		                      |;
		                      |
		                      |////////////////////////////////////////////////////////////////////////////////
		                      |ВЫБРАТЬ
		                      |	ВТ_Заявка.Ссылка КАК Ссылка,
		                      |	ВТ_Заявка.КоличествоЗаявка КАК КоличествоЗаявка,
		                      |	ВТ_Возврат.КоличествоВозврат КАК КоличествоВозврат,
		                      |	ВТ_Ордер.КоличествоПриходныйОрдер КАК КоличествоПриходныйОрдер
		                      |ИЗ
		                      |	ВТ_Заявка КАК ВТ_Заявка
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Ордер КАК ВТ_Ордер
		                      |		ПО ВТ_Заявка.Ссылка = ВТ_Ордер.Распоряжение
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Возврат КАК ВТ_Возврат
		                      |		ПО ВТ_Заявка.Ссылка = ВТ_Возврат.ЗаявкаНаВозвратТоваровОтКлиента");	
	...
	Иначе
			ЗАпрос = Новый ЗАпрос("ВЫБРАТЬ
			                      ...
			                      |	Документ.ЗаявкаНаВозвратТоваровОтКлиента.ВозвращаемыеТовары КАК ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары
			                      |ГДЕ
			                      |	ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка.Проведен
			                      |	И НЕ ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Отменено
			                      |
			                      |СГРУППИРОВАТЬ ПО
			                      |	ЗаявкаНаВозвратТоваровОтКлиентаВозвращаемыеТовары.Ссылка
	...
	Выборка = ЗАпрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		СтатусЗаявки = ОпределитьСтатус(Выборка.КоличествоЗаявка,?(Выборка.КоличествоПриходныйОрдер = Null,0, Выборка.КоличествоПриходныйОрдер),?(Выборка.КоличествоВозврат = Null,0, Выборка.КоличествоВозврат) );
		
		Если ЗначениеЗаполнено(СтатусОтбор) ТОгда
			Если НЕ СтатусЗаявки = 	СтатусОтбор ТОгда Продолжить; КонецЕсли;
		КонецЕсли;
		НоваяСтрока = ЗаявкиНаВозврат.Добавить();
		НоваяСтрока.Заявка = Выборка.Ссылка;
		НоваяСтрока.Статус = СтатусЗаявки;
		НоваяСтрока.КоличествоТоваров =Выборка.КоличествоЗаявка; 
	КонецЦикла;
	
	
	ЗаявкиНаВозврат.Сортировать("Статус");
	
	
	
	МассивИменКолонокДляПодсветки = Новый Массив;
	Для каждого Стр из Элементы.ЗаявкиНаВозврат.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(Стр) = Тип("ГруппаФормы") Тогда
			Для каждого Стрр из Стр.ПодчиненныеЭлементы Цикл
				МассивИменКолонокДляПодсветки.Добавить(Стрр.Имя);
			КонецЦикла;
		Иначе
	...
	ЭлементУсловия.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.ЗаявкиНаВозврат.Статус");
	ЭлементУсловия.ПравоеЗначение = "1.Непоступило";
...
Функция ОпределитьСтатус(КоличествоЗаявка, КоличествоПриходныйОрдер, КоличествоВозврат)
	СтатусЗаявки =  "";
	Если КоличествоПриходныйОрдер = 0 ТОгда 
		СтатусЗаявки = "1.Непоступило";        //не красим
		Возврат СтатусЗаявки;
	КонецЕсли;
	
	Если КоличествоПриходныйОрдер = КоличествоЗаявка И КоличествоВозврат = 0 ТОгда 
		СтатусЗаявки = "2.Поступило полностью (ордер есть нет возврата)"; //розовый
		Возврат СтатусЗаявки;
	КонецЕсли;
	
	Если НЕ КоличествоПриходныйОрдер = КоличествоЗаявка И КоличествоВозврат = 0 ТОгда 
		СтатусЗаявки = "3.Поступило есть изменения (ордер есть, количество отличается, возврата нет)"; //розовый
		Возврат СтатусЗаявки;
	КонецЕсли;
	
	Если КоличествоПриходныйОрдер = КоличествоЗаявка И НЕ КоличествоВозврат = КоличествоЗаявка ТОгда 
		СтатусЗаявки = "4. Обработано есть изменения (ордер есть, возврат есть, количество в возврате отличается)"; //розовый
		Возврат СтатусЗаявки;
	КонецЕсли;
	
	Если  КоличествоПриходныйОрдер = КоличествоЗаявка И КоличествоВозврат = КоличествоЗаявка ТОгда 
		СтатусЗаявки = "5. Обработано (закрыто полностью)";  //зеленым
		Возврат СтатусЗаявки;
	КонецЕсли;
	
	Если  НЕ КоличествоПриходныйОрдер = КоличествоЗаявка И НЕ КоличествоВозврат = КоличествоЗаявка ТОгда 
		СтатусЗаявки = "6. Требуется корр заявки (требует корректировки заявки менеджером)";    //синим
		Возврат СтатусЗаявки;
	КонецЕсли;
	
	
	Возврат "6. Требуется корр заявки (требует корректировки заявки менеджером)";
...
//|ВЫБРАТЬ
//|	ПриходныйОрдерНаТоварыТовары.Ссылка.Распоряжение КАК Распоряжение,
//|	КОЛИЧЕСТВО(ЕСТЬNULL(ПриходныйОрдерНаТоварыТовары.Количество, 0)) КАК КоличествоОрдер
//|ПОМЕСТИТЬ ВТ_Ордер
//|ИЗ
...
//|ВЫБРАТЬ
//|	ВТ_Заявка.Ссылка КАК Ссылка,
//|	СУММА(ВТ_Заявка.КоличествоЗаявка) КАК КоличествоЗаявка,
//|	СУММА(ВТ_Ордер.КоличествоОрдер) КАК КоличествоПриходныйОрдер,
//|	СУММА(ВТ_Возврат.КоличествоВозврат) КАК КоличествоВозврат
//|ИЗ
//|	ВТ_Заявка КАК ВТ_Заявка
//|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Ордер КАК ВТ_Ордер
//|		ПО ВТ_Заявка.Ссылка = ВТ_Ордер.Распоряжение
