...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтруктураДанных.Свойство("Товары") Тогда
		...
		Если ЗначениеЗаполнено(СтруктураДанных.СтруктураДействий) Тогда
			...
			Если СтруктураДанных.СтруктураДействий.Свойство("УстановитьПартнера", ПараметрДействия) Тогда
				Объект.Партнер = ПараметрДействия;
			КонецЕсли;
...
//ПРФТ 20190730++
&НаСервере
Процедура ОбновитьПартнеровПоШК()
	//Для Каждого ТекСТрока Из Объект.Товары Цикл
	//	Запрос = Новый Запрос("ВЫБРАТЬ
	//	|	ШтрихкодыНоменклатуры.Партнер КАК Партнер
	//	|ИЗ
	//	|	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	//	|ГДЕ
	//	|	ШтрихкодыНоменклатуры.Штрихкод = &Штрихкод");
	//	Запрос.УстановитьПараметр("Штрихкод",ТекСТрока.Штрихкод);
	//	ВЫборка = Запрос.Выполнить().Выбрать();
	//	Если Выборка.Следующий() ТОгда
	//		ТекСтрока.Партнер = Выборка.Партнер;
	//	Иначе
	//		ТекСТрока.Партнер = Справочники.Партнеры.ПустаяСсылка();
	//	КонецЕсли;
	//КонецЦикла;
КонецПроцедуры
//ПРФТ 20190730--
...
Функция ПолучитьПараметры()
	...
	//18.05.2017 18:00:28
	//
	Товары.Колонки.Добавить("Импортер", Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("ШтрихкодПартнера", Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("НомерГТДИмпортер", Новый ОписаниеТипов("Строка"));
	Товары.Колонки.Добавить("НомерГТДДатаПроизводства", Новый ОписаниеТипов("Дата"));
	//}Кожемякин А.Г.
...
Процедура УстановитьОбязательныеПоляДанных(СтруктураНастроек, ДобавитьШаблоны = Ложь)
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//05.09.2017 17:18:51
	СтруктураНастроек.ОбязательныеПоля.Добавить("НомерГТДИмпортер");
	СтруктураНастроек.ОбязательныеПоля.Добавить("НомерГТДДатаПроизводства");
	//}Кожемякин А.Г.
	
	СтруктураНастроек.ОбязательныеПоля.Добавить("ШтрихкодПартнера");
	Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры") Тогда
...
Процедура ЗаполнитьТаблицуТоваровНаСервере(СохранятьШаблоныИКоличество, ОбновлениеДанныхПоВыбраннымТоварам = Ложь)
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.Таблица Цикл
		...
		//
		Попытка
			НоваяСтрока.Импортер 			 	 = СтрокаТЧ.Импортер;
		ИСключение
		КонецПопытки;
		Попытка
			НоваяСтрока.НомерГТДИмпортер 		 = СтрокаТЧ.НомерГТДИмпортер;
		ИСключение
		КонецПопытки;
		Попытка
			НоваяСтрока.НомерГТДДатаПроизводства = СтрокаТЧ.НомерГТДДатаПроизводства;
		ИСключение
		КонецПопытки;
		Попытка
			НоваяСтрока.ШтрихкодПартнера 			 = СтрокаТЧ.ШтрихкодПартнера;
		ИСключение
		КонецПопытки;
		//}Кожемякин А.Г.
		...
		//Если ЗначениеЗаполнено(Объект.Партнер) Тогда
			Если ПолучитьФункциональнуюОпцию("ИспользоватьУпаковкиНоменклатуры") Тогда
				ТекУпаковка             = СтрокаТЧ.Упаковка;
			Иначе
				ТекУпаковка				= Справочники.УпаковкиНоменклатуры.ПустаяСсылка();
			КонецЕсли;
			Если ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры") Тогда
			...
			КонецЕсли;
			//НоваяСтрока.Штрихкод = ПолучитьШтрихКодТовара(СтрокаТЧ.Штрихкод, Объект.Партнер, СтрокаТЧ.Номенклатура, ТекХарактеристика, ТекУпаковка);
			НоваяСтрока.ШтрихкодПартнера = ПолучитьШтрихКодТовара(СтрокаТЧ.ШтрихкодПартнера, Объект.Партнер, СтрокаТЧ.Номенклатура, ТекХарактеристика, ТекУпаковка);
		//КонецЕсли;
		...
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//28.12.2017 18:07:02
		НоваяСтрока.КодЦС =  дп_ДополнительныеПроцедуры.ПолучитьКодЦС(СтрокаТЧ.Номенклатура, СтрокаТЧ.Характеристика);		
		//}Кожемякин А.Г.
		...
		///ПРФТ++
		Если НЕ НоваяСтрока.ШтрихкодПартнера = "" ТОгда
			НоваяСтрока.ШтрихКод = 	НоваяСтрока.ШтрихкодПартнера;
			НоваяСтрока.Партнер = Объект.Партнер;
			НоваяСтрока.ШтрихкодПартнера = "";
		КонецЕсли;
	...
	//ПРФТ++
	Для Каждого Текстрока Из Объект.Товары Цикл
		ТЕкСтрока.БольшеОдногоШК   = Ложь;
		Если ЗначениеЗаполнено(Текстрока.Партнер) ТОгда Продолжить; КонецЕсли;
		ЗАпрос = Новый Запрос("ВЫБРАТЬ
		...
		Если Результат.Количество() > 1 Тогда
			ТекСтрока.ШтрихКод = Результат[0].Штрихкод;
				ТЕкСтрока.БольшеОдногоШК   = Истина;
	...
	ЭлементУсловия.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.Партнер");
	ЭлементУсловия.ПравоеЗначение = Справочники.Партнеры.ПустаяСсылка();
	...
	ЭлементУсловия                = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементУсловия.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары.БольшеОдногоШК");
...
&НаСервере
Функция  ПолучитьШтрихКодТовара(ТекШтрихкод, ТекПартнер, ТекНоменклатура, ТекХарактеристика, ТекУпаковка)
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	               |ГДЕ
	               |	ВЫБОР КОГДА &ОтборПартнер ТОГДА ШтрихкодыНоменклатуры.Партнер = &Партнер ИНАЧЕ ШтрихкодыНоменклатуры.Партнер <> Значение(Справочник.Партнеры.ПустаяСсылка) КОнец
	...
	Запрос.УстановитьПараметр("ОтборПартнер", ЗначениеЗаполнено(ТекПартнер));
	...
	//               |	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	//               |ГДЕ
	//               |	ШтрихкодыНоменклатуры.Партнер = &Партнер
	//               |	И ШтрихкодыНоменклатуры.Номенклатура = &Номенклатура
	//               |	И ШтрихкодыНоменклатуры.Характеристика = &Характеристика
	//               //|	И ШтрихкодыНоменклатуры.Упаковка = &Упаковка
	//			   |";
	...
	//		Возврат Выборка.Штрихкод;
		Иначе
			//Кожемякин А.Г. agkozhemyakin@gmail.com {
			//27.09.2016 14:48:57
			//устанавливаем пустой штрихкод если не найден
...
Процедура УпорядочитьНаСервере()
	ВременнаяТаблица = Объект.Товары.Выгрузить();
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(РазмещениеОсновнаяЯчейка.Ячейка.Код, 0) КАК ПорядокОбхода
		|ИЗ
		|	ВремТовары КАК ТаблицаТовары
		...
		|			И (РазмещениеОсновнаяЯчейка.Помещение = ЗНАЧЕНИЕ(Справочник.СкладскиеПомещения.ПустаяСсылка))
		...
		|			И (РазмещениеОстальныеЯчейки.Помещение = ЗНАЧЕНИЕ(Справочник.СкладскиеПомещения.ПустаяСсылка))
		|
		|УПОРЯДОЧИТЬ ПО
		|	РабочийУчасток,
		|	ПорядокОбхода,
	...
	Запрос.УстановитьПараметр("ИспользоватьСкладскиеПомещения", Объект.Склад.ИспользоватьСкладскиеПомещения);
		
	РезультатЗапроса = Запрос.Выполнить();
	
	ВременнаяТаблица.Колонки.Добавить("РабочийУчасток", Новый ОписаниеТипов("СправочникСсылка.РабочиеУчастки"));
	ВременнаяТаблица.Колонки.Добавить("ПорядокОбхода", Новый ОписаниеТипов("Строка"));
	
	Выборка = РезультатЗапроса.Выбрать();
	ПорядокОбхода = 0;
	Пока Выборка.Следующий() Цикл
		Попытка
			СтрокаТовары = ВременнаяТаблица.Получить(Выборка.НомерСтроки-1);	
			СтрокаТовары.РабочийУчасток = Выборка.РабочийУчасток;
			СтрокаТовары.ПорядокОбхода = Выборка.ПорядокОбхода;
			//ПорядокОбхода = ПорядокОбхода + 1;
		Исключение
		КонецПопытки;
	КонецЦикла;
	
	ВременнаяТаблица.Сортировать("ПорядокОбхода");
	
	Объект.Товары.Загрузить(ВременнаяТаблица);
...
&НаСервере
Процедура УпорядочитьНаСервереПоОтбору()
	ВременнаяТаблица = Объект.Товары.Выгрузить();
	...
	Запрос.Текст = 
		...
		|	ОтборРазмещениеТоваровТоварыОтбор.НомерСтроки
		...
		|	ТаблицаТовары.Номенклатура КАК Номенклатура,
		|	ТаблицаТовары.НомерСтроки,
		|	ОтборРазмещениеТоваровТоварыОтбор.НомерСтроки КАК НомерСтрокиОтбора
		|ИЗ
		|	ВремТовары КАК ТаблицаТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТоварыОтбора КАК ОтборРазмещениеТоваровТоварыОтбор
		|		ПО ТаблицаТовары.Номенклатура = ОтборРазмещениеТоваровТоварыОтбор.Номенклатура
		|			И ТаблицаТовары.Характеристика = ОтборРазмещениеТоваровТоварыОтбор.Характеристика
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтрокиОтбора,
		|	Номенклатура,
	...
	//Запрос.УстановитьПараметр("Склад", Объект.Склад);
	//Запрос.УстановитьПараметр("ИспользоватьСкладскиеПомещения", Объект.Склад.ИспользоватьСкладскиеПомещения);
	Запрос.УстановитьПараметр("Отбор", ОтборРазмещениеДляСортировки);	
	РезультатЗапроса = Запрос.Выполнить();
	
	ВременнаяТаблица.Колонки.Добавить("РабочийУчасток", Новый ОписаниеТипов("СправочникСсылка.РабочиеУчастки"));
	ВременнаяТаблица.Колонки.Добавить("ПорядокОбхода", Новый ОписаниеТипов("Число"));
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			СтрокаТовары = ВременнаяТаблица.Получить(Выборка.НомерСтроки-1);	
			//СтрокаТовары.РабочийУчасток = Выборка.РабочийУчасток;
			СтрокаТовары.ПорядокОбхода = Выборка.НомерСтрокиОтбора;
			//ПорядокОбхода = ПорядокОбхода + 1;
		Исключение
		КонецПопытки;
	КонецЦикла;
