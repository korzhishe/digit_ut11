...
#Область ВыполнениеЗапроса
	...
	Если Режим = "Товары" Тогда
		...
		Если ИсходныеДанные <> Неопределено Тогда
			...
			|		0 КАК Количество,
			|		"""" КАК Импортер,
			|		"""" КАК ШтрихкодПартнера,
			|		"""" КАК НомерГТДИмпортер,
			|		"""" КАК НомерГТДДатаПроизводства)";
			...
			//26.05.2017 10:05:09
			Если ИсходныеДанные.Колонки.Найти("Импортер") = Неопределено Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИсходныеДанные.Импортер", """""");		
			КонецЕсли;
			Если ИсходныеДанные.Колонки.Найти("ШтрихкодПартнера") = Неопределено Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИсходныеДанные.ШтрихкодПартнера", """""");		
			КонецЕсли;
			Если ИсходныеДанные.Колонки.Найти("НомерГТДИмпортер") = Неопределено Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИсходныеДанные.НомерГТДИмпортер", """""");		
			КонецЕсли;
			Если ИсходныеДанные.Колонки.Найти("НомерГТДДатаПроизводства") = Неопределено Тогда
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИсходныеДанные.НомерГТДДатаПроизводства", """""");		
			КонецЕсли;
			//}Кожемякин А.Г.
...
#Область ФормированиеТабличногоДокумента
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		...
		Для Каждого СтрокаТовары Из СтруктураРезультата.Таблица Цикл
			...
			Если СтрокаТовары[ПечатнаяФорма.ИмяКолонкиКоличество] > 0 И (СтрокаТовары[ПечатнаяФорма.ИмяКолонкиШаблон] = ПечатнаяФорма.Шаблон или доп_ОднаФорма) Тогда
				...
				Для каждого ПараметрШаблона Из СтруктураШаблона.ПараметрыШаблона Цикл
					//Кожемякин А.Г. agkozhemyakin@gmail.com {
					//23.08.2017 10:52:14
					Если ПараметрШаблона.Ключ = "КодЦС" Тогда
					
						Область.Параметры[ПараметрШаблона.Значение] = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(СтрокаТовары.Номенклатура, СтрокаТовары.Характеристика);	
					...
					//Кожемякин А.Г. agkozhemyakin@gmail.com {
					//23.08.2017 10:52:14
					Если ПараметрШаблона.Ключ = "НомерГТДДатаПроизводства" Тогда
					
						Область.Параметры[ПараметрШаблона.Значение] = Формат(СтрокаТовары.НомерГТДДатаПроизводства,"ДФ=MM.yyyy");	
				...
				Для каждого Рисунок Из Область.Рисунки Цикл
					...
					//07.06.2017 12:38:14
					//
					Если СтрНайти(Рисунок.Имя, "ШКПартнера") = 1 Тогда
						
						ЗначениеШтрихкода = СтрокаТовары.ШтрихкодПартнера;
						Если ЗначениеЗаполнено(ЗначениеШтрихкода) Тогда
