...
Процедура ЗаполнитьРеквизитыШапкиТОРГ12(ДанныеПечати, Макет, ТабличныйДокумент, ВыводитьКПП = Ложь, ВыводитьКПППоставщикПлательщик = Ложь)
	...
	Иначе
		...
		//Кожемякин А. 15.05.2014
		Если ВыводитьКПП Тогда
	...
	Если ВыводитьКПП Тогда
		...
		"ПолноеНаименование,ИНН,КПП,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		ОбластьМакета.Параметры.ПредставлениеПоставщика  = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике,"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		ОбластьМакета.Параметры.ПредставлениеПлательщика = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе,"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
	ИначеЕсли ВыводитьКПППоставщикПлательщик Тогда
		ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОГрузополучателе, 
		"ПолноеНаименование,ИНН,ФактическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		ОбластьМакета.Параметры.ПредставлениеПоставщика  = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПоставщике,"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
		ОбластьМакета.Параметры.ПредставлениеПлательщика = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПокупателе,"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,НомерСчета,Банк,БИК,КоррСчет");
	...
	Если ДанныеПечати.Партнер.Код = "00-00001656" Тогда
		СтруктураПараметров.Вставить("ПредставлениеГрузополучателя","ООО «Энфант», ИНН 7723384030, 109383, г. Москва, ул. Шоссейная, д. 90, стр. 73, р/с 40702810402440000645 в банке АО ""АЛЬФА-БАНК"" БИК 044525593 к/с 30101810200000000593");
	Иначе
...
Процедура ЗаполнитьТабличныйДокументТОРГ12(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	//Кожемякин А. 15.05.2014
	ВыводитьШтрихкод 	= ?(ПараметрыПечати.Свойство("Штрихкод"), ПараметрыПечати.Штрихкод, Ложь);
	ВыводитьКПП         = ?(ПараметрыПечати.Свойство("ВыводитьКПП"), ПараметрыПечати.ВыводитьКПП, Ложь);
	ВыводитьКПППоставщикПлательщик         = ?(ПараметрыПечати.Свойство("ВыводитьКПППоставщикПлательщик"), ПараметрыПечати.ВыводитьКПППоставщикПлательщик, Ложь);
	//Кожемякин А.
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ЗаполнитьРеквизитыШапкиТОРГ12(ДанныеПечати, Макет, ТабличныйДокумент, ВыводитьКПП, ВыводитьКПППоставщикПлательщик);
		...
		Попытка
			Если ВыводитьШтрихкод Тогда
				ОбластьЗаголовокТаблицы.Параметры.Код = "штрихкод";
			Иначе
				ОбластьЗаголовокТаблицы.Параметры.Код = "код";
			КонецЕсли;
		Исключение
			ЗаписьЖурналаРегистрации("Печать",УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Печать ТОРГ-12 ошибка указания поля код для печати.");
		КонецПопытки;
		//Кожемякин А.
		...
		Пока СтрокаТовары.Следующий() Цикл
			...
			//Кожемякин А. 15.05.2014
			Если ВыводитьШтрихкод Тогда
				ОбластьМакета.Параметры.НоменклатураКод = дп_ДополнительныеПроцедуры.ПолучитьШтрихКодНоменклатуры(СтрокаТовары.Номенклатура, СтрокаТовары.Характеристика);
				...
				ЗапросШК.УстановитьПараметр("Партнер",ДанныеПечати.Контрагент.Партнер);
				ЗапросШК.УстановитьПараметр("Номенклатура",СтрокаТовары.Номенклатура);
				ЗапросШК.УстановитьПараметр("Характеристика",СтрокаТовары.Характеристика);
				ШКК = ЗапросШК.Выполнить().Выбрать();
				Если ШКК.Следующий() Тогда
					//ОбластьСтрокаТаблицы.Параметры.Штрихкод = ШКК.Штрихкод;
					ОбластьМакета.Параметры.НоменклатураКод = ШКК.Штрихкод;
				Иначе
					ЗапросШК = Новый ЗАпрос("ВЫБРАТЬ
					|	ШтрихкодыНоменклатуры.Штрихкод КАК Штрихкод,
					|	ШтрихкодыНоменклатуры.Номенклатура КАК Номенклатура,
			...
			//19.09.2017 9:50:34
			Попытка
				ОбластьМакета.Параметры.КодЦС = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(СтрокаТовары.Номенклатура, СтрокаТовары.Характеристика);
			Исключение
			КонецПопытки;
			//}Кожемякин А.Г.
