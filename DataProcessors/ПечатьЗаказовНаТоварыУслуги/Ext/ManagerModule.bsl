...
#Область ВыводШапки
		...
		Если ЗначениеЗаполнено(ДанныеПечати.АдресДоставки) Тогда
			...
			//04.07.2016 15:15:52
			//вывод дополнительной информации в заказе
			УстановитьПараметр(ОбластьМакета, "ДопИнформацияОДоставке", ДанныеПечати.Ссылка.ДополнительнаяИнформацияПоДоставке);
			//}Кожемякин А.Г.
			ТабличныйДокумент.Вывести(ОбластьМакета);
		КонецЕсли;
...
#Область ВыводТаблицыТовары
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			Если ВыводитьКоды Тогда
				...
				//Кожемякин А.Г. agkozhemyakin@gmail.com {
				//25.08.2017 14:54:40
				ТекКодЦС = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(СтрокаТовары.Номенклатура, СтрокаТовары.ХарактеристикаСсылка);
				УстановитьПараметр(ОбластьКодовСтроки, "КодЦС", ТекКодЦС);				
				//}Кожемякин А.Г.
