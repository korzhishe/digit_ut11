...
Процедура ЗарегистрироватьШК()
	Попытка
		ШК = ЭтотОбъект.Выгрузить(,"Штрихкод")[0].Штрихкод;
		Набор = РегистрыСведений.РегистрацияШК.СоздатьНаборЗаписей();
		Набор.Отбор.Штрихкод.Установить(ШК);
		Набор.Прочитать();
		
		Если Набор.Количество() = 0 Тогда
			НЗ = Набор.Добавить();
			НЗ.ПериодРегистрации = ТекущаяДата();
			НЗ.Штрихкод = ШК;
			Набор.Записать();
		КонецЕсли;
	Исключение
