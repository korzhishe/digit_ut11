...
//
&НаКлиенте
Процедура КомплектПечатныхФормПринтерПриИзменении(Элемент)
	ТекущиеДанные = Элементы.КомплектПечатныхФорм.ТекущиеДанные;
	Если Не ТекущиеДанные = Неопределено Тогда
		ТекущиеДанные.ПринтерИзменен = Истина;
		УстановитьПринтерДляМакета(ТекущиеДанные.Представление, ТекущиеДанные.Принтер);
	КонецЕсли;	
КонецПроцедуры

&НаСервере
Процедура УстановитьПринтерДляМакета(Макет, Принтер)

	РегистрыСведений.дп_ПринтерыМакетов.УстановитьПринтерМакета(Макет, Принтер, Пользователи.ТекущийПользователь());	
