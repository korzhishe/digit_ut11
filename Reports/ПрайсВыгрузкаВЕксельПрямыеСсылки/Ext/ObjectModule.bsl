...
Процедура ПриКомпоновкеРезультата(ТабДок, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Для ТекНомер = 1 по 5 Цикл
		ОблШапкаРекв.Параметры.ИмяРеквизит = "Ссылка №"+ТекНомер;
		ТабДок.Присоединить(ОблШапкаРекв);
	КонецЦикла;
	...
	//	//ОблСтрока.параметры.характеристика = СокрЛП(Характ4);
		...
		Для ТекНомер = 1 по 5 Цикл
			...
			СсылкаНаФайл = "http://files.digit-style.ru/files/images/"+Результат.Артикул+"/"+ШтрихКод+"/"+ТекНомер+".jpg";			
			ОблСтрокаРекв.Параметры.ЗначениеРеквизита = СсылкаНаФайл;			
