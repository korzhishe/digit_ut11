...
Процедура ЗаполнитьИндексАкцизнойМарки(ТекущаяСтрока, ИмяКолонкиКоличество, КэшированныеЗначения = Неопределено) Экспорт
	...
	Если ТекущаяСтрока.МаркируемаяАлкогольнаяПродукция = Истина Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, "Упаковка") Тогда
			КоличествоЕдиницАкцизныхМарок = АкцизныеМаркиЕГАИСКлиентСерверПереопределяемый.КоличествоЕдиницАкцизныхМарок(
				ТекущаяСтрока.Упаковка,
				ТекущаяСтрока.КоличествоАкцизныхМарок,
				КэшированныеЗначения);
		Иначе
			КоличествоЕдиницАкцизныхМарок = ТекущаяСтрока.КоличествоАкцизныхМарок;
		КонецЕсли;
		
		Если  ТекущаяСтрока[ИмяКолонкиКоличество] = КоличествоЕдиницАкцизныхМарок
...
//
Процедура ЗаполнитьИндексАкцизнойМаркиДляСтрокиТабличнойЧасти(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ЗаполнитьИндексАкцизнойМарки", ПараметрыДействия) Тогда
		...
		АкцизныеМаркиКлиентСервер.ЗаполнитьИндексАкцизнойМарки(ТекущаяСтрока, ИмяКолонкиКоличество, КэшированныеЗначения);
...
Функция ПараметрыСканированияАкцизныхМарок() Экспорт
	...
	Результат.Вставить("КэшированныеЗначения"               , Неопределено);
