...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
		...
		//30.08.2016 13:57:30
		//Если ПечатнаяФорма.ИмяВРЕГ = "ЗАКАЗКЛИЕНТА" 
			//или ПечатнаяФорма.ИмяВРЕГ = "КОМПЛЕКТДОКУМЕНТОВ" Тогда
			ТекНастройкиПринтера = РегистрыСведений.дп_ПринтерыМакетов.ПолучитьПринтерМакета(ПечатнаяФорма.СинонимМакета, Пользователи.ТекущийПользователь());
			Если ЗначениеЗаполнено(ТекНастройкиПринтера) Тогда
				//ПечатнаяФорма.ТабличныйДокумент.ИмяПринтера = ТекНастройкиПринтера.Принтер;
				ЗаполнитьЗначенияСвойств(ПечатнаяФорма.ТабличныйДокумент, ТекНастройкиПринтера);
				//Результат = Новый Структура("ИмяПринтера,ПолеСверху,ПолеСнизу,ПолеСправа,ПолеСлева,ВысотаСтраницы,ШиринаСтраницы");
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ИмяПринтера) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ИмяПринтера = ТекНастройкиПринтера.ИмяПринтера;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ПолеСверху) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ПолеСверху = ТекНастройкиПринтера.ПолеСверху;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ПолеСнизу) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ПолеСнизу = ТекНастройкиПринтера.ПолеСнизу;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ПолеСправа) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ПолеСправа = ТекНастройкиПринтера.ПолеСправа;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ПолеСлева) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ПолеСлева = ТекНастройкиПринтера.ПолеСлева;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ВысотаСтраницы) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ВысотаСтраницы = ТекНастройкиПринтера.ВысотаСтраницы;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ШиринаСтраницы) Тогда
					ПечатнаяФорма.ТабличныйДокумент.ШиринаСтраницы = ТекНастройкиПринтера.ШиринаСтраницы;
				КонецЕсли;
				Если ЗначениеЗаполнено(ТекНастройкиПринтера.ВысотаСтраницы) 
					или ЗначениеЗаполнено(ТекНастройкиПринтера.ШиринаСтраницы) Тогда 
					//ПечатнаяФорма.ТабличныйДокумент.РазмерСтраницы = "Custom";
					//ПечатнаяФорма.ТабличныйДокумент.РазмерСтраницы = "ОСНОВНОЙ";
				КонецЕсли;
			КонецЕсли;	
		//КонецЕсли;
		//}Кожемякин А.Г.
