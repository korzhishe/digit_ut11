...
Процедура ЗаписатьСохранениеПрайсаГотово(Клиент, вSharePoint = Ложь)
	...
	Если вSharePoint Тогда
		ДопРеквизитДатаРассылки = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Дата сохранения прайса в SharePoint (Общие)");	
	Иначе	
		ДопРеквизитДатаРассылки = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Дата сохранения прайса на диск (Общие)");
	КонецЕсли;
...
Функция СформироватьПрайсыКакУРозничного(Клиенты = Неопределено
	...
	,ПредЗаказ 	 						= Ложь
	,Рассылать 							= Ложь
	,адреспочты							= Неопределено //указывается адрес почты на который нужно отправить прайсы. нужен если нужно отправить разово на адрес не указанный в форме партнера
	,сохрдиск							= Ложь //флаг. Если стина прайс будет сохранятся на диск
	,ТемаДляКлиента 					= Неопределено 
	,ПараметрыSharePoint 				= Неопределено //Параметры подключения к SharePoint для сохранения прайсов 
	,вSharePoint 						= Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Клиенты) Тогда
		Клиенты.Добавить(Справочники.Партнеры.РозничныйПокупатель);	
	КонецЕсли;	
	
	ТаблицаУчетокМенеджера 	= ПолучитьУчетныеЗаписиЭлектроннойПочтыМенеджера();
	ТаблицаКлиентов 		= ПолучитьСписокКлиентовДляРассылки(Клиенты, НаДиск, ПредЗаказ, вSharePoint);
	...
	ТаблицаКлиентовДляРассылки = ТаблицаКлиентов.Скопировать();
	ТаблицаКлиентов.Очистить();
	
	НайденнаяСтрока = ТаблицаКлиентовДляРассылки.Найти(Справочники.Партнеры.РозничныйПокупатель,"Клиент");
	
	Если Не НайденнаяСтрока = Неопределено Тогда
		НоваяСтрока = ТаблицаКлиентов.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденнаяСтрока);
		ТаблицаКлиентовДляРассылки.Удалить(НайденнаяСтрока);	
	КонецЕсли;
		
	ЕстьВидыЦен = Ложь;
	
	//ПроверитьНаличияВидовЦенДляРассылки(вхСписокЦен, ЕстьВидыЦен, ПредЗаказ, ТаблицаКлиентов);
	
	//ЕстьВидыЦен = Истина;
	
	Если вхСписокЦен = Неопределено Тогда
		Если ТаблицаКлиентовДляРассылки.Количество() <> 0 Тогда
			вхСписокЦен	= ПолучитьСписокЦенВсехКлиентов(ТаблицаКлиентовДляРассылки.ВыгрузитьКолонку("Клиент"), ПредЗаказ);
			Если вхСписокЦен.Количество() = 0 Тогда
				ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовДляВсехКакРозничный"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов (для всех как розничный) - не найдены виды цен для всех клиентов в текущей выгрузке");
			Иначе
				ЕстьВидыЦен = Истина;
			КонецЕсли;	
		КонецЕсли;
	...
	Если ЕстьВидыЦен Тогда
		СформироватьСохранитьОтправитьПрайсы(адреспочты
											, вхСписокЦен
											, ЕстьВидыЦен
											, Отладка
											, ПрайсыКлиентов
											...
											, СегментНоменклатуры
											, сохрдиск
											, СписокНоменклатурыДляРассылки
											, СтрокиМатрицыАппаратов
											, ТаблицаКлиентов
											, ТаблицаПочтовыхАдресов
											, ТаблицаУчетокМенеджера
											, УникальныйИдентификаторДляФайлов
											, ТемаДляКлиента
											, ПараметрыSharePoint
											, вSharePoint
											, ТаблицаКлиентовДляРассылки);
	...
	Если Не сохрдиск и Не вSharePoint и не Рассылать Тогда
...
Функция СформироватьПрайсы(Клиенты = Неопределено
	...
	,сохрдиск							= Ложь
	,ТемаДляКлиента = Неопределено 
	,ПараметрыSharePoint 				= Неопределено
	,вSharePoint 						= Ложь) Экспорт
...
Процедура СформироватьСохранитьОтправитьПрайсы(Знач адреспочты
												, Знач вхСписокЦен
												, Знач ЕстьВидыЦен
												, Знач Отладка
												, Знач ПрайсыКлиентов
												, Знач ПредЗаказ
												, Знач ПроверкаДатаВыпуска
												, Знач Рассылать
												, СегментНоменклатуры
												, Знач сохрдиск
												, СписокНоменклатурыДляРассылки
												, Знач СтрокиМатрицыАппаратов
												, Знач ТаблицаКлиентов
												, Знач ТаблицаПочтовыхАдресов
												, Знач ТаблицаУчетокМенеджера
												, Знач УникальныйИдентификаторДляФайлов
												, ТемаДляКлиента
												, ПараметрыSharePoint
												, вSharePoint
												, ТаблицаКлиентовДляРассылки = Неопределено)
	
	Перем EmailОсновногоМенеджера, ExcelПриложение, Адрес, АдресНаДиске, АдресНаДискеШаблон, ВсегоКолонок, ВсегоСтрок, Выборка, ВыборкаСРазбивкой, ДанныеПоПрайсамКлиента, ДанныеФайла, ДвоичныеДанные, ДвоичныеДанныеФайл, ДД, ЕстьВыборкаПоЦене, ЕстьЛист, Заголовок, индСтрок, Клиент, КлиентНаименование, Книга, КоличествоСтрок, Лист, МассивКОМ, МассивПрайсов, МассивПрайсовАдрес, МассивПрайсовАдреса, МенеджерВременТаблиц, НаименованиеКлиента, НомерСтроки, Отсрочка, Ротация, СоответсвиеЦеновыхГрупп, СоответствиеЦенаФайл, СоответствиеЦенаФайлАдрес, СписокЦен, Строка, СтрокаТаблицы, Строки, ТекБренд, ТекДата, ТекЛист, ТекНаименованиеЦеноваяГруппа, ТекСегментНоменклатуры, ТекСтрока, ТекЭлемент, ТелефонОсновногоМенеджера, тт_Цена, тт_Цена_тт, ФайлДляРассылки, ФормаДоговора, ФорматДаты, Цена, ЦеныПоЦеновымГруппам, Ячейка;
	
	Если ТаблицаКлиентов.Количество() <> 0 И Отладка И ЕстьВидыЦен Тогда
		
		СоздатьComObjectExcel(ExcelПриложение, ПредЗаказ);
		
		СегментНоменклатуры 			= Неопределено;
		СписокНоменклатурыДляРассылки 	= Неопределено;
		МенеджерВременТаблиц 			= Неопределено;
		
		СформироватьТаблицыЕксельПрайсов(ExcelПриложение
										, адреспочты
										, вхСписокЦен
										, МенеджерВременТаблиц
										, ПрайсыКлиентов
										, ПредЗаказ
										, ПроверкаДатаВыпуска
										, Рассылать
		...
		ЗакрытьЕксель(ExcelПриложение, ПредЗаказ);
		
	Иначе
		//Записать в лог файл
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены клиенты для рассылки");
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗакрытьЕксель(ExcelПриложение, ПредЗаказ)
	
	Попытка
		ExcelПриложение.DisplayAlerts = 1;
		ExcelПриложение.Quit();
		ExcelПриложение = Неопределено;
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовЕксель"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - закрытие ексель");
	Исключение
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовЕксель"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось закрыть Excel");
	КонецПопытки;
	
КонецПроцедуры

Процедура СформироватьТаблицыЕксельПрайсов(Знач ExcelПриложение
											, Знач адреспочты
											, Знач вхСписокЦен
											, МенеджерВременТаблиц
											, Знач ПрайсыКлиентов
	...
	Перем EmailОсновногоМенеджера, Адрес, АдресНаДиске, АдресНаДискеШаблон, ВсегоКолонок, ВсегоСтрок, Выборка, ВыборкаСРазбивкой, ДанныеПоПрайсамКлиента, ДанныеФайла, ДвоичныеДанные, ДвоичныеДанныеФайл, ДД, ЕстьВыборкаПоЦене, ЕстьЛист, Заголовок, индСтрок, Клиент, КлиентНаименование, Книга, КоличествоСтрок, Лист, МассивКОМ, МассивПрайсов, МассивПрайсовАдрес, МассивПрайсовАдреса, НаименованиеКлиента, НомерСтроки, Отсрочка, Ротация, СоответсвиеЦеновыхГрупп, СоответствиеЦенаФайл, СоответствиеЦенаФайлАдрес, СписокЦен, СтрокаТаблицы, Строки, ТекБренд, ТекДата, ТекЛист, ТекНаименованиеЦеноваяГруппа, ТекСегментНоменклатуры, ТекСтрока, ТекЭлемент, ТелефонОсновногоМенеджера, тт_Цена, тт_Цена_тт, ФайлДляРассылки, ФормаДоговора, ФорматДаты, Цена, ЦеныПоЦеновымГруппам, Ячейка;
	
	Для Каждого ТекСтрока Из ТаблицаКлиентов Цикл
		
		Если Рассылать и ТаблицаКлиентовДляРассылки = Неопределено Тогда
		...
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - начало обработки клиента " + ТекСтрока.Клиент);
		
		Клиент 			= ТекСтрока.Клиент;
		ФайлДляРассылки = ТекСтрока.Файл;
		Если ФайлДляРассылки = Неопределено Тогда
			//Записать в лог файл
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найден файл прайса для клиента" + ТекСтрока.Наименование);
			Продолжить;
		КонецЕсли;
		
		Если вхСписокЦен = Неопределено Тогда //edb 2016-02-11
			СписокЦен 	= ПолучитьСписокЦен(Клиент, ПредЗаказ);
		Иначе
		...
		Если СписокНоменклатурыДляРассылки = Неопределено Тогда //edb 2016-02-11
			...
			Если Не ЗначениеЗаполнено(ТекСегментНоменклатуры) Тогда
				//Записать в лог файл
				ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены сегменты номенклатуры для клиента" + ТекСтрока.Наименование);
				СписокНоменклатурыДляРассылки = Неопределено;
				Продолжить;
			Иначе 
				Если ТекСегментНоменклатуры <> СегментНоменклатуры Тогда
					...
					МенеджерВременТаблиц			= ПолучитьВременныеТаблицы(ПредЗаказ, СписокНоменклатурыДляРассылки);
					//КонецЕсли;
				КонецЕсли;	
			КонецЕсли;
		КонецЕсли;
		...
		Если СписокЦен.Количество() = 0 Тогда
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены виды цен для клиента" + ТекСтрока.Наименование);
			Продолжить;
		Иначе
			ПолучитьНаименованиеКлиента(Клиент, НаименованиеКлиента);
			...
			СформироватьПрайсыПоВидамЦен(ExcelПриложение
										, АдресНаДискеШаблон
										, ВыборкаСРазбивкой
										, ДанныеФайла
										, ДвоичныеДанные
			...
			ОтправитьСохранитьПрайсы(адреспочты
										, Клиент
										, МассивПрайсов
										, ПрайсыКлиентов
										, Рассылать
		...
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - конец обработки клиента " + ТекСтрока.Клиент);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ОтправитьСохранитьПрайсы(Знач адреспочты
									, Знач Клиент
									, Знач МассивПрайсов
									, Знач ПрайсыКлиентов
									, Знач Рассылать
									, Знач СоответствиеЦенаФайл
									, Знач сохрдиск
									, Знач ТаблицаПочтовыхАдресов
									, Знач ТаблицаУчетокМенеджера
									, Знач ТекСтрока
	...
	Перем Адрес, ДанныеПоПрайсамКлиента, ДвоичныеДанныеФайл, МассивПрайсовАдрес, СоответствиеЦенаФайлАдрес, ТекЭлемент, тт_Цена;
	
	Если Рассылать Тогда
		Если ТаблицаКлиентовДляРассылки = Неопределено Тогда
			ОтправитьПрайсКлиенту(адреспочты, Клиент, МассивПрайсов, СоответствиеЦенаФайл, ТаблицаПочтовыхАдресов, ТаблицаУчетокМенеджера, ТекСтрока, ТемаДляКлиента);
		Иначе
			Для Каждого СтрокаКлиентДляРассылки из ТаблицаКлиентовДляРассылки Цикл
				МассивВидовЦенКлиента = ПолучитьСписокЦен(СтрокаКлиентДляРассылки.Клиент, Ложь);
				ОтправитьПрайсКлиенту(адреспочты, СтрокаКлиентДляРассылки.Клиент, МассивПрайсов, СоответствиеЦенаФайл, ТаблицаПочтовыхАдресов, ТаблицаУчетокМенеджера, ТекСтрока, ТемаДляКлиента, МассивВидовЦенКлиента);	
			КонецЦикла;	
		КонецЕсли;	
	...
	Иначе
		...
		Иначе
			...
			МассивДанныеПоПрайсамКлиента = Новый Соответствие;
			...
			Для Каждого ТекЭлемент Из МассивПрайсов Цикл
				...
				СписокКлиентов 	= ПолучитьСписокКлиентовПоВидуЦен(ТаблицаКлиентовДляРассылки, тт_Цена);
				
				Для Каждого СтрокаКлиентовДляРассылки из СписокКлиентов Цикл
					ТекКлиент = СтрокаКлиентовДляРассылки;
					
					ДанныеПоПрайсамКлиента = МассивДанныеПоПрайсамКлиента.Получить(ТекКлиент);
					
					Если ДанныеПоПрайсамКлиента = Неопределено Тогда
						СоответствиеЦенаФайлАдрес 	= Новый Соответствие;
						МассивПрайсовАдрес 			= Новый Массив;
						ДанныеПоПрайсамКлиента 		= Новый Структура("МассивПрайсов, СоответствиеЦенаФайл, НаименованиеКлиента", МассивПрайсовАдрес, СоответствиеЦенаФайлАдрес, ТекКлиент.Наименование);
					Иначе
						СоответствиеЦенаФайлАдрес 	= ДанныеПоПрайсамКлиента.СоответствиеЦенаФайл;
						МассивПрайсовАдрес 			= ДанныеПоПрайсамКлиента.МассивПрайсов;
					КонецЕсли;   
					
					МассивПрайсовАдрес.Добавить(Адрес);
					СоответствиеЦенаФайлАдрес.Вставить(Адрес, тт_Цена);
					
					МассивДанныеПоПрайсамКлиента.Вставить(ТекКлиент, ДанныеПоПрайсамКлиента);
				КонецЦикла;	
				
			КонецЦикла;
			
			Для Каждого ЗначениеПрайсы из МассивДанныеПоПрайсамКлиента Цикл
				ПрайсыКлиентов.Вставить(ЗначениеПрайсы.Ключ, ЗначениеПрайсы.Значение);
			КонецЦикла;	
...
Процедура СформироватьПрайсыПоВидамЦен(Знач ExcelПриложение
										, АдресНаДискеШаблон
										, Знач ВыборкаСРазбивкой
										, Знач ДанныеФайла
										, Знач ДвоичныеДанные
										, Знач Клиент
										, Знач МассивПрайсов
										, Знач НаименованиеКлиента
										, Знач ПредЗаказ
										, Знач ПроверкаДатаВыпуска
										, Знач СоответствиеЦенаФайл
										, Знач сохрдиск
										, Знач СписокНоменклатурыДляРассылки
										, Знач СписокЦен
										, Строка
										, Знач СтрокиМатрицыАппаратов
										, Знач ТекСтрока
	...
	Перем EmailОсновногоМенеджера, АдресНаДиске, ВсегоКолонок, ВсегоСтрок, Выборка, ДД, ЕстьВыборкаПоЦене, ЕстьЛист, Заголовок, индСтрок, КлиентНаименование, Книга, КоличествоСтрок, Лист, МассивКОМ, НомерСтроки, Отсрочка, Ротация, СоответсвиеЦеновыхГрупп, СтрокаТаблицы, Строки, ТекБренд, ТекДата, ТекЛист, ТекНаименованиеЦеноваяГруппа, ТелефонОсновногоМенеджера, тт_Цена, тт_Цена_тт, ФормаДоговора, Цена, ЦеныПоЦеновымГруппам, Ячейка;
	...
	Для Каждого Цена Из СписокЦен Цикл
		Если СписокНоменклатурыДляРассылки <> Неопределено Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось открыть шаблон для клиента " + Клиент);
				Продолжить;
			КонецПопытки;
			
			НеЗаполнятьДанныеКлиента = ?(ТаблицаКлиентовДляРассылки = Неопределено, Ложь, Истина);
			
			ЗаполнитьЦеновоеПредложение(EmailОсновногоМенеджера, ЕстьЛист, Клиент, Книга, Лист, ПредЗаказ, ТекСтрока, ТелефонОсновногоМенеджера, Цена, НеЗаполнятьДанныеКлиента);
			
			ЗаполнитьПрайс(EmailОсновногоМенеджера, ЕстьЛист, Заголовок, Клиент, Лист, ПроверкаДатаВыпуска, СписокНоменклатурыДляРассылки, Строка, ТекСтрока, ТелефонОсновногоМенеджера, Цена, НеЗаполнятьДанныеКлиента);
			
			ЗаполнитьРазбивкуПоЛистам(EmailОсновногоМенеджера, ВыборкаСРазбивкой, Заголовок, Клиент, Книга, Лист, ПредЗаказ, ТекСтрока, ТелефонОсновногоМенеджера, Цена, НеЗаполнятьДанныеКлиента);
			
			ЗаполнитьРекомендацииПоРасширениюМатрицы(EmailОсновногоМенеджера, Заголовок, Клиент, Лист, ПроверкаДатаВыпуска, СписокНоменклатурыДляРассылки, Строка, ТекСтрока, ТелефонОсновногоМенеджера, Цена, НеЗаполнятьДанныеКлиента);
			
			ЗаполнитьМатрицуАппаратов(Клиент, Книга, ПроверкаДатаВыпуска, Строка, СтрокиМатрицыАппаратов, ТекСтрока);
			...
			Если сохрдиск или вSharePoint Тогда
				
				Если ТаблицаКлиентовДляРассылки = Неопределено  Тогда
					СохранитьПрайсНаДиск(АдресНаДиске
					, Клиент
					, НаименованиеКлиента
					, Цена
					, ПараметрыSharePoint
				...
				Иначе
					...
					СписокКлиентов 	= ПолучитьСписокКлиентовПоВидуЦен(ТаблицаКлиентовДляРассылки, Цена);
					
					Для Каждого СтрокаКлиентовДляРассылки из СписокКлиентов Цикл
						НаименованиеКлиентаДляРассылки = "";
						ПолучитьНаименованиеКлиента(СтрокаКлиентовДляРассылки, НаименованиеКлиентаДляРассылки);
						СохранитьПрайсНаДиск(АдресНаДиске
						, СтрокаКлиентовДляРассылки
						, НаименованиеКлиентаДляРассылки
						, Цена
						, ПараметрыSharePoint
						, вSharePoint);
...
Процедура СохранитьПрайсНаДиск(Знач АдресНаДиске
								, Знач Клиент
								, Знач НаименованиеКлиента
								, Знач Цена
								, ПараметрыSharePoint
	...
	Если  Клиент = Справочники.Партнеры.РозничныйПокупатель Тогда
		Если вSharePoint Тогда
			НаименованиеОсновногоМенеджера = "Общая";
			НаименованиеКлиента = "Прайсы для всех";
			//НаименованиеКлиентаКаталог = НаименованиеОсновногоМенеджера+"/"+НаименованиеКлиента;
			НаименованиеКлиентаКаталог = НаименованиеКлиента;
			ПараметрыSharePoint.Вставить("НаименованиеКлиента", НаименованиеКлиентаКаталог);
			ПараметрыSharePoint.Вставить("ИмяФайла","Прайс_" + тт_Цена_тт + ".xlsx");
			
			дп_ДополнительныеПроцедуры.ОтправитьДанныеНаSharePoint(ПараметрыSharePoint, ДД);
			ЗаписатьСохранениеПрайсаГотово(Клиент, вSharePoint);
		Иначе 
			СоздатьКаталог("C:\Price_auto\Общая\Прайсы для всех");
			Попытка
				ДД.Записать("C:\Price_auto\Общая\Прайсы для всех\Прайс_" + тт_Цена_тт + ".xlsx");
	...
	Иначе	
		...
		Иначе
			...
			Попытка
				ДД.Записать(НаименованиеКаталога+"\Прайс_" + тт_Цена_тт + ".xlsx");
				ЗаписатьСохранениеПрайсаГотово(Клиент);
			Исключение
				ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов",УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось сохранить на диск" + ОписаниеОшибки());
			КонецПопытки;
...
Процедура ЗаполнитьМатрицуАппаратов(Знач Клиент, Знач Книга, Знач ПроверкаДатаВыпуска, Строка, Знач СтрокиМатрицыАппаратов, Знач ТекСтрока)
	
	Перем ВсегоКолонок, ВсегоСтрок, ЕстьЛист, индСтрок, КоличествоСтрок, Лист, МассивКОМ, НомерСтроки, СтрокаТаблицы, ТекДата, Ячейка;
...
Процедура ЗаполнитьРекомендацииПоРасширениюМатрицы(Знач EmailОсновногоМенеджера
													, Знач Заголовок
													, Знач Клиент
													, Знач Лист
													, Знач ПроверкаДатаВыпуска
													...
													, Строка
													, Знач ТекСтрока
													, Знач ТелефонОсновногоМенеджера
													, Знач Цена
													, НеЗаполнятьДанныеКлиента = Ложь)
	
	Перем ВсегоКолонок, ВсегоСтрок, ЕстьЛист, индСтрок, КоличествоСтрок, МассивКОМ, НомерСтроки, СтрокаТаблицы, Строки, ТекДата, Ячейка;
	...
	Если ЕстьЛист Тогда
		...
		Иначе
			...
			Если Не НеЗаполнятьДанныеКлиента Тогда
...
Процедура ЗаполнитьРазбивкуПоЛистам(Знач EmailОсновногоМенеджера
									, Знач ВыборкаСРазбивкой
									, Знач Заголовок
									, Знач Клиент
									, Знач Книга
	...
	Перем Выборка, ЕстьВыборкаПоЦене, ЕстьЛист, НомерСтроки, Строки, ТекЛист, Ячейка;
	...
	Если Не ЕстьВыборкаПоЦене Тогда	
	...
	ИначеЕсли Не Выборка.Следующий() Тогда
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов",УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - нет данных для формирования прайса");
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЕстьЛист Тогда
				...
				ИначеЕсли ТекЛист = "Чехлы, накладки, стекла" Тогда
					...
					Иначе											
						ЗаполнитьЧехлы(Строки, Лист, НомерСтроки, ТекЛист, Книга);
					КонецЕсли;
				Иначе
...
Процедура ЗаполнитьПрайс(Знач EmailОсновногоМенеджера
						, Знач ЕстьЛист
						, Знач Заголовок
						, Знач Клиент
						, Знач Лист
	...
	Перем ВсегоКолонок, ВсегоСтрок, индСтрок, КоличествоСтрок, МассивКОМ, НомерСтроки, СтрокаТаблицы, Строки, ТекДата, Ячейка;
...
Процедура ЗаполнитьЦеновоеПредложение(Знач EmailОсновногоМенеджера
										, ЕстьЛист
										, Знач Клиент
										, Знач Книга
										, Лист
	...
	Перем КлиентНаименование, НомерСтроки, Отсрочка, Ротация, СоответсвиеЦеновыхГрупп, СтрокаТаблицы, ТекБренд, ТекНаименованиеЦеноваяГруппа, ФормаДоговора, ЦеныПоЦеновымГруппам, Ячейка;
	
	#Область ЦеновоеПредложение
	
	ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - начало ценовое предложение клиента " + ТекСтрока.Клиент);
	
	Попытка
		Лист 						= Книга.WorkSheets("Ценовое_предложение");
		ЕстьЛист = Истина
	Исключение
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось найти лист Ценовое предложение в шаблоне для клиента " + Клиент);
		ЕстьЛист = Ложь;
	КонецПопытки;
	...
	Если ЕстьЛист ТОгда
		...
		Если Не ПредЗаказ и Не НеЗаполнятьДанныеКлиента Тогда
		...
		Иначе
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось получить ценовые группы для листа Ценовое предложение");
		КонецЕсли;	
		...
		Если ЦеныПоЦеновымГруппам.Количество() Тогда
			...
			КонецЦикла; 
		Иначе
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - нет данных по ценовым группам для листа Ценовое предложение" + ТекСтрока.Наименование);
		КонецЕсли;	
	КонецЕсли;
	
	ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - конец ценовое предложение клиента " + ТекСтрока.Клиент);
	
	#КонецОбласти
	
КонецПроцедуры

Процедура ПолучитьНаименованиеКлиента(Знач Клиент, НаименованиеКлиента)
...
Процедура СоздатьComObjectExcel(ExcelПриложение, ПредЗаказ)
	...
	ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - открытие ексель.");
	
КонецПроцедуры

Процедура ПроверитьНаличияВидовЦенДляРассылки(Знач вхСписокЦен, ЕстьВидыЦен, Знач ПредЗаказ, Знач ТаблицаКлиентов)
	
	Перем СписокЦенВсехКлиентов;
	...
	Если вхСписокЦен = Неопределено и ТаблицаКлиентов.Количество() <> 0 Тогда
		...
		Если СписокЦенВсехКлиентов.Количество() = 0 Тогда
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсов"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены виды цен для всех клиентов в текущей выгрузке");
		Иначе
			ЕстьВидыЦен = Истина;
		КонецЕсли;	
	Иначе
...
Функция ПолучитьУчетныеЗаписиЭлектроннойПочтыМенеджера()
	
	Перем Запрос, ТаблицаУчетокМенеджера;
	...
	Возврат ТаблицаУчетокМенеджера;
...
Функция СформироватьПрайсыКосметика(Клиенты = Неопределено
	...
	Если вхСписокЦен = Неопределено и ТаблицаКлиентов.Количество() <> 0 Тогда
		СписокЦенВсехКлиентов 	= ПолучитьСписокЦенВсехКлиентов(ТаблицаКлиентов.ВыгрузитьКолонку("Клиент"), ПредЗаказ);
		Если СписокЦенВсехКлиентов.Количество() = 0 Тогда
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены виды цен для всех клиентов в текущей выгрузке");
		Иначе
			ЕстьВидыЦен = Истина;
		КонецЕсли;	
	КонецЕсли;
	...
	Если ТаблицаКлиентов.Количество() <> 0 И Отладка И ЕстьВидыЦен Тогда
		...
		ExcelПриложение.ScreenUpdating 	= 0;
		ExcelПриложение.EnableEvents 	= 0;
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовЕксельКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - открытие ексель.");
		...
		Для Каждого ТекСтрока Из ТаблицаКлиентов Цикл
			...
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - начало обработки клиента " + ТекСтрока.Клиент);
			
			Клиент 			= ТекСтрока.Клиент;
			ФайлДляРассылки = ТекСтрока.Файл;
			Если ФайлДляРассылки = Неопределено Тогда
				//Записать в лог файл
				ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найден файл прайса для клиента" + ТекСтрока.Наименование);
				Продолжить;
			КонецЕсли;
			...
			Если СписокНоменклатурыДляРассылки = Неопределено Тогда //edb 2016-02-11
				...
				Если Не ЗначениеЗаполнено(ТекСегментНоменклатуры) Тогда
					//Записать в лог файл
					ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены сегменты номенклатуры для клиента" + ТекСтрока.Наименование);
					СписокНоменклатурыДляРассылки = Неопределено;
					Продолжить;
				Иначе 
					Если ТекСегментНоменклатуры <> СегментНоменклатуры Тогда
			...
			Если СписокЦен.Количество() = 0 Тогда
				ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены виды цен для клиента" + ТекСтрока.Наименование);
				Продолжить;
			Иначе
				НаименованиеКлиента = Клиент.Наименование;
				//заменим спецсимволы \ / ? : * " > < |
				...
				Для Каждого Цена Из СписокЦен Цикл
					Если СписокНоменклатурыДляРассылки <> Неопределено Тогда
						...
						Попытка
							Книга 						= ExcelПриложение.WorkBooks.Open(СокрЛП(АдресНаДискеШаблон));
						Исключение
							ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось открыть шаблон для клиента " + Клиент);
							Продолжить;
						КонецПопытки;
						...
						Попытка
							...
							ЕстьЛист = Истина
						Исключение
							ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось найти лист Прайс в шаблоне для клиента " + Клиент);
							ЕстьЛист = Ложь;
							Продолжить;
						КонецПопытки;
						...
						Если ЕстьЛист Тогда
							...
							ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - начало прайс клиента " + ТекСтрока.Клиент);
							
							
							Строки 	= ПолучитьСтрокиДляЗаполненияФайлаКосметика(СписокНоменклатурыДляРассылки, Цена, Клиент,МенеджерВременТаблиц);
							Если Строки = Неопределено Тогда
								ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - нет данных для формирования прайса");
							Иначе
								Лист.UnProtect();
								НомерСтроки = 2;
						...
						ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - конец прайс клиента " + ТекСтрока.Клиент);
						...
						Если сохрдиск Тогда
							...
							Если  Клиент = Справочники.Партнеры.РозничныйПокупатель Тогда
								Попытка
									ДД.Записать("C:\Price_auto\Прайс_косметика_" + тт_Цена_тт + ".xlsx");
									ЗаписатьСохранениеПрайсаГотово(Клиент);
								Исключение
									ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось сохранить на диск" + ОписаниеОшибки());
								КонецПопытки;	
								//ДД.Записать("C:\Users\Administrator\Documents\Прайс_" + тт_Цена_тт + ".xlsx");
							Иначе	
								...
								//СоздатьКаталог("C:\Users\Administrator\Documents\"+НаименованиеКлиента);
								Попытка
									ДД.Записать("C:\Price_auto\"+НаименованиеКлиента+"\Прайс_косметика_" + тт_Цена_тт + ".xlsx");
									ЗаписатьСохранениеПрайсаГотово(Клиент);
								Исключение
									ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось сохранить на диск" + ОписаниеОшибки());
								КонецПопытки;
			...
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - конец обработки клиента " + ТекСтрока.Клиент);
		...
		Попытка
			...
			ExcelПриложение.Quit();
			ExcelПриложение = Неопределено;
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовЕксельКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Информация, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - закрытие ексель");
		Исключение
			ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовЕксельКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не удалось закрыть Excel");
		КонецПопытки;
		
	Иначе
		//Записать в лог файл
		ЗаписьЖурналаРегистрации("ОбработкаРассылкиПрайсовКосметика"+?(ПредЗаказ,"ПРЕДЗАКАЗ",""),УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Автоматическая рассылка прайсов - не найдены клиенты для рассылки");
	КонецЕсли;
...
Процедура ОтправитьПрайсКлиенту(Знач адреспочты
								, Знач Клиент
								, Знач МассивПрайсов
								, Знач СоответствиеЦенаФайл
								, Знач ТаблицаПочтовыхАдресов
	...
	Если ЗначениеЗаполнено(КомуПредставление) Тогда
		...
		Если ТемаДляКлиента = Неопределено Тогда
			ПараметрыПисьма.Вставить("Тема", "Прайс-лист ООО ""ДиджитСтайл"" от "+Формат(ТекущаяДата(),"ДФ=dd.MM.yy"));
		Иначе
			ПараметрыПисьма.Вставить("Тема", "Прайс-лист для " +Клиент.Наименование + " от " + Формат(ТекущаяДата(),"ДФ=dd.MM.yy"));
		КонецЕсли;
		...
		Для Каждого ТекЭлемент Из МассивПрайсов Цикл
			...
			//ПРФТ++
			//Рушится  рассылка прайсов менеджерам изза этого куска.  Ставлю  попытку
			//исправил ошибку и убрал
			//Попытка
				Если Не МассивВидовЦен = Неопределено Тогда
					Если МассивВидовЦен.Найти(тт_Цена) = Неопределено Тогда
						Продолжить;
					КонецЕсли;	
				КонецЕсли;
			//Исключение
...
Функция ПолучитьСписокКлиентовПоВидуЦен(ТаблицаКлиентовДляРассылки, Цена)
	Запрос =  Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	                       |	СоглашенияСКлиентами.Партнер КАК Клиент
	                       |ИЗ
	                       |	Справочник.СоглашенияСКлиентами КАК СоглашенияСКлиентами
	                       |ГДЕ
	                       |	СоглашенияСКлиентами.Партнер В(&Клиенты)
	                       |	И НЕ СоглашенияСКлиентами.Типовое
	                       |	И СоглашенияСКлиентами.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСКлиентами.Действует)
	                       |	И СоглашенияСКлиентами.ВидЦен = &ВидЦены");
		
		Запрос.УстановитьПараметр("Клиенты", ТаблицаКлиентовДляРассылки.ВыгрузитьКолонку("Клиент"));
		Запрос.УстановитьПараметр("ВидЦены", Цена);
		
		СписокКлиентов = Запрос.Выполнить().Выгрузить();
		Возврат СписокКлиентов.ВыгрузитьКолонку("Клиент");
КонецФункции	
	
Функция ПолучитьСписокЦен(Клиент, ПредЗаказ)
...
Функция ПолучитьСтрокиДляЗаполненияФайла(СписокНоменклатурыДляРассылки, Цена, Клиент)
	...
	|	ВЫБОР КОГДА ШКПроизводителя.Штрихкод ЕСТЬ NULL ТОГДА ЕСТЬNULL(ШК.Штрихкод, """") ИНАЧЕ ШКПроизводителя.Штрихкод КОНЕЦ КАК Артикул,
	...
	|		ИНАЧЕ ЕСТЬNULL(вт_СвободноДляПродажи.СвободноДляПродажи, 0)
	|	КОНЕЦ,
	|	ВЫБОР КОГДА ШКПроизводителя.Штрихкод ЕСТЬ NULL ТОГДА ЕСТЬNULL(ШК.Штрихкод, """") ИНАЧЕ ШКПроизводителя.Штрихкод КОНЕЦ
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаВыпуска УБЫВ,
	|	ДляМоделиУстройства,
...
Функция ПолучитьСтрокиДляЗаполненияФайлаКосметика(СписокНоменклатурыДляРассылки, Цена, Клиент, МенеджерВременТаблиц)
	...
	|	ВЫБОР КОГДА ШКПроизводителя.Штрихкод ЕСТЬ NULL ТОГДА ЕСТЬNULL(ШК.Штрихкод, """") ИНАЧЕ ШКПроизводителя.Штрихкод КОНЕЦ КАК ШтрихКод,
	|	ЕСТЬNULL(вт_Номенклатура.Номенклатура.Производитель, """") КАК Производитель,
...
Функция ПолучитьСтрокиДляЗаполненияФайлаПрайсСРазбивкойНовый4(СписокНоменклатурыДляРассылки, СписокЦен, Клиент, ПредЗаказ, МенеджерВременТаблиц)
...
Функция ПолучитьВременныеТаблицы(ПредЗаказ, СписокНоменклатурыДляРассылки)
...
Функция ПолучитьСписокКлиентовДляРассылки(Клиенты, НаДиск, ПредЗаказ, вSharePoint = Ложь)
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
    |	СоглашенияСКлиентами.Партнер КАК Клиент,
    |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СоглашенияСКлиентами.ВидЦен) КАК ВидЦенКоличество
    |ПОМЕСТИТЬ ТипыЦенПоКлиентам
    |ИЗ
    |	Справочник.СоглашенияСКлиентами КАК СоглашенияСКлиентами
    |ГДЕ
    |	СоглашенияСКлиентами.Партнер В
    |			(ВЫБРАТЬ
    |				ВремТаблКлиенты.Клиент
    |			ИЗ
    |				ВремТабл КАК ВремТаблКлиенты)
    |	И НЕ СоглашенияСКлиентами.Типовое
    |	И СоглашенияСКлиентами.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСоглашенийСКлиентами.Действует)
    |	И СоглашенияСКлиентами.ВидЦен <> ЗНАЧЕНИЕ(справочник.Видыцен.пустаяссылка)
    |
	...
	|	ЕСТЬNULL(ТипыЦенПоКлиентам.ВидЦенКоличество, 0) КАК КолВидовЦен
	|ИЗ
	|	ВремТабл КАК ВремТабл  
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТипыЦенПоКлиентам КАК ТипыЦенПоКлиентам
	|		ПО ВремТабл.Клиент = ТипыЦенПоКлиентам.Клиент
	|ГДЕ
	|	ЕСТЬNULL(ТипыЦенПоКлиентам.ВидЦенКоличество, 0) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВремТабл.Сегмент,
	|	ДатаОтправки");
	...
	Если вSharePoint Тогда
		Запрос.УстановитьПараметр("СвойствоДатаСохранения", ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Дата сохранения прайса в SharePoint (Общие)"));	
	Иначе	
...
Процедура ЗаполнитьОстальные(СтрокаТаблицы, Лист, НомерСтроки)
	...
	Пока СтрокаТаблицы.Следующий() Цикл
		НомерСтроки = НомерСтроки + 1;
		//Лист.Cells(НомерСтроки, 13).formulaLocal = "=ЕСЛИ([@[ЗАКАЗ (кол-во)]]>0,[@Стоимость]*[@[ЗАКАЗ (кол-во)]],"""")";
...
Процедура ЗаполнитьКабеля(СтрокаТаблицы, Лист, НомерСтроки)
	...
	//Добавить формулу
	Для Строка = 8 по КоличествоСтрок Цикл
		//Лист.Cells(Строка, 11).formulaLocal = "=ЕСЛИ([@[ЗАКАЗ (кол-во)]]>0;[@Стоимость]*[@[ЗАКАЗ (кол-во)]];"""")";
		Ячейка = Лист.Cells(Строка, 17);
		Ячейка.formulaLocal = "=ГИПЕРССЫЛКА("""+Ячейка.Value+""",""Ссылка на веб"")";
		//Лист.Hyperlinks.Add(Ячейка, Ячейка.Value);
		Ячейка = Лист.Cells(Строка, 18);
...
Процедура ЗаполнитьЧехлы(СтрокаТаблицы, Лист, НомерСтроки, ТекЛист, Книга)
	...
	//МассивКОМФормула = Новый COMSafeArray("VT_VARIANT", ВсегоКолонок, ВсегоСтрок);
	//МассивКОМФормулаСсылка = Новый COMSafeArray("VT_VARIANT", ВсегоКолонок, ВсегоСтрок);
	//МассивКОМФормулаСсылкаАрхив = Новый COMSafeArray("VT_VARIANT", ВсегоКолонок, ВсегоСтрок);
	...
	Пока СтрокаТаблицы.Следующий() Цикл
		...
		//МассивКОМ.SetValue(12, индСтрок,"=ЕСЛИ([@[ЗАКАЗ (кол-во)]]>0;[@Стоимость]*[@[ЗАКАЗ (кол-во)]];"""")");
		
		//МассивКОМФормула.SetValue(0,индСтрок,"=ЕСЛИ([@[ЗАКАЗ (кол-во)]]>0;[@Стоимость]*[@[ЗАКАЗ (кол-во)]];"""")");
		//МассивКОМФормулаСсылка.SetValue(0,индСтрок,"=ГИПЕРССЫЛКА("""+СтрокаТаблицы.СсылкаWeb+""";""Ссылка на веб"")");
		//МассивКОМФормулаСсылкаАрхив.SetValue(0,индСтрок,"=ГИПЕРССЫЛКА("""+СтрокаТаблицы.СсылкаНаАрхивФото+""";""Ссылка на архив фото"")");
	...
	//Лист.Range(Лист.Cells(НомерСтроки,13), Лист.Cells(КоличествоСтрок,13)).formulaLocal = "=ЕСЛИ([@[ЗАКАЗ (кол-во)]]>0,[@Стоимость]*[@[ЗАКАЗ (кол-во)]],"""")";
	//Лист.Range(Лист.Cells(НомерСтроки,13), Лист.Cells(КоличествоСтрок,13)).formulaLocal = МассивКОМФормула;
	//Лист.Range(Лист.Cells(НомерСтроки,20), Лист.Cells(КоличествоСтрок,20)).formulaLocal = МассивКОМФормулаСсылка;
	//Лист.Range(Лист.Cells(НомерСтроки,21), Лист.Cells(КоличествоСтрок,21)).formulaLocal = МассивКОМФормулаСсылкаАрхив;
	//Добавить гиперссылку в 2 колонки
	//Добавить формулу
	...
	Пока СтрокаТаблицы.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СсылкаWeb) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СсылкаНаАрхивФото) Тогда
...
Процедура ОбработчикРассылкиПрайсов() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ПРФТ_ПрайсыКРассылке.ДатаРегистрации КАК ДатаРегистрации,
	|	ПРФТ_ПрайсыКРассылке.ДатаОбработки КАК ДатаОбработки,
	|	ПРФТ_ПрайсыКРассылке.Пользователь КАК Пользователь,
	|	ПРФТ_ПрайсыКРассылке.Клиент КАК Клиент,
	|	ПРФТ_ПрайсыКРассылке.Почта КАК Почта,
	|	ПРФТ_ПрайсыКРассылке.докЗаказ КАК докЗаказ
	|ИЗ
	|	РегистрСведений.ПРФТ_ПрайсыКРассылке КАК ПРФТ_ПрайсыКРассылке
	|ГДЕ
	|	ПРФТ_ПрайсыКРассылке.ДатаОбработки = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)");
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.докЗаказ) Тогда
			
			Клиент  = Выборка.докЗаказ.Партнер;		
			Склад	= Выборка.докЗаказ.Склад;
			
			ВидЦены = Выборка.докЗаказ.Соглашение.ВидЦен;
			Если Не ЗначениеЗаполнено(ВидЦены) Тогда
				Возврат;	
			КонецЕсли;	
			
			СегментНоменклатуры = дп_РассылкаПрайсов.ПолучитьСегментНоменклатуры(Клиент);
			Если СегментНоменклатуры = Неопределено Тогда
				Возврат;
			КонецЕсли;
			
			НоменклатураСегмента = СегментыСервер.МассивЭлементов(СегментНоменклатуры);		
			Если НоменклатураСегмента.Количество() = 0 Тогда
				Возврат;
			КонецЕсли;	
			...
			|ПОМЕСТИТЬ вт_РанееПроданная
			|ИЗ
			|	РегистрНакопления.ВыручкаИСебестоимостьПродаж.Обороты(, , , АналитикаУчетаПоПартнерам.Партнер = &Клиент) КАК ВыручкаИСебестоимостьПродажОбороты
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|ПОМЕСТИТЬ вт_НоменклатураНеПроданнаяРанее
			|ИЗ
			|	вт_Номенклатура КАК вт_Номенклатура
			|ГДЕ
			|	НЕ (вт_Номенклатура.Номенклатура, вт_Номенклатура.Характеристика) В
			|				(ВЫБРАТЬ
			|					Т.Номенклатура,
			|					Т.Характеристика
			|				ИЗ
			|					вт_РанееПроданная КАК Т
			|		
			|				ОБЪЕДИНИТЬ ВСЕ
			|		
			|				ВЫБРАТЬ
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	МАКСИМУМ(ЛОЖЬ) КАК Пометка,
			|	вт_НоменклатураНеПроданнаяРанее.Номенклатура,
			|	вт_НоменклатураНеПроданнаяРанее.Характеристика,
			|	МАКСИМУМ(0) КАК Количество,
			|	вт_НоменклатураНеПроданнаяРанее.Номенклатура.ЕдиницаИзмерения КАК Единица,
			|	МАКСИМУМ(СвободныеОстаткиОстатки.ВНаличииОстаток) КАК Остаток,
			|	МАКСИМУМ(СвободныеОстаткиОстатки.ВНаличииОстаток - СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток - СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток) КАК Доступно,
			|	МАКСИМУМ(ЦеныНоменклатурыСрезПоследних.Цена) КАК Цена,
			|	МАКСИМУМ(ВЫБОР
			|			КОГДА НоменклатураДополнительныеРеквизиты.Свойство.Заголовок = ""Для аппарата""
			|				ТОГДА НоменклатураДополнительныеРеквизиты.Значение
			...
			|				ТОГДА НоменклатураДополнительныеРеквизиты.Значение
			|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
			|		КОНЕЦ) КАК ДатаВыходаАппарата,
			|	МАКСИМУМ(ВЫБОР
			|			КОГДА НоменклатураДополнительныеРеквизиты.Свойство.Заголовок = ""Для модели устройства""
			|				ТОГДА НоменклатураДополнительныеРеквизиты.Значение
			|			ИНАЧЕ """"
			|		КОНЕЦ) КАК ДляМоделиУстройства,
			|	ЕСТЬNULL(вт_НоменклатураНеПроданнаяРанее.Номенклатура.Производитель, """") КАК Производитель
			|ИЗ
			|	вт_НоменклатураНеПроданнаяРанее КАК вт_НоменклатураНеПроданнаяРанее
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(, ВидЦены = &ВидЦены) КАК ЦеныНоменклатурыСрезПоследних
			|		ПО вт_НоменклатураНеПроданнаяРанее.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
			|			И вт_НоменклатураНеПроданнаяРанее.Характеристика = ЦеныНоменклатурыСрезПоследних.Характеристика
			...
			|						ТОГДА ИСТИНА
			|					ИНАЧЕ Склад = &Склад
			|				КОНЕЦ) КАК СвободныеОстаткиОстатки
			|		ПО вт_НоменклатураНеПроданнаяРанее.Номенклатура = СвободныеОстаткиОстатки.Номенклатура
			|			И вт_НоменклатураНеПроданнаяРанее.Характеристика = СвободныеОстаткиОстатки.Характеристика
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура.ДополнительныеРеквизиты КАК НоменклатураДополнительныеРеквизиты
			|		ПО вт_НоменклатураНеПроданнаяРанее.Номенклатура = НоменклатураДополнительныеРеквизиты.Ссылка
			|		ЛЕВОЕ СОЕДИНЕНИЕ вт_Номенклатура_Топ КАК вт_Номенклатура_Топ
			|		ПО вт_НоменклатураНеПроданнаяРанее.Номенклатура = вт_Номенклатура_Топ.Номенклатура
			|		ЛЕВОЕ СОЕДИНЕНИЕ вт_Характеристика_Новинка КАК вт_Характеристика_Новинка
			|		ПО вт_НоменклатураНеПроданнаяРанее.Характеристика = вт_Характеристика_Новинка.Характеристика
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактеристикиНоменклатуры.ДополнительныеРеквизиты КАК ХарактеристикиНоменклатурыДополнительныеРеквизиты
			|		ПО вт_НоменклатураНеПроданнаяРанее.Номенклатура = ХарактеристикиНоменклатурыДополнительныеРеквизиты.Ссылка.Владелец
			|			И вт_НоменклатураНеПроданнаяРанее.Характеристика = ХарактеристикиНоменклатурыДополнительныеРеквизиты.Ссылка
			|ГДЕ
			...
			|
			|СГРУППИРОВАТЬ ПО
			|	ЕСТЬNULL(вт_НоменклатураНеПроданнаяРанее.Номенклатура.Производитель, """"),
			|	вт_НоменклатураНеПроданнаяРанее.Номенклатура,
			|	вт_НоменклатураНеПроданнаяРанее.Характеристика,
			|	вт_НоменклатураНеПроданнаяРанее.Номенклатура.ЕдиницаИзмерения
			|
			|УПОРЯДОЧИТЬ ПО
			|	ДатаВыходаАппарата УБЫВ";
			
			Запрос.УстановитьПараметр("НоменклатураСегмента" , НоменклатураСегмента);	
			Запрос.УстановитьПараметр("ВидЦены"              , ВидЦены);
			Запрос.УстановитьПараметр("Клиент"               , Клиент);
			Запрос.УстановитьПараметр("Склад"                , Склад);
			Запрос.УстановитьПараметр("ЗаказКлиента"         , Выборка.докЗаказ);
			
			СписокНоменклатуры = Запрос.Выполнить().Выгрузить();
			//СписокНоменклатуры.Загрузить(Запрос.Выполнить().Выгрузить());
			СписокНоменклатурыДляРассылки = Новый Массив;
			Для Каждого Строка Из СписокНоменклатуры Цикл
				//Если Строка.Пометка Тогда
				СписокНоменклатурыДляРассылки.Добавить(Строка.Номенклатура);
				//КонецЕсли;	
			КонецЦикла;	
			СписокЦен = ПолучитьСписокЦен2(Выборка.докЗаказ);
			
			дп_РассылкаПрайсов.СформироватьПрайсы(Клиент,,,,СписокНоменклатурыДляРассылки, СписокЦен,,Истина,Выборка.Почта,,Истина);
			//дп_РассылкаПрайсов.СформироватьПрайсы(Клиент,,,,, СписокЦен,,Истина,Выборка.Почта,,Истина);

			
		Иначе
			дп_РассылкаПрайсов.СформироватьПрайсы(Выборка.Клиент,,,,,,,Истина,Выборка.Почта,,Истина);
		КонецЕсли;
		Набор = РегистрыСведений.ПРФТ_ПрайсыКРассылке.СоздатьНаборЗаписей();
		Набор.Отбор.Клиент.Установить(Выборка.Клиент);
		Набор.Отбор.Пользователь.Установить(Выборка.Пользователь);
		Набор.Отбор.Почта.Установить(Выборка.Почта);
		Набор.Отбор.ДатаРегистрации.Установить(Выборка.ДатаРегистрации);
		Набор.Прочитать();
...
&НаСервере
Функция ПолучитьСписокЦен2(докЗАказ)
	СписокЦен = Новый Массив;
	Если ЗначениеЗаполнено(докЗАказ.Соглашение.ВидЦен) Тогда 
		СписокЦен.Добавить(докЗАказ.Соглашение.ВидЦен);
		Возврат СписокЦен;
	Иначе
		Возврат Неопределено;
	КонецЕсли;	
