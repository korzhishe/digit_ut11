...
Функция РазобратьSharePointАдрес(ПолныйSharePointАдрес) Экспорт
	
	Результат = Новый Структура;
	Результат.Вставить("Сервер", "");
	Результат.Вставить("Каталог", "/");
	
	SharePointАдрес = ПолныйSharePointАдрес;
	
	// Вырезать "https://".
	Поз = СтрНайти(SharePointАдрес, "://");
	Если Поз > 0 Тогда
		SharePointАдрес = Сред(SharePointАдрес, Поз + 3);
	КонецЕсли;
	
	// Каталог.
	Поз = СтрНайти(SharePointАдрес, "/");
	Если Поз > 0 Тогда
		Результат.Каталог = Сред(SharePointАдрес, Поз);
		SharePointАдрес = Лев(SharePointАдрес, Поз - 1);
	КонецЕсли;
	
	Результат.Сервер = SharePointАдрес;
