...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатков(ДокументОбъект,
	...
	//ПРФТ  Временно для проведения отчёта комисионера
	Если НЕ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ОтчетКомиссионера") ТОгда
...
Процедура ТаблицаОстатковСписанныхНаРасходыТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	&Организация КАК Организация,
	|	&Склад КАК Склад,
	|	МАКСИМУМ(Аналитика.КлючАналитики) КАК АналитикаУчетаНоменклатуры,
	...
	|	МАКСИМУМ(ТоварыОрганизаций.Назначение) КАК Назначение,
	|	ТоварыОрганизаций.ДокументРеализации КАК ДокументРеализации,
	|	ТоварыОрганизаций.СтатьяРасходов КАК СтатьяРасходов,
	|	ТоварыОрганизаций.АналитикаРасходов КАК АналитикаРасходов,
	|	ВЫБОР
