...
Процедура ЗарегистрироватьВыгружаемыеДанные(ТаблицаДанных, ИмяКолонки, УзелОбмена)
	...
	Для Каждого ЭлементМассива Из МассивУникальныхЗначений Цикл
		Попытка ПланыОбмена.ЗарегистрироватьИзменения(УзелОбмена, ЭлементМассива); Исключение КонецПопытки;
	КонецЦикла;
...
//	"Марка",Строка(ДанныеНоменклатуры.Номенклатура.Марка));
//роцедура ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, Наименование, Значение)
//	                                     ЗначенияРеквизитовXDTO

Если ЗначениеЗаполнено(ДанныеНоменклатуры.Номенклатура.Марка)	ТОгда
		ИДИзготовитель = УникальныйИдентификаторОбъекта(ДанныеНоменклатуры.Номенклатура.Марка);

		ИзготовительТип = ПакетCML.Получить("Контрагент");
		ИзготовительXDTO = ФабрикаXDTO.Создать(ИзготовительТип);
		ИзготовительXDTO.Ид = ИДИзготовитель;
		ИзготовительXDTO.Наименование = Строка(ДанныеНоменклатуры.Номенклатура.Марка);
		
		ТоварXDTO.Изготовитель = ИзготовительXDTO;

//	
//	ЗначениеРеквизитаXDTO = ФабрикаXDTO.Создать(ПакетCML.Получить("Марка"));
//	
//	ЗначениеРеквизитаXDTO.Наименование = "Производитель";
//	ЗначениеРеквизитаXDTO.Значение.Добавить(СтрокаЗаписиДляXML(Строка(ДанныеНоменклатуры.Номенклатура.Марка)));
//	
...
КонецЕсли;		
	...
	ДобавитьЗначениеРеквизитаXDTO(ЗначенияРеквизитовXDTO, ПакетCML, 
		"ПолноеОписание",Строка(ДанныеНоменклатуры.Номенклатура.ПолноеОписание));


	//ТоварXDTO.Марка = Строка(ДанныеНоменклатуры.Номенклатура.Марка);
	//ТоварXDTO.БазоваяЕдиница = БазоваяЕдиницаXDTO(ТоварXDTO, ДанныеНоменклатуры);
...
Процедура ДобавитьРеквизитыТовараДляПредложенияXDTO(ПредложениеXDTO, СтрокаПредложения, Параметры)
	...
	//Кожемякин 
	//добавление свойства ИзображениеСвойства
	ПредложениеXDTO.ИзображениеСвойства = дп_ДополнительныеПроцедуры.ПолучитьСсылкуДляСайта(СтрокаПредложения.Номенклатура, СтрокаПредложения.Характеристика);
	//Кожемякин
...
Процедура ЗаполнитьДанныеКонтрагента(ДанныеКонтрагента, ДокументXDTO)
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "ИД") Тогда
		ИДКонтрагента = КонтрагентXDTO.ИД;
	КонецЕсли;
	ДанныеКонтрагента.Вставить("ИДКонтрагента", ИДКонтрагента);
	//****************
