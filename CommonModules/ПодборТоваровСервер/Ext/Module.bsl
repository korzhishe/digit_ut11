...
Процедура ОбработатьЭлементСпискаПоиска(ЭлементСписка, РезультатПоиска)
	...
	Иначе
		...
		//07.08.2017 15:04:37
		ИначеЕсли МетаданныеЭлемента = "КодыЦентральногоСправочника" Тогда
			РезультатПоиска.КодыЦентральногоСправочника.Добавить(ЗначениеЭлемента);		
		//}Кожемякин А.Г.	
...
Функция РезультатПоискаНоменклатуры(Форма)
	...
	Для каждого ОбластьПоиска Из ОбластиПоиска Цикл
		...
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//07.08.2017 14:55:49
		Если ОбластьПоиска = Метаданные.РегистрыСведений.КодыЦентральногоСправочника Тогда
			Запрос.УстановитьПараметр("СтрокаПоискаПоКодыЦентральногоСправочника", ИсходнаяСтрокаПоиска(СтрокаПоиска, ТочноеСоответствие));
		КонецЕсли;		
		//}Кожемякин А.Г.
...
Функция ОбластиПоискаНоменклатуры(Форма)
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//07.08.2017 14:32:45
	ОбластиПоиска.Добавить(Метаданные.РегистрыСведений.КодыЦентральногоСправочника);	
	//}Кожемякин А.Г.
...
Функция СтруктураРезультатовПоиска()
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//07.08.2017 14:53:37
	РезультатПоиска.Вставить("КодыЦентральногоСправочника", Новый Массив);	
	//}Кожемякин А.Г.
...
Процедура ДополнитьРезультатыПоискаНоменклатуры(РезультатПоиска)
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//07.08.2017 14:51:53
	Если РезультатПоиска.КодыЦентральногоСправочника.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + ?(ЗначениеЗаполнено(Запрос.Текст), " ОБЪЕДИНИТЬ ВСЕ ", "") + 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	КодыЦентральногоСправочника.КодЦС В(&СписокКодыЦентральногоСправочника)";
		
		Запрос.УстановитьПараметр("СписокКодыЦентральногоСправочника", РезультатПоиска.КодыЦентральногоСправочника);
	КонецЕсли;	
	//}Кожемякин А.Г.
...
Функция ТекстЗапросаПоискаПоОбласти(ОбластьПоиска, ТочноеСоответствие)
	...
	Если ОбластьПоиска = Метаданные.РегистрыСведений.КодыЦентральногоСправочника Тогда
		...
		ТекстЗапроса = "
		|ВЫБРАТЬ ПЕРВЫЕ %МаксКоличество%
		|	КодыЦентральногоСправочника.КодЦС КАК Значение,
		|	""КодыЦентральногоСправочника"" КАК Метаданные
		|ИЗ
		|	РегистрСведений.КодыЦентральногоСправочника КАК КодыЦентральногоСправочника
		|ГДЕ
		|	КодыЦентральногоСправочника.КодЦС = &СтрокаПоискаПоКодыЦентральногоСправочника";
