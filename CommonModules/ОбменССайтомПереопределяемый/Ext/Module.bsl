...
Процедура ПолучитьТекстыЗапросовПакетаПредложений(ТекстыЗапросов) Экспорт
	...
	|	мегапрайсЦеныНоменклатурыПоставщиковСрезПоследних.ХарактеристикаНоменклатуры,
	|	СУММА(мегапрайсЦеныНоменклатурыПоставщиковСрезПоследних.Количество) КАК Количество
	|ПОМЕСТИТЬ ОстаткиПоставщиков
	|ИЗ
	...
	|	ЕСТЬNULL(РезервИзПоступлений.Количество, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0) + ЕСТЬNULL(ОстаткиПоставщиков.Количество, 0) КАК ОстатокНаСкладе
	|{ВЫБРАТЬ
	|	Номенклатура,
	|	Характеристика.*,
	|	Склад.*,
	...
	|			И (РезервИзПоступлений.ДатаДоступности = ДАТАВРЕМЯ(1, 1, 1))
	|			И (РезервИзПоступлений.Количество < 0)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиПоставщиков КАК ОстаткиПоставщиков
	|		ПО (втНоменклатура.Номенклатура = ОстаткиПоставщиков.Номенклатура)
	|			И (ОстаткиПоставщиков.ХарактеристикаНоменклатуры = ЕСТЬNULL(ХарактеристикиНоменклатуры.Ссылка, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)))
	|{ГДЕ
	|	(ЕСТЬNULL(РезервИзПоступлений.Количество, 0) + ЕСТЬNULL(СвободныеОстаткиОстатки.ВНаличииОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток, 0) - ЕСТЬNULL(СвободныеОстаткиОстатки.ВРезервеПодЗаказОстаток, 0)) КАК ОстатокНаСкладе}";
...
Процедура ЗаполнитьЗаказ(ДокументОбъект, РеквизитыЗаказа, СвойстваЗаказа,
	...
	Если ДокументОбъект.Статус = Перечисления.СтатусыЗаказовКлиентов.КОтгрузке Тогда
		Для Каждого Стр из ДокументОбъект.Товары Цикл
			Стр.ДатаОтгрузки = ДокументОбъект.Дата; 
		КонецЦикла;
	КонецЕсли;	
	//Смирнов Д.А. }
	
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//6/27/2017 1:32:45 PM
	//
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Партнер) Тогда
		...
		МассивОбъектов.Добавить(ДокументОбъект.Партнер.Ссылка);
		...
		Для Каждого Адрес Из Адреса Цикл
			ДокументОбъект.АдресДоставки = Адрес.Представление;
			Прервать;
		КонецЦикла;
	КонецЕсли;
	...
	//	КонецЕсли;
	//	СтрокаТовары.ВидЦены = Неопределено;	
	//	ИдентификаторыСтрок.Добавить(Идент);
	//	Идент = Идент + 1;
	//КонецЦикла;	
	//
	//Если Не ЗначениеЗаполнено(СтрокаТовары.ДатаОтгрузки) Тогда
	//	ДокументОбъект.ЖелаемаяДатаОтгрузки = ТекущаяДата();
	//КонецЕсли;
	//
	...
	//
	//Рез = ЗаполнитьОбеспечениеВУстановленномПорядке(ПереченьВариантов, ДокументОбъект, ИдентификаторыСтрок);	
	//}Кожемякин А.Г.
...
//Кожемякин А.Г. agkozhemyakin@gmail.com {
//6/27/2017 6:33:09 PM
//
&НаСервере
Функция ЗаполнитьОбеспечениеВУстановленномПорядке(ПереченьВариантов, Объект ,ИдентификаторыСтрок) Экспорт
	...
	ТекстОповещения = Документы.ЗаказКлиента.ЗаполнитьВариантОбеспечения(Объект, Неопределено, "ИндексыСтрок", Таблица,,);
...
Процедура ИдентифицироватьКонтрагента(СтруктураСвойстваЗаказа, ДанныеКонтрагента, ПрикладныеПараметры, ОписаниеОшибки) Экспорт
	...
	//Смирнов Д.А. }	
		...
		//Кожемякин 210715
		Если ПрикладныеПараметры.СпособИдентификацииКонтрагентаПоИДСайта
			и ЗначениеЗаполнено(ПрикладныеПараметры.РеквизитИдентификацииКонтрагентаПоИДСайта) Тогда
			
			ТекстСообщенияНайденоНесколькоКонтрагентов = " " + НСтр("ru = 'по ИД:'")+ " " + ДанныеКонтрагента.ИДКонтрагента;
			
			Запрос.Текст = Запрос.Текст + 	" ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Партнеры.ДополнительныеРеквизиты КАК КонтрагентыДополнительныеРеквизиты
			|	ПО КонтрагентыДополнительныеРеквизиты.Ссылка = Контрагенты.Партнер
			|ГДЕ
			|	КонтрагентыДополнительныеРеквизиты.Свойство = &ДопРеквизит
			|	И КонтрагентыДополнительныеРеквизиты.Значение = &ИДКонтрагента";
			
			Запрос.УстановитьПараметр("ДопРеквизит", ПрикладныеПараметры.РеквизитИдентификацииКонтрагентаПоИДСайта);
			Запрос.УстановитьПараметр("ИДКонтрагента", ДанныеКонтрагента.ИДКонтрагента);
		//***********************	
		...
		Если РезультатЗапроса.Пустой() Тогда
			...
			КонтрагентСсылка = Справочники.Настройки.НеизвестныйПокупательССайта.Значение;//Справочники.Контрагенты.ПустаяСсылка();
			ОбменССайтом.ДобавитьОписаниеОшибки(ОписаниеОшибки, 
			"Не удалось найти контрагента по Id site ="+ДанныеКонтрагента.ИДКонтрагента+" способ идентификации по ИД=" +ПрикладныеПараметры.СпособИдентификацииКонтрагентаПоИДСайта+" доп. реквизит поиска="+ПрикладныеПараметры.РеквизитИдентификацииКонтрагентаПоИДСайта);
			//****************
		Иначе
...
//сохранена процедура типового варианта
Процедура ИдентифицироватьКонтрагентаТиповаяПроцедура(СтруктураСвойстваЗаказа, ДанныеКонтрагента, ПрикладныеПараметры, ОписаниеОшибки) Экспорт
...
Процедура НайтиСоздатьНоменклатуру(Номенклатура, ДанныеНоменклатуры, ПрикладныеПараметры, ОписаниеОшибки) Экспорт
	...
	Если Не ЗначениеЗаполнено(Номенклатура) Тогда
		...
		Запрос.УстановитьПараметр("Наименование", ДанныеНоменклатуры.НаименованиеНоменклатуры);
...
Процедура ЗаполнитьПрикладныеРеквизитыФормыУзлаОбменСсайтом(МассивРеквизитов) Экспорт
	...
	//Кожемякин 210715
	РеквизитыЗаказов.Добавить("СпособИдентификацииКонтрагентаПоИДСайта");
	РеквизитыЗаказов.Добавить("РеквизитИдентификацииКонтрагентаПоИДСайта");
	//******************
