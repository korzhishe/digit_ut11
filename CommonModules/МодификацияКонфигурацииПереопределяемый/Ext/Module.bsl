...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	//22.02.2018 14:50:26
	//
//	дп_ДополнительныеПроцедуры.ЗаполнитьСтатусДляЗаказа(Форма.Объект);
	//}Кожемякин А.Г.	
КонецПроцедуры
...
Процедура ПослеЗаписиНаСервере(Форма, ТекущийОбъект, ПараметрыЗаписи)Экспорт
	Если ТипЗнч(ТекущийОбъект) = Тип("ДокументОбъект.ПриходныйОрдерНаТовары") Тогда
	
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//10.03.2016 20:31:29
		Если дп_ДополнительныеПроцедуры.СоздаватьЗаказНаПремещение(ТекущийОбъект.Ссылка) 
			и ТекущийОбъект.Статус = Перечисления.СтатусыПриходныхОрдеров.Принят Тогда
			//ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Создаем.");
			дп_ДополнительныеПроцедуры.СоздатьЗаказНаПремещение(ТекущийОбъект.Ссылка); 	
			МенеджерЗаписи 									= РегистрыСведений.дпСоздаватьЗаказыНаПремещения.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.Основание 						= ТекущийОбъект.Ссылка;
			МенеджерЗаписи.флСоздаватьЗаказНаПеремещение 	= Истина;
