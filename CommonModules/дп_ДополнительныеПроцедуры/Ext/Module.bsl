...
Процедура ПолучитьТокенДоступаOneDrive(SSL, ПараметрыПодключения)
	...
	ЗаголовкиHTTPЗапрос = Новый Соответствие;
	ЗаголовкиHTTPЗапрос.Вставить("Content-Type", "application/x-www-form-urlencoded");
	HTTPОтвет = ВыполнитьHTTPЗапрос("https://login.microsoftonline.com/"+ТенантИД+"/oauth2/v2.0/token", ТексЗапроса, ЗаголовкиHTTPЗапрос, SSL);
...
Процедура ОтправитьФайлНаOneDriveREST(ПараметрыПодключения, Файл)
	...
	ПараметрыПодключения.Вставить("флСайт", Ложь);
	ПараметрыПодключения.Вставить("ИДСайта", );
	Если Найти(Логин, "@") = 0 Тогда
		ПараметрыПодключения.Вставить("флСайт", Истина);
		ПолучитьИДСайта(ПараметрыПодключения,Отказ);
		Если Отказ Тогда	
			ЗаписьЖурналаРегистрации("ОтправкаФайлаНаOneDrive",УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,"Не удалось получить ИД Сайта:"+Логин);
			Возврат;
		КонецЕсли;
	КонецЕсли;	
	...
	ИДСайта			= ПараметрыПодключения.ИДСайта;
	...
	РазмерФайла = вуДД.Размер();
	Если РазмерФайла >= 4000000 Тогда
		URLЗапроса = СоздатьСессию(ПараметрыПодключения ,вуФайл.Имя, Отказ);
		ЗаголовкиHTTPЗапрос = Новый Соответствие;
		ЗаголовкиHTTPЗапрос.Вставить("content-length", Формат(РазмерФайла, "ЧГ=0"));
		ЗаголовкиHTTPЗапрос.Вставить("content-range", "bytes 0-"+Формат(РазмерФайла-1, "ЧГ=0")+"/"+Формат(РазмерФайла, "ЧГ=0"));
	Иначе	
		Если ПараметрыПодключения.флСайт Тогда
			URLЗапроса = "https://graph.microsoft.com/v1.0/sites/"+ИДСайта+"/drive/items/"+ИДКаталога+":/"+вуФайл.Имя+":/content";
		Иначе	
...
Функция СоздатьСессию(ПараметрыПодключения ,ИмяФайла, Отказ)
	...
	Если ПараметрыПодключения.флСайт Тогда
		URLЗапроса = "https://graph.microsoft.com/v1.0/sites/"+ПараметрыПодключения.ИДСайта+"/drive/items/"+ПараметрыПодключения.ИДКаталога+":/"+ИмяФайла+":/createUploadSession";
	Иначе	
		URLЗапроса = "https://graph.microsoft.com/v1.0/users/"+ПараметрыПодключения.Логин+"/drive/items/"+ПараметрыПодключения.ИДКаталога+":/"+ИмяФайла+":/createUploadSession";
	КонецЕсли;
	
	ЗаголовкиHTTPЗапрос = Новый Соответствие;
	ЗаголовкиHTTPЗапрос.Вставить("Authorization", "Bearer " + ПараметрыПодключения.Токен);
	...
	Если HTTPОтвет.КодСостояния = 200 Тогда
		...
		uploadUrl 	= ОтветJSON.Получить("uploadUrl");
	Иначе
		ТекстСообщения = "Произошла ошибка обращения к серверу," + "не создана сессия загрузки файла" +
		Символы.ПС + "Статус ответа сервера: " + HTTPОтвет.КодСостояния + 
...
Процедура ПолучитьИДСайта(ПараметрыПодключения,Отказ)
	Токен 				= ПараметрыПодключения.Токен;
	Логин 				= ПараметрыПодключения.Логин;
	...
	URLЗапроса 	= "https://graph.microsoft.com/v1.0/sites/digitstyle.sharepoint.com:/sites/"+Логин;
	...
	Если HTTPОтвет.КодСостояния = 200 Тогда
		...
		ЧтениеJSON.УстановитьСтроку(Ответ);
		ОтветJSON 		= ПрочитатьJSON(ЧтениеJSON, Истина);
		ИДСайта = ОтветJSON.Получить("id");
		ПараметрыПодключения.Вставить("ИДСайта",ИДСайта);
	Иначе
		ТекстСообщения = "Произошла ошибка обращения к серверу," + "не удалось получить идентификатор сайта:" + Логин+
		Символы.ПС + "Статус ответа сервера: " + HTTPОтвет.КодСостояния+ 
		Символы.ПС + HTTPОтвет.ПолучитьТелоКакСтроку();
		ЗаписьЖурналаРегистрации("ОтправкаФайлаНаOneDrive",УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ДополнительныеОтчетыИОбработки,,ТекстСообщения);
		Отказ = Истина;
...
Процедура СоздатьКатлогВOneDriveREST(ПараметрыПодключения, Отказ)
	...
	Если ПараметрыПодключения.флСайт Тогда
		ИДСайта 	= ПараметрыПодключения.ИДСайта;
		URLЗапроса 	= "https://graph.microsoft.com/v1.0/sites/"+ИДСайта+"/drive/root/children";
	Иначе	
	...
	Если ПараметрыПодключения.Свойство("НаименованиеКлиента",НаименованиеКлиента) Тогда
...
Процедура СоздатьВложенныйКаталогВOneDriveREST(ПараметрыПодключения, ИДКаталога, ИмяПапки, Отказ)
	...
	Если ПараметрыПодключения.флСайт Тогда
		ИДСайта 	= ПараметрыПодключения.ИДСайта;
		URLЗапроса = "https://graph.microsoft.com/v1.0/sites/"+ИДСайта+"/drive/items/"+ИДКаталога+"/children";
	Иначе	
