...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	ПараметрыДоставки.Вставить("ИспользоватьSharePointРесурс", Рассылка.ИспользоватьSharePointРесурс);
	ПараметрыДоставки.Вставить("ИспользоватьOneDriveРесурс", Рассылка.ИспользоватьOneDriveРесурс);
	...
	Если Не ПараметрыДоставки.ИспользоватьПапку
		...
		И Не ПараметрыДоставки.ИспользоватьSharePointРесурс
		И Не ПараметрыДоставки.ИспользоватьOneDriveРесурс
	...
	// Подготовка параметров доставки на SharePoint ресурс.
	Если ПараметрыДоставки.ИспользоватьSharePointРесурс Тогда
		ПараметрыДоставки.Вставить("Сервер",              Рассылка.SharePointСервер);
		ПараметрыДоставки.Вставить("Логин",               Рассылка.SharePointЛогин);
		УстановитьПривилегированныйРежим(Истина);
		ПараметрыДоставки.Вставить("Пароль", ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(Рассылка, "SharePointПароль"));
		УстановитьПривилегированныйРежим(Ложь);
		ПараметрыДоставки.Вставить("Каталог",             Рассылка.SharePointКаталог);
		
		ПараметрыДоставки.Вставить("ИДКлиента",            Рассылка.SharePointИДКлиента);
		ПараметрыДоставки.Вставить("СекретКлиента",        Рассылка.SharePointСекретКлиента);
		ПараметрыДоставки.Вставить("ТенантИД",             Рассылка.SharePointТенантИД);
	КонецЕсли;
	
	// Подготовка параметров доставки на OneDrive ресурс.
	Если ПараметрыДоставки.ИспользоватьOneDriveРесурс Тогда
		ПараметрыДоставки.Вставить("Логин",               Рассылка.OneDriveЛогин);
		ПараметрыДоставки.Вставить("Каталог",             Рассылка.OneDriveКаталог);
		
		ПараметрыДоставки.Вставить("ИДКлиента",            Рассылка.OneDriveИДКлиента);
		ПараметрыДоставки.Вставить("СекретКлиента",        Рассылка.OneDriveСекретКлиента);
		ПараметрыДоставки.Вставить("ТенантИД",             Рассылка.OneDriveТенантИД);
	КонецЕсли;
		...
		Иначе
			...
			Если Получатели.Количество() = 0 Тогда
				...
				Если Не ПараметрыДоставки.ИспользоватьПапку
					...
					И Не ПараметрыДоставки.ИспользоватьFTPРесурс
					И Не ПараметрыДоставки.ИспользоватьSharePointРесурс 
					И Не ПараметрыДоставки.ИспользоватьOneDriveРесурс Тогда
...
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	////////////////////////////////////////////////////////////////////////////
	// На SharePoint ресурс.
	
	Если ПараметрыДоставки.ИспользоватьSharePointРесурс Тогда
		
		Цель = "https://"+ ПараметрыДоставки.Сервер + ПараметрыДоставки.Каталог;
		...
		Попытка
			...
			Иначе
				УстановитьПривилегированныйРежим(Истина);
				ДанныеИзХранилища = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(ПараметрыДоставки.Владелец, "SharePointПароль");
				УстановитьПривилегированныйРежим(Ложь);
				Пароль = ?(ЗначениеЗаполнено(ДанныеИзХранилища), ДанныеИзХранилища, "");
			КонецЕсли;
						
			Для Каждого Вложение Из Вложения Цикл
				ПараметрыДоставки.Вставить("ИмяФайла",Вложение.Ключ);
				ПараметрыДоставки.Вставить("Пароль",Пароль);
				дп_ДополнительныеПроцедуры.ОтправитьДанныеНаSharePoint(ПараметрыДоставки, Вложение.Значение);			
			КонецЦикла;
			
			Результат = Истина;
			ПараметрыДоставки.ВыполненаНаSharePoint = Истина;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				ШаблонСообщенияОбОшибке, ИнформацияОбОшибке(),);
		КонецПопытки;
	...
	Если ПараметрыДоставки.ИспользоватьOneDriveРесурс Тогда
		...
		Попытка
			...
			Для Каждого Вложение Из Вложения Цикл
				ПараметрыДоставки.Вставить("ИмяФайла",Вложение.Ключ);
				дп_ДополнительныеПроцедуры.ОтправитьДанныеНаOneDrive(ПараметрыДоставки, Вложение.Значение);			
			КонецЦикла;
			
			Результат = Истина;
			ПараметрыДоставки.ВыполненаНаOneDrive = Истина;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				ШаблонСообщенияОбОшибке, ИнформацияОбОшибке(),);
		КонецПопытки;
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если Не ВременныйФайл.Существует() Тогда
			...
			Продолжить;
		Иначе //edb 2016-01-27
			
			Если ПараметрыОтчета.Отчет.Наименование = СокрЛП("Продажи") 
				И ПараметрыФормата.ТипФайла = ТипФайлаТабличногоДокумента.XLS Тогда
				
			 	ИсправитьФорматДатыВЭекселе(ПолноеИмяФайла);
			КонецЕсли;	
						
		КонецЕсли; //edb 2016-01-27	
...
//edb 2016-01-27
Процедура ИсправитьФорматДатыВЭекселе(ИмяФайлаНаДиске)

		ExcelПриложение 		= Новый COMОбъект("Excel.Application");
		ExcelПриложение.Visible = 0;	
		
		Книга 	= ExcelПриложение.WorkBooks.Open(СокрЛП(ИмяФайлаНаДиске));
		
		Лист = Книга.WorkSheets(1);
		Лист.UnProtect();
		
		НомерКолонки = 1;
		Ячейка     = Лист.Cells(1, НомерКолонки);
		ИмяКолонки = Ячейка.Text;
		
		Пока Не ПустаяСтрока(ИмяКолонки) Цикл
			
			
			Если Найти(ИмяКолонки, "Дата") Тогда
			    Лист.Range(Лист.Cells(2, НомерКолонки), Лист.Cells(9999, НомерКолонки)).NumberFormat = "ДД.ММ.ГГГГ";
				
				Для Сч = 2 По 9999 Цикл
					
					Ячейка = Лист.Cells(Сч,НомерКолонки);					     
					ТекстДата = Ячейка.Text;
					
					Если ПустаяСтрока(ТекстДата) Тогда
						Ячейка.Value = "";
					Иначе	
						Попытка
							Ячейка.Value = Дата(Прав(ТекстДата,4), Сред(ТекстДата,4,2), Лев(ТекстДата, 2));
						Исключение
						КонецПопытки;	
					КонецЕсли;
