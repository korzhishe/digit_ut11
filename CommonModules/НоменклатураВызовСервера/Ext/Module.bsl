...
Процедура ХарактеристикиНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 50
	|	СпрНоменклатура.Характеристика.Ссылка,
	|	СпрНоменклатура.Характеристика.ПометкаУдаления,
	|	СпрНоменклатура.Характеристика.Наименование
	|ИЗ
	|	РегистрСведений.КодыЦентральногоСправочника КАК СпрНоменклатура
	|ГДЕ
	|	СпрНоменклатура.КодЦС ПОДОБНО &СтрокаПоиска
	//|   и СпрНоменклатура.Номенклатура.Ссылка = &Номенклатура
	//}Кожемякин А.Г.
	|УПОРЯДОЧИТЬ ПО
...
Процедура НоменклатураОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 50
	|	СпрНоменклатура.Номенклатура.Ссылка,
	|	СпрНоменклатура.Номенклатура.ПометкаУдаления,
	|	СпрНоменклатура.Номенклатура.КодДляПоиска,
	|	СпрНоменклатура.Номенклатура.Качество.Порядок,
	|	4,
