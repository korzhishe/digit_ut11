...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "ТоварыОтменено" Тогда
		ТоварыОтмененоПриИзменении(Элемент);
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//10/26/2017 2:08:49 PM
	//
	ИначеЕсли Элемент.Имя = "ТоварыдпНомерГТД" Тогда
	//}Кожемякин А.Г.	
	Иначе
...
//04.09.2017 18:31:11
&НаКлиенте
Процедура дпЗаполнитьВыделенныеСтрокиГТД(Команда)
	ОткрытьФорму("Справочник.НомераГТД.ФормаВыбора",,,,,,Новый ОписаниеОповещения("ЗаполнитьГТДВыборЗавершение",ЭтаФорма),РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьГТДВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если ЗначениеЗаполнено(Результат) Тогда
		Для Каждого Строка из Элементы.Товары.ВыделенныеСтроки Цикл
			 ДанныеСтроки = Объект.Товары.НайтиПоИдентификатору(Строка);
			 ДанныеСтроки.дпНомерГТД = Результат;
			 ДанныеСтроки.дпИмпортер = ПолучитьИмпортера(Результат);
		КонецЦикла;	
	КонецЕсли;		
КонецПроцедуры

&НаСервере
Функция ПолучитьИмпортера(Знач Результат)
...
Процедура ТоварыПриИзменении(Элемент)
	...
	Если Элементы.Товары.ТекущийЭлемент.Имя = "ТоварыдпНомерГТД" Тогда
	
		ТекущиеДанные.дпИмпортер = ПолучитьИмпортера(ТекущиеДанные.дпНомерГТД);	
...
Процедура ЗаполнитьНомераГТДПоПоследнимПоступлениямСервер(ЗаполненыНомераГТД)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |ГДЕ
	                      |	ИсходнаяТаблица.дпНомерГТД = ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка)
	                      ...
	                      |ИЗ
	                      |	вт_ТаблицаТовары КАК ИсходнаяТаблица
	                      |ГДЕ
	                      |	ИсходнаяТаблица.Номенклатура.ВестиУчетПоГТД
	                      |;
	                      |
	                      |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("ИсходнаяТаблица", Объект.Товары.Выгрузить(,"НомерСтроки, Номенклатура, дпНомерГТД, ТипНоменклатуры"));
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		СтрокаТаблицы = Объект.Товары.Получить(Выборка.НомерСтроки - 1);
		СтрокаТаблицы.дпНомерГТД = Выборка.НомерГТД;
		СтрокаТаблицы.дпИмпортер = Выборка.НомерГТД.Импортер;
...
&НаКлиенте
Процедура ТоварыдпНомерГТДПриИзменении(Элемент)
	//ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	//ТекущиеДанные.дпИмпортер = ПолучитьИмпортера(ТекущиеДанные.дпНомерГТД);
КонецПроцедуры
