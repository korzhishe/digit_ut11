...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//8/16/2017 3:15:20 PM
	ЗаполнитьСпецУсловияОтгрузки();	
	//}Кожемякин А.Г.
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	//
	дп_ДополнительныеПроцедурыКлиент.ЗаполнитьКодЦС(Объект,"ТоварыОтбор");
	дп_ДополнительныеПроцедурыКлиент.ЗаполнитьКодЦС(Объект,"ТоварыРазмещение");
	//}Кожемякин А.Г.
КонецПроцедуры
...
//Кожемякин А.Г. agkozhemyakin@gmail.com {
//14.08.2017 10:45:19
Процедура ЗаполнитьСпецУсловияОтгрузки()

	ДопРеквизитПокупетеля = Неопределено;
	
	Если ТипЗнч(Объект.Распоряжение) = Тип("ДокументСсылка.РасходныйОрдерНаТовары") Тогда
		ДопРеквизитПокупетеля = дп_ДополнительныеПроцедуры.ПолучитьСпецУсловияОтгрузки(Объект.Распоряжение.Получатель);
	КонецЕсли;

	Если ДопРеквизитПокупетеля = Неопределено Тогда
		Элементы.СпециальныеУсловияОтгрузки.Видимость = Ложь;
	Иначе
		СпециальныеУсловияОтгрузки = ДопРеквизитПокупетеля.ТекстоваяСтрока;
		Элементы.СпециальныеУсловияОтгрузки.Видимость = Истина;
	КонецЕсли;	

КонецПроцедуры

&НаКлиенте
Процедура ТоварыОтборПриИзменении(Элемент)
...
&НаКлиенте
Процедура ЗаполнитьФрагмент(СписокТоваров = Неопределено)
    
    ЗаполнитьСервер(СписокТоваров);
    
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСервер(СписокТоваров = Неопределено)
	...
	Если СписокТоваров <> Неопределено Тогда
		Для Каждого ЭлементОтбора Из ОтборПересчета.Настройки.Отбор.Элементы Цикл
	...
	ИспользоватьАдресноеХранение = ИнициализироватьАдресноеХранение();
	...
	Объект.ТоварыОтбор.Загрузить(ПроцессорВывода.Вывести(ПроцессорКомпоновкиДанных));
	
	Для Каждого Строка из Объект.ТоварыОтбор Цикл
		Строка.ФлагПоОтбору = Истина;
	КонецЦикла;
...
&НаСервере
Функция ИнициализироватьАдресноеХранение()
	
	Перем ИспользоватьАдресноеХранение;
...
Процедура ЗаполнитьЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
    ...
    ЗаполнитьФрагмент(СписокТоваров);
...
//14.08.2017 10:45:19
&НаКлиенте
Процедура РазбитьСтрокуТоварыОтбор(Команда)
	
	ТаблицаФормы  = Элементы.ТоварыОтбор;
	ДанныеТаблицы = Объект.ТоварыОтбор;
	
	Оповещение = Новый ОписаниеОповещения("РазбитьСтрокуТоварыОтборЗавершение", ЭтотОбъект);
	ОбщегоНазначенияУТКлиент.РазбитьСтрокуТЧ(ДанныеТаблицы, ТаблицаФормы, Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура РазбитьСтрокуТоварыОтборЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт 
...
&НаКлиенте
Процедура РазбитьСтрокуТоварыРазмещение(Команда)
	
	ТаблицаФормы  = Элементы.ТоварыРазмещение;
	ДанныеТаблицы = Объект.ТоварыРазмещение;
	
	Оповещение = Новый ОписаниеОповещения("РазбитьСтрокуТоварыРазмещениеЗавершение", ЭтотОбъект);
	ОбщегоНазначенияУТКлиент.РазбитьСтрокуТЧ(ДанныеТаблицы, ТаблицаФормы, Оповещение);
	
КонецПроцедуры

&НаКлиенте
Процедура РазбитьСтрокуТоварыРазмещениеЗавершение(НоваяСтрока, ДополнительныеПараметры) Экспорт 
...
#Область ComplexService

&НаКлиенте
Процедура ЗаполнитьЯчейкуВВыделенныхСтрокахРазмещение(Команда)

	// CS_Начало, #69, Алексеев С.В., 15.11.2017
	// ---
	// +++
	ПараметрыФормы = Новый Структура("Склад,Помещение", Объект.Склад, Объект.Помещение);
	...
	ОткрытьФорму("Документ.ОтборРазмещениеТоваров.Форма.ФормаУказанияЯчейки",ПараметрыФормы,ЭтаФорма,,,, Новый ОписаниеОповещения("ЗаполнитьЯчейкуВВыделенныхСтрокахРазмещениеЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	// CS_Конец
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьЯчейкуВВыделенныхСтрокахРазмещениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
    ...
    Если ЗначениеЗаполнено(Ячейка) Тогда
        ЗаполнитьЯчейкуВВыделенныхСтрокахТЧ("ТоварыРазмещение", Ячейка);	
	КонецЕсли;
	// CS_Конец
...
&НаСервере
Процедура ЗаполнитьЯчейкуВВыделенныхСтрокахТЧ(ИмяТЧ, Ячейка)
	...
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		НайденнаяСтрока        = Объект[ИмяТЧ].НайтиПоИдентификатору(ВыделеннаяСтрока);
