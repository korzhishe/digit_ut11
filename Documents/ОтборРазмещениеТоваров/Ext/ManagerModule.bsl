...
Функция ПечатьЗадания(МассивОбъектов, ОбъектыПечати)
	...
	//25.08.2017 16:19:57
	|	ОтборРазмещениеТоваровТоварыОтбор.Номенклатура КАК НоменклатураСсылка,
	|	ОтборРазмещениеТоваровТоварыОтбор.Характеристика КАК ХарактеристикаСсылка,
	//}Кожемякин А.Г.
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//25.08.2017 16:19:57
	|	ОтборРазмещениеТоваровТоварыРазмещение.Номенклатура КАК НоменклатураСсылка,
	|	ОтборРазмещениеТоваровТоварыРазмещение.Характеристика КАК ХарактеристикаСсылка,
	//}Кожемякин А.Г.
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Если ВыводитьКоды Тогда
				...
				//Кожемякин А.Г. agkozhemyakin@gmail.com {
				//25.08.2017 16:15:44
				ОбластьСтрокаТаблицыКолонкаКодов.Параметры.КодЦС = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(ВыборкаПоСтрокамТЧ.НоменклатураСсылка, ВыборкаПоСтрокамТЧ.ХарактеристикаСсылка);				
				//}Кожемякин А.Г.
...
Функция ДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	|	ОтборРазмещениеТоваровТоварыОтбор.НомерСтроки КАК Порядок,
	...
	|	ОтборРазмещениеТоваровТоварыРазмещение.НомерСтроки КАК Порядок,
	|	ОтборРазмещениеТоваровТоварыРазмещение.Ссылка.ВидОперации КАК ВидОперации
	...
	|	МИНИМУМ(ТоварыОрганизацийОстатки.НомерГТД) КАК НомерГТД,
	...
	|						ИЗ
	|							ТоварыОтбор КАК ТоварыОтбор)
	|					ИЛИ (АналитикаУчетаНоменклатуры.Номенклатура, АналитикаУчетаНоменклатуры.Характеристика) В
	|						(ВЫБРАТЬ
	|							ТоварыОтбор.Номенклатура,
	|							ТоварыОтбор.Характеристика
	|						ИЗ
	|							ТоварыРазмещение КАК ТоварыОтбор))) КАК ТоварыОрганизацийОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПриобретениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
	|		ПО ТоварыОрганизацийОстатки.НомерГТД = ПоступлениеТоваровУслугТовары.НомерГТД
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	МАКСИМУМ(ОстаткиГТД.ДатаПоступления) КАК ДатаПоступления
	|ПОМЕСТИТЬ МаксДатаПоступленияНоменклатуры
	|ИЗ
	...
	|	ВремТабл.Порядок КАК Порядок,
	|	ВремТабл.Упаковка КАК Упаковка,
	|	ВремТабл.КоличествоУпаковок КАК КоличествоУпаковок,
	|	ВремТабл.Количество КАК Количество,
	|	ОстаткиГТД.НомерГТД.дп_ДатаПроизводства КАК НомерГТДДатаПроизводства,
	|	ОстаткиГТД.НомерГТД.Импортер КАК НомерГТДИмпортер
	|ИЗ
	|	ВремТабл КАК ВремТабл
	|		ЛЕВОЕ СОЕДИНЕНИЕ МаксДатаПоступленияНоменклатуры КАК МаксДатаПоступленияНоменклатуры
	|			ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиГТД КАК ОстаткиГТД
	|			ПО МаксДатаПоступленияНоменклатуры.Номенклатура = ОстаткиГТД.Номенклатура
	|				И МаксДатаПоступленияНоменклатуры.Характеристика = ОстаткиГТД.Характеристика
	|				И МаксДатаПоступленияНоменклатуры.ДатаПоступления = ОстаткиГТД.ДатаПоступления
	|		ПО ВремТабл.Номенклатура = МаксДатаПоступленияНоменклатуры.Номенклатура
	|			И ВремТабл.Характеристика = МаксДатаПоступленияНоменклатуры.Характеристика
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок";
	...
	//ПРФТ++
	СтруктураДействий.Вставить("УстановитьПартнера",МассивДокументов[0].Распоряжение.Получатель);
	//ПРФТ--
