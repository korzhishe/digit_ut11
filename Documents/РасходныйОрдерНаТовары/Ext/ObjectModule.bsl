...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//05.06.2017 16:52:10
	//заполним комментарий документа из распоряжения Заказ клиента
	дп_ДополнительныеПроцедуры.ЗаполнитьКомментарийОрдераИзРаспоряжений(ЭтотОбъект);
	//}Кожемякин А.Г.
КонецПроцедуры
...
Процедура ПроверитьСоответствиеСтрокТабличныхЧастей(Отказ)
	...
	//ПРФТ Временно ++
	Если ПараметрыСеанса.ТекущийПользователь.Наименование = "Покровский Сергей"  ТОгда
		Сообщить("Отработала временная заглушка!!!!");
		Возврат;
	КонецЕсли;
