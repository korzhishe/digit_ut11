...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокРаспоряженияНаОформление.ТекстЗапроса = Документы.ПриобретениеТоваровУслугТовары.ТекстЗапросаЗаказыКОформлению();
	СписокРаспоряженияНаПриемку.ТекстЗапроса = Документы.ПриобретениеТоваровУслугТовары.ТекстЗапросаРаспоряженияНаПриемку();
	...
	ПравоДобавлятьПоступление = ПравоДоступа("Добавление", Метаданные.Документы.ПриобретениеТоваровУслугТовары);
...
Процедура ОформитьАктОРасхождениях(Команда)
	...
	Если ТипЗнч(ТекущиеДанные.Ссылка) <> Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда
...
Процедура ОформитьПоПриемке(Команда)
	...
	Если ТипЗнч(ТекущиеДанные.Ссылка) = Тип("СправочникСсылка.СоглашенияСПоставщиками")
		...
		Иначе
			...
			ОткрытьФорму("Документ.ПриобретениеТоваровУслугТовары.ФормаОбъекта", ПараметрыЗаполнения, ЭтаФорма);
		КонецЕсли;
...
Функция СсылкаНаЭлементСпискаПоШтрихкоду(Штрихкод)
	...
	Менеджеры.Добавить(ПредопределенноеЗначение("Документ.ПриобретениеТоваровУслугТовары.ПустаяСсылка"));
...
Функция ОпределитьНаличиеОформленныхНакладных(РаспоряжениеСсылка, Склад)
	...
	|ГДЕ
	|	ЗаказыПоставщикамОстаткиИОбороты.Регистратор ССЫЛКА Документ.ПриобретениеТоваровУслугТовары
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	Документ.ПриобретениеТоваровУслугТовары.Товары КАК ДокументПоступленияТовары
	|	
	|ГДЕ
...
Функция НастройкиФормыПереоформленияНакладных(РаспоряжениеСсылка, Склад)
	...
	НастройкиФормы.ИмяФормыНакладной = "Документ.ПриобретениеТоваровУслугТовары.Форма.ФормаДокумента";
...
Процедура УстановитьТекущуюСтраницу()
	...
	Если Параметры.Свойство("ИмяТекущейСтраницы", ИмяТекущейСтраницы) Тогда
		...
		КонецЕсли;
	Иначе
		Если Не Документы.ПриобретениеТоваровУслугТовары.ЕстьЗаказыКОформлению(Организация,Склад)
			И Документы.ПриобретениеТоваровУслугТовары.ЕстьРаспоряженияНаПриемку(Организация,Склад) Тогда
