...
//Кожемякин А.Г. agkozhemyakin@gmail.com {
//10.04.2017 17:54:37
//Заполняем ГТД в табоичной части по номеру ГТД укзанному в документе заказ поставщику
&НаКлиенте
Процедура дпЗаполнитьНомераГТДИзЗаказаПоставщику(Команда)
	флГТДЕсть = Ложь;
	
	Если ЗначениеЗаполнено(Объект.ЗаказПоставщику) Тогда
		Если ЗначениеЗаполнено(Объект.ЗаказПоставщику.дпНомерГТД) Тогда
			НомерГТД = Объект.ЗаказПоставщику.дпНомерГТД;
			ЗаполненыНомераГТД = Ложь;
			ПоказатьВопрос(Новый ОписаниеОповещения("дпЗаполнитьНомераГТДИзЗаказаПоставщикуЗавершение", ЭтотОбъект, Новый Структура("ЗаполненыНомераГТД, НомерГТД", ЗаполненыНомераГТД, НомерГТД)), "Заполнить номера ГТД из заказа поставщику?",РежимДиалогаВопрос.ДаНет);
            Возврат;
						
		КонецЕсли;
	КонецЕсли; 
	
	дпЗаполнитьНомераГТДИзЗаказаПоставщикуФрагмент(флГТДЕсть);
КонецПроцедуры

&НаКлиенте
Процедура дпЗаполнитьНомераГТДИзЗаказаПоставщикуЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	ЗаполненыНомераГТД = ДополнительныеПараметры.ЗаполненыНомераГТД;
	НомерГТД = ДополнительныеПараметры.НомерГТД;
...
&НаКлиенте
Процедура дпЗаполнитьНомераГТДИзЗаказаПоставщикуФрагмент(Знач флГТДЕсть)
	
	Если Не флГТДЕсть Тогда
		
		ПоказатьПредупреждение(Неопределено,"В заказе поставщику не указан номер ГТД для заполнения",,);	
...
Процедура ТоварыНомерГТДПриИзменении(Элемент)
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	ТекущиеДанные.дпИмпортер = дп_ДополнительныеПроцедуры.ПолучитьИмпортера(ТекущиеДанные.НомерГТД);
КонецПроцедуры
//}Кожемякин А.Г.
