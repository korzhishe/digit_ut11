...
Функция ДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	//05.04.2018 15:32:18
	//
	|	ПриобретениеТоваровУслугТовары.НомерГТД.Импортер,
	|	ПриобретениеТоваровУслугТовары.НомерГТД.дп_ДатаПроизводства КАК НомерГТДДатаПроизводства
	//}Кожемякин А.Г.
	|ИЗ
	|	Документ.ПриобретениеТоваровУслуг.Товары КАК ПриобретениеТоваровУслугТовары
	|	
	...
	//
	|	ПриобретениеТоваровУслугТовары.НомерГТД.Импортер,
	|	ПриобретениеТоваровУслугТовары.НомерГТД.дп_ДатаПроизводства
	//}Кожемякин А.Г.
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок
