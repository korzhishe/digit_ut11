...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//02.11.2017 14:37:40
	//
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
		ЗаполнитьДокументНаОснованииПриходногоОрдера(ДанныеЗаполнения);
	//}Кожемякин А.Г.	
	КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПоставщику(Знач ДокументОснование, Знач СкладПоступления = Неопределено, РеквизитыЗаказа = Неопределено)
	...
	Если ТипОснования = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		ЗаполнитьГТДПоЗаказуПоставщика(ДокументОснование);
	ИначеЕсли ТипОснования = Тип("Массив") Тогда
		Если ТипЗнч(ДокументОснование[0]) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
			ЗаполнитьГТДПоЗаказуПоставщика(ДокументОснование[0]);
		КонецЕсли;	
	КонецЕсли;
КонецПроцедуры
...
//05.04.2018 15:35:36
//
Процедура ЗаполнитьГТДПоЗаказуПоставщика(Знач ДокументОснование)
	
	Перем ЗаполненыНомераГТД, МассивВедетсяУчетПоГТД, НайденнаяСтрока, НомерГТД, СтрокаТаблицы;
	
	//Если ЗначениеЗаполнено(ДокументОснование.дпНомерГТД) Тогда
	ЗаполненыНомераГТД = Ложь;
	//ЗапасыСервер.ЗаполнитьНомераГТДвТабличнойЧасти(
	//Товары,
	//ДокументОснование.дпНомерГТД,
	//Товары,
	//ЗаполненыНомераГТД);
	
	НомерГТД = ДокументОснование.дпНомерГТД;
	...
	Для Каждого СтрокаТаблицы Из Товары Цикл
		Если МассивВедетсяУчетПоГТД.Найти(СтрокаТаблицы.Номенклатура) <> Неопределено Тогда
			НайденнаяСтрока = ДокументОснование.Товары.Найти(СтрокаТаблицы.Номенклатура,"Номенклатура");
			Если НайденнаяСтрока <> Неопределено Тогда
				СтрокаТаблицы.НомерГТД = НайденнаяСтрока.дпНомерГТД;
				СтрокаТаблицы.дпИмпортер = НайденнаяСтрока.дпИмпортер;
...
//02.11.2017 14:38:24
//
Процедура ЗаполнитьДокументНаОснованииПриходногоОрдера(Знач ДокументОснование)

	ЗаполнитьПоОрдеру(ДокументОснование);
	
	//МассивЗаказов = Новый Массив();
	//МассивЗаказов.Добавить(ДокументОснование.Распоряжение);
	//
	//РеквизитыЗаказа = Неопределено;
	//
	//ЗапонитьШапкуПоЗаказу(ДокументОснование.Распоряжение, РеквизитыЗаказа);
	//
	//СкладПоступления = Склад;
	//
	...
	ЗаполнитьГТДПоЗаказуПоставщика(ДокументОснование.Распоряжение);
	
	ОчиститьСтрокиПоПриходномуОрдеру(ДокументОснование);
	
	ЗаказыСервер.ЗаполнитьЗаказВШапкеПоЗаказамВТабличнойЧасти(ЗаказПоставщику, Товары, "ЗаказПоставщику");
КонецПроцедуры

Процедура ОчиститьСтрокиПоПриходномуОрдеру(Знач ТекОрдер)
	СтрокиНаУдаление = Новый Массив;
	ТаблицаТоварыОрдера = ТекОрдер.Товары.Выгрузить();
	//ТаблицаТоварыОрдера.Свернуть("Номенлкатура", "Количество, КоличествоУпаковок");
	
	Для Каждого СтрокаТоваров из Товары Цикл
		НайденныеСтроки = ТаблицаТоварыОрдера.НайтиСтроки(Новый Структура("Номенклатура, Характеристика", СтрокаТоваров.Номенклатура, СтрокаТоваров.Характеристика));
		Если НайденныеСтроки.Количество() Тогда
			Для Каждого СтрокаОрдера из НайденныеСтроки Цикл
				Если СтрокаОрдера.Количество = 0 Тогда
					ТаблицаТоварыОрдера.Удалить(СтрокаОрдера);
				КонецЕсли;
				
				Если СтрокаОрдера.Количество = СтрокаТоваров.Количество Тогда
					СтрокаОрдера.Количество = 0;
					ТаблицаТоварыОрдера.Удалить(СтрокаОрдера);
				ИначеЕсли СтрокаОрдера.Количество > СтрокаТоваров.Количество Тогда
					СтрокаОрдера.Количество = СтрокаОрдера.Количество - СтрокаТоваров.Количество;
				ИначеЕсли СтрокаОрдера.Количество < СтрокаТоваров.Количество и СтрокаОрдера.Количество > 0 Тогда
					СтрокаТоваров.Количество = СтрокаТоваров.Количество - СтрокаОрдера.Количество;
					Ценообразование.ПересчитатьСуммыВСтроке(СтрокаТоваров, Ложь, Ложь, Истина, ЦенаВключаетНДС);
					СтрокаОрдера.Количество = 0;
					ТаблицаТоварыОрдера.Удалить(СтрокаОрдера);
				Иначе
					СтрокиНаУдаление.Добавить(СтрокаТоваров);
				КонецЕсли;	
			КонецЦикла;	
		Иначе
			СтрокиНаУдаление.Добавить(СтрокаТоваров);
...
Процедура ЗаполнитьПоОрдеру(Знач ДокументОснование)
	
	Перем Выборка, Запрос, РезультатЗапросаПакет, СтрокаТовары;
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДокументОснования", ДокументОснование);
	Запрос.УстановитьПараметр("Распоряжение", ДокументОснование.Распоряжение);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВозвратТоваровПоставщику.Распоряжение.Ссылка КАК Ссылка,
	|	ВозвратТоваровПоставщику.Распоряжение.Ссылка КАК ЗаказПоставщику,
	|	ВозвратТоваровПоставщику.Распоряжение.Организация КАК Организация,
	|	ВозвратТоваровПоставщику.Распоряжение.Партнер КАК Партнер,
	|	ВозвратТоваровПоставщику.Распоряжение.Контрагент КАК Контрагент,
	|	ВозвратТоваровПоставщику.Распоряжение.Подразделение КАК Подразделение,
	|	ВозвратТоваровПоставщику.Распоряжение.Валюта КАК Валюта,
	|	ВозвратТоваровПоставщику.Распоряжение.Валюта КАК ВалютаВзаиморасчетов,
	|	ВозвратТоваровПоставщику.Распоряжение.ФормаОплаты КАК ФормаОплаты,
	|	ВозвратТоваровПоставщику.Распоряжение.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ВозвратТоваровПоставщику.Распоряжение.НалогообложениеНДС КАК НалогообложениеНДС,
	|	ВозвратТоваровПоставщику.Распоряжение.ЗакупкаПодДеятельность КАК ЗакупкаПодДеятельность,
	|	ВозвратТоваровПоставщику.Распоряжение.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ВозвратТоваровПоставщику.Распоряжение.Статус КАК СтатусДокумента,
	|	ВозвратТоваровПоставщику.Распоряжение.ГруппаФинансовогоУчета КАК ГруппаФинансовогоУчета,
	|	ВозвратТоваровПоставщику.Распоряжение.ВернутьМногооборотнуюТару КАК ВернутьМногооборотнуюТару,
	|	ВозвратТоваровПоставщику.Распоряжение.СрокВозвратаМногооборотнойТары КАК СрокВозвратаМногооборотнойТары,
	|	ВозвратТоваровПоставщику.Распоряжение.ТребуетсяЗалогЗаТару КАК ТребуетсяЗалогЗаТару,
	|	ЕСТЬNULL(ВозвратТоваровПоставщику.Распоряжение.Соглашение.РассчитыватьДатуВозвратаТарыПоКалендарю, ЛОЖЬ) КАК РассчитыватьДатуВозвратаТарыПоКалендарю,
	|	ЕСТЬNULL(ВозвратТоваровПоставщику.Распоряжение.Соглашение.КалендарьВозвратаТары, ЗНАЧЕНИЕ(Справочник.ПроизводственныеКалендари.ПустаяСсылка)) КАК КалендарьВозвратаТары,
	|	НЕ ВозвратТоваровПоставщику.Распоряжение.Проведен КАК ЕстьОшибкиПроведен,
	|	ВЫБОР
	|		КОГДА ВозвратТоваровПоставщику.Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден)
	|				ИЛИ ВозвратТоваровПоставщику.Распоряжение.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Закрыт)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ЕстьОшибкиСтатус,
	|	ВозвратТоваровПоставщику.Распоряжение.БанковскийСчет КАК БанковскийСчетОрганизации,
	|	ВозвратТоваровПоставщику.Распоряжение.ПорядокРасчетов КАК ПорядокРасчетов,
	|	ВозвратТоваровПоставщику.Распоряжение.НаправлениеДеятельности КАК НаправлениеДеятельности
	|ИЗ
	|	Документ.ПриходныйОрдерНаТовары КАК ВозвратТоваровПоставщику
	|ГДЕ
	|	ВозвратТоваровПоставщику.Ссылка = &ДокументОснования
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЗаказПоставщикуТовары.дпНомерГТД КАК НомерГТД
	|ПОМЕСТИТЬ вт_ТоварыЗаказа
	|ИЗ
	|	Документ.ЗаказПоставщику.Товары КАК ЗаказПоставщикуТовары
	|ГДЕ
	|	ЗаказПоставщикуТовары.Ссылка = &Распоряжение
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЗаказПоставщикуТовары.дпНомерГТД
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	вт_ТоварыЗаказа.НоменклатураПоставщика КАК НоменклатураПоставщика,
	|	вт_ТоварыЗаказа.Номенклатура КАК Номенклатура,
	|	вт_ТоварыЗаказа.Характеристика КАК Характеристика,
	|	вт_ТоварыЗаказа.Упаковка КАК Упаковка,
	|	вт_ТоварыЗаказа.ДатаПоступления КАК ДатаПоступления,
	|	вт_ТоварыЗаказа.ВидЦеныПоставщика КАК ВидЦеныПоставщика,
	|	вт_ТоварыЗаказа.Цена КАК Цена,
	|	вт_ТоварыЗаказа.Сумма КАК Сумма,
	|	вт_ТоварыЗаказа.СтавкаНДС КАК СтавкаНДС,
	|	вт_ТоварыЗаказа.СуммаНДС КАК СуммаНДС,
	|	вт_ТоварыЗаказа.СуммаСНДС КАК СуммаСНДС,
	...
	|	вт_ТоварыЗаказа.КодСтроки КАК КодСтроки,
	|	вт_ТоварыЗаказа.НомерГТД КАК НомерГТД,
	|	&Распоряжение КАК ЗаказПоставщику
	|ИЗ
	...
	|		ПО вт_ТоварыЗаказа.Номенклатура = ПриходныйОрдерНаТоварыТовары.Номенклатура
	|			И вт_ТоварыЗаказа.Характеристика = ПриходныйОрдерНаТоварыТовары.Характеристика
	|ГДЕ
	|	ПриходныйОрдерНаТоварыТовары.Ссылка = &ДокументОснования";
	
	РезультатЗапросаПакет = Запрос.ВыполнитьПакет();
	
	Выборка = РезультатЗапросаПакет[0].Выбрать();
	Выборка.Следующий();
	...
	// Заполнение шапки и табличных частей
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
	Товары.Загрузить(РезультатЗапросаПакет[2].Выгрузить());
	Для Каждого СтрокаТовары из Товары Цикл
		Ценообразование.ПересчитатьСуммыВСтроке(СтрокаТовары, Ложь, Ложь, Истина, Выборка.ЦенаВключаетНДС);
	КонецЦикла;
