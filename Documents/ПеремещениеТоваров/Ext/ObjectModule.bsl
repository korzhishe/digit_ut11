...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		КонецЕсли;
	// CS_Начало, #55, Алексеев С.В., 07.112017
	// ---
	// +++
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
		
		РаспоряжениеСвязка = ДанныеЗаполнения;
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//2/28/2018 4:01:27 PM
		//
		ВыполнитьСортировкуСтрокПоОснованию(РаспоряжениеСвязка, "Товары", "Товары");
		//}Кожемякин А.Г.
	// CS_Конец	
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ПРФТ++ 
	Если ВыполнитьПеремещениеБезОрдеровИОтборов Тогда
		ТЗВременно = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТоварыНаСкладах.Скопировать();
		Для Каждого ТекСтрока Из ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТоварыНаСкладах Цикл
			НоваяСтрока = ТЗВременно.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока,ТекСтрока);
			НоваяСтрока.ВидДвижения = ВидДвиженияНакопления.Расход;
			НоваяСтрока.Склад = СкладОтправитель;
		КонецЦикла;
		ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТоварыНаСкладах.Очистить();
		Для Каждого ТекСтрока ИЗ ТЗВременно Цикл
			НоваяСтрока = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТоварыНаСкладах.ДОбавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
		КонецЦикла;
		//ДополнительныеСвойства.ТаблицыДляДвижений.ОтразитьТоварыКОтгрузке.Очистить();
	КонецЕсли;
	ЗапасыСервер.ОтразитьТоварыНаСкладах(ДополнительныеСвойства, Движения, Отказ);
	Если НЕ ВыполнитьПеремещениеБезОрдеровИОтборов Тогда
...
Процедура ВыполнитьСортировкуСтрокПоОснованиюПриНеобходимости()
	
	// CS_Начало, #67, Алексеев С.В., 15.11.2017
	// ---
	// +++
	СортировкаНеобходима = Ложь;
	...
	Если ЗначениеЗаполнено(РаспоряжениеСвязка) Тогда
		СортировкаНеобходима = Истина;
		
		ДокументОснование = РаспоряжениеСвязка;
		
		ИмяТЧОснования = "Товары";
		
		ИмяТЧПриемника = "Товары";
	КонецЕсли;
	
	Если СортировкаНеобходима Тогда
		ВыполнитьСортировкуСтрокПоОснованию(ДокументОснование, ИмяТЧОснования, ИмяТЧПриемника);
	КонецЕсли;
	// CS_Конец
	
КонецПроцедуры

Процедура ВыполнитьСортировкуСтрокПоОснованию(ДокументОснование, ИмяТЧОснования, ИмяТЧПриемника)
	
	// CS_Начало, #67, Алексеев С.В., 15.11.2017
	// ---
	// +++
	ТЗОснования = ДокументОснование[ИмяТЧОснования].Выгрузить(, "Номенклатура, Характеристика");
	
	ТЗОбработанных = ТЗОснования.СкопироватьКолонки();
	
	ТЗИсходная = ЭтотОбъект[ИмяТЧПриемника].Выгрузить();
	
	ТЗИтоговая = ТЗИсходная.СкопироватьКолонки();
	...
	Для Каждого СтрокаОснования Из ТЗОснования Цикл
		...
		НайденныеСтроки = ТЗОбработанных.НайтиСтроки(ПараметрыОтбора);
		
		Если НайденныеСтроки.Количество() <> 0 Тогда
			Продолжить;
		КонецЕсли;
		
		НайденныеСтроки = ТЗИсходная.НайтиСтроки(ПараметрыОтбора);
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			...
			НоваяСтрока = ТЗОбработанных.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ПараметрыОтбора);
		КонецЦикла;
	КонецЦикла;
	
	ЭтотОбъект[ИмяТЧПриемника].Очистить();
	ЭтотОбъект[ИмяТЧПриемника].Загрузить(ТЗИтоговая);
	// CS_Конец
