		...
		Для Каждого Шапка Из ИспользованиеСерий.Строки Цикл
			...
			Пока ВыборкаПоТоварам.НайтиСледующий(Новый Структура("Ссылка",Шапка.Ссылка)) Цикл
				...
				Если ВыводитьКоды Тогда
					...
					//28.08.2017 16:48:36
					ОбластьКодовСтрока.Параметры.КодЦС = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(ВыборкаПоТоварам.Товар, ВыборкаПоТоварам.Характеристика);					
					//}Кожемякин А.Г.
...
Функция СформироватьПечатнуюФормуТОРГ13(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			//Кожемякин А.Г. agkozhemyakin@gmail.com {
			//28.08.2017 16:52:41
			ОбластьМакетаСтрока.Параметры.КодЦС = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(ВыборкаСтрокТовары.Товар, ВыборкаСтрокТовары.ХарактеристикаСсылка);			
			//}Кожемякин А.Г.
