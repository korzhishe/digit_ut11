...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	//Если ПравоДоступа("Добавление", Метаданные.Документы.УдалитьАктОРасхожденияхПриПоступленииТоваров) Тогда
	//	КомандаСоздатьНаОсновании = КомандыСоздатьНаОсновании.Добавить();
	//	КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.УдалитьАктОРасхожденияхПриПоступленииТоваров.ПолноеИмя();
	...
	//	КомандаСоздатьНаОсновании.ФункциональныеОпции = "УдалитьИспользоватьАктыОРасхожденияхПриПоступленииТоваров";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.УдалитьАктОРасхожденияхПриПоступленииТоваров)
...
Процедура ЗаполнитьТабличныйДокументТОРГ(ИмяМакета, ПечатьВНоменклатуреПоставщика, ТабличныйДокумент, Запрос, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.УдалитьАктОРасхожденияхПриПоступленииТоваров.ПФ_MXL_" + ИмяМакета);
...
Процедура СформироватьТекстЗапросаПоДокументам(ИмяМакета, ТекстЗапроса)
	...
	|	СчетФактураПолученныйДокументыОснования.Ссылка.ДатаСоставления КАК ДатаСчетаФактуры
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА НЕ АктОРасхожденияхПриПоступленииТоваров.ДокументОснование.УдалитьЕстьРасхождения
	|				ИЛИ АктОРасхожденияхПриПоступленииТоваров.ДокументОснование.УдалитьСуммаИзлишков = 0
	|					И АктОРасхожденияхПриПоступленииТоваров.ДокументОснование.УдалитьСуммаНедостачи = 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	КОНЕЦ КАК Упаковка,
	|	&ТекстЗапросаЕдиницаИзмерения1 КАК ЕдиницаИзмерения,
	...
	|	КОНЕЦ КАК Упаковка,
	|	&ТекстЗапросаЕдиницаИзмерения2 КАК ЕдиницаИзмерения,
	...
	|ИЗ
	|	Документ.УдалитьАктОРасхожденияхПриПоступленииТоваров КАК АктОРасхожденияхПриПоступленииТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.УдалитьТоварыПоДаннымПоставщика КАК ПоступлениеТоваровУслугТоварыПоДаннымПоставщика
	...
	|	ИЗ
	|		Документ.УдалитьАктОРасхожденияхПриПоступленииТоваров КАК ДанныеДокумента
...
Функция ДоступныеДляШаблоновПечатныеФормы() Экспорт
	...
	МассивДоступныхПечатныхФорм.Добавить(ШаблоныСообщенийСервер.СтруктураПараметровДоступнойПечатнойФормы( 
	                                     "ТОРГ2", НСтр("ru = 'ТОРГ-2'"), "Документ.УдалитьАктОРасхожденияхПриПоступленииТоваров", Неопределено));
	МассивДоступныхПечатныхФорм.Добавить(ШаблоныСообщенийСервер.СтруктураПараметровДоступнойПечатнойФормы( 
	                                     "ТОРГ3", НСтр("ru = 'ТОРГ-3'"), "Документ.УдалитьАктОРасхожденияхПриПоступленииТоваров", Неопределено));
