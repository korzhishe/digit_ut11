...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
		...
		ЗаполнитьДокументНаОснованииОрдера(ДанныеЗаполнения);
		ЗаполненНаОснованииДокумента = Истина;
	//ПРФТ--
	КонецЕсли;
...
//ПРФТ++
Процедура ЗаполнитьДокументНаОснованииОрдера(Знач ДокументОснование)
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//13.01.2017 15:25:54
	ОтразитьВыручкуИСебестоимостьПродажКлиента(ДополнительныеСвойства, Движения, Отказ);	
	//}Кожемякин А.Г.
	...
	// CS_Начало, #43, Алексеев С.В., 31.10.2017
	// ---
	// +++
	ДозаполнитьПустыеГТД();
	// CS_Конец
...
Процедура ЗаполнитьДокументНаОснованииЗаявкиНаВозвратТоваровОтКлиента(Знач ДокументОснование)
	...
	//ПРФТ++
	Если ЗначениеЗаполнено(ДокументОснование.ДатаДляДокументаВозврата) ТОгда
		Дата = ДокументОснование.ДатаДляДокументаВозврата;
	КонецЕсли;
	//ПРФТ--
...
//
Процедура ОтразитьВыручкуИСебестоимостьПродажКлиента(ДополнительныеСвойства, Движения, Отказ) Экспорт
	Если Релиз Тогда
		ТаблицаДвижений = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаВыручкаИСебестоимостьПродажКлиента;
		
		Если Отказ ИЛИ ТаблицаДвижений.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
		ДвиженияРасчеты = Движения.ВыручкаИСебестоимостьПродажКлиента;
		ДвиженияРасчеты.Записывать = Истина;
		ДвиженияРасчеты.Загрузить(ТаблицаДвижений);
	КонецЕсли;	
КонецПроцедуры // ОтразитьПрочиеРасходы()

Процедура ДозаполнитьПустыеГТД()
	...
	|	МАКСИМУМ(Поступления.ДатаПоступления) КАК ДатаПоступления
	|ПОМЕСТИТЬ втСводная
	|ИЗ
	|	втТовары КАК втТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК Поступления
	|		ПО втТовары.Номенклатура = Поступления.Номенклатура
	|			И втТовары.Характеристика = Поступления.Характеристика
	|			И (Поступления.ДатаПоступления <= &Дата)
	|			И (Поступления.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	втСводная.Характеристика КАК Характеристика,
	|	МАКСИМУМ(Поступления.НомерГТД) КАК НомерГТД
	|ИЗ
	|	втСводная КАК втСводная
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыПоступленияТоваровОрганизаций КАК Поступления
	|		ПО втСводная.Номенклатура = Поступления.Номенклатура
	|			И втСводная.Характеристика = Поступления.Характеристика
	|			И втСводная.ДатаПоступления = Поступления.ДатаПоступления
	|			И (Поступления.НомерГТД <> ЗНАЧЕНИЕ(Справочник.НомераГТД.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
	|	втСводная.Номенклатура,
	|	втСводная.Характеристика";
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("Дата",   Дата);
	...
	ДанныеГТД = РезультатЗапроса.Выгрузить();
	
	// Регистр накопления "ТоварыПереданныеНаКомиссию"
	ДозаполнитьДвиженияНомерамиГТД(ДанныеГТД, Движения.ТоварыПереданныеНаКомиссию);
	
	// Регистр накопления "ТоварыКОформлениюОтчетовКомитенту"
	ДозаполнитьДвиженияНомерамиГТД(ДанныеГТД, Движения.ТоварыКОформлениюОтчетовКомитенту);
	
	// Регистр накопления "ТоварыОрганизаций"
	ДозаполнитьДвиженияНомерамиГТД(ДанныеГТД, Движения.ТоварыОрганизаций);
	
	// Регистр накопления "ТоварыОрганизацийКПередаче"
	ДозаполнитьДвиженияНомерамиГТД(ДанныеГТД, Движения.ТоварыОрганизацийКПередаче);
	
КонецПроцедуры

Процедура ДозаполнитьДвиженияНомерамиГТД(ДанныеГТД, ТаблицаДвижений)
	
	Для каждого Движение Из ТаблицаДвижений Цикл
		Если ЗначениеЗаполнено(Движение.НомерГТД) Тогда
			Продолжить;
		КонецЕсли;
		
		ПараметрыОтбора = Новый Структура("Номенклатура, Характеристика");
		
		ЗаполнитьЗначенияСвойств(ПараметрыОтбора, Движение);
		
		НайденныеСтроки = ДанныеГТД.НайтиСтроки(ПараметрыОтбора);
		...
		Движение.НомерГТД = СтрокаСДанными.НомерГТД;
	КонецЦикла;
