...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	//13.01.2017 15:17:14
	ТекстЗапросаТаблицаВыручкаИСебестоимостьПродажКлиента(Запрос, ТекстыЗапроса, Регистры);	
	//}Кожемякин А.Г.
...
//Кожемякин А.Г. agkozhemyakin@gmail.com {
//13.01.2017 15:17:37
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродажКлиента(Запрос, ТекстыЗапроса, Регистры)
	ИмяРегистра = "ВыручкаИСебестоимостьПродажКлиента";
	...
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента КАК СуммаВыручки,
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента  КАК СуммаВыручкиБезНДС,
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента КАК СуммаВыручкиРегл,
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента  КАК СуммаВыручкиСНДСРегл,
	|
	...
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента КАК СуммаВВалютеВзаиморасчетов,
	//|	-(ВидыЗапасов.СуммаСНДС - ВидыЗапасов.СуммаНДС) КАК СуммаБезНДСВВалютеВзаиморасчетов,
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента КАК СуммаБезНДСВВалютеВзаиморасчетов,
	|
	|	&Валюта КАК ВалютаДокумента,
	//|	-ВидыЗапасов.СуммаСНДС КАК СуммаВВалютеДокумента,
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента КАК СуммаВВалютеДокумента,
	//|	-(ВидыЗапасов.СуммаСНДС - ВидыЗапасов.СуммаНДС) КАК СуммаБезНДСВВалютеДокумента,
	|	- ВидыЗапасов.Количество * ЦеныКлиента.ЦенаКлиента КАК СуммаБезНДСВВалютеДокумента,
	|
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//13.01.2017 15:21:05
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента.Товары КАК ЦеныКлиента
	|		ПО ВидыЗапасов.АналитикаРеализации = ЦеныКлиента.АналитикаУчетаНоменклатуры
	|			И ВидыЗапасов.Ссылка = ЦеныКлиента.Ссылка	
	//}Кожемякин А.Г.
	|ГДЕ
...
Функция СформироватьПечатнуюФормуВозвратОтКлиента(МассивОбъектов, ОбъектыПечати)
	...
	Запрос = Новый Запрос("
		...
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//19.09.2017 10:07:17
		|	Таблица.Характеристика                          				   КАК ХарактеристикаСсылка,	
		//}Кожемякин А.Г.
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Для Каждого ВыборкаПоТоварам Из ТаблицаТовары Цикл
			...
			Если ВыводитьКоды Тогда
				...
				//Кожемякин А.Г. agkozhemyakin@gmail.com {
				//19.09.2017 10:08:38
				ОбластьКодов.Параметры.КодЦС = дп_ДополнительныеПроцедуры.ПолучитьКодЦС(ВыборкаПоТоварам.Номенклатура, ВыборкаПоТоварам.ХарактеристикаСсылка);				
				//}Кожемякин А.Г.
				ТабличныйДокумент.Присоединить(ОбластьКодов);
			КонецЕсли;
