...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ЭтоПартнер Тогда
		...
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьОбщихФорм";
		КомандаПечати.Идентификатор = "ТОРГ12";
		КомандаПечати.Представление = НСтр("ru = 'ТОРГ-12 (Товарная накладная) + штрихкод'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Истина);
		КомандаПечати.ДополнительныеПараметры.Вставить("Штрихкод",    Истина);                  
		КомандаПечати.Порядок = 19.1;
		
		// Товарная накладная с номерами ГТД (ТОРГ-12)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьОбщихФорм";
		КомандаПечати.Идентификатор = "ТОРГ12";
		КомандаПечати.Представление = НСтр("ru = 'ТОРГ-12 (Товарная накладная) + КПП'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Истина);
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьКПП",    Истина);
		КомандаПечати.Порядок = 19.2;
		
		// Товарная накладная с номерами ГТД (ТОРГ-12)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьОбщихФорм";
		КомандаПечати.Идентификатор = "ТОРГ12";
		КомандаПечати.Представление = НСтр("ru = 'ТОРГ-12 (Товарная накладная) + КПП + Штрихкод'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Истина);
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьКПППоставщикПлательщик", Истина);
		КомандаПечати.ДополнительныеПараметры.Вставить("Штрихкод",    Истина);
		КомандаПечати.Порядок = 19.3;
		//}Кожемякин А.Г.
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//5/24/2018 11:17:38 AM
	//
	|	ВЫБОР КОГДА РеализацияТоваровУслуг.Основание = """" ТОГДА РеализацияТоваровУслуг.ЗаказКлиента ИНАЧЕ РеализацияТоваровУслуг.Основание КОНЕЦ КАК Основание,
	|	ВЫБОР КОГДА РеализацияТоваровУслуг.ОснованиеДата = ДатаВремя(1,1,1) ТОГДА РеализацияТоваровУслуг.ЗаказКлиента.Дата  ИНАЧЕ РеализацияТоваровУслуг.ОснованиеДата КОНЕЦ КАК ОснованиеДата,
	|	ВЫБОР КОГДА РеализацияТоваровУслуг.ОснованиеНомер = """" ТОГДА РеализацияТоваровУслуг.ЗаказКлиента.Номер ИНАЧЕ РеализацияТоваровУслуг.ОснованиеНомер КОНЕЦ КАК ОснованиеНомер,
	//}Кожемякин А.Г.
