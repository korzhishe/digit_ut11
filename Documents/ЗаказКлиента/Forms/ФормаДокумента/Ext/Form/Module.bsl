...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//04.04.2018 15:22:50
	//
	ЗаполнитьСтатусДляЗаказа();
	//}Кожемякин А.Г.
	
	// fld 25.01.2014 {{
	fldЗаполнитьКолонкуСвободныйОстаток();
	// }} fld
	
	
	ПроверитьПоказатьШККоробов();
	
	Элементы.ЗагрузкаШК.Видимость = Ложь;

		//ПРФт++  видимость порядка расчетов
	Если ПараметрыСеанса.ТекущийПользователь.Наименование = "Покровский Сергей" ИЛИ  ПараметрыСеанса.ТекущийПользователь.Наименование = "Горбунов Александр" ТОгда
		
		Элементы.НеОчищатьТоварыПриСменеСоглашения.Видимость = Истина;
	Иначе
		
		Элементы.НеОчищатьТоварыПриСменеСоглашения.Видимость = Ложь;
	КонецЕсли;
	
	//

	  Элементы.ЗагрузкаПоSKU.Видимость = Ложь;
ДоставкаМенеджерам();	
КонецПроцедуры
...
Процедура УстановитьУсловноеОформление()
	...
	//22.02.2018 14:46:56
	//
	УстановитьУсловноеОформлениеСтатусаЗаказа();
	//}Кожемякин А.Г.
...
Процедура ПриИзмененииСоглашенияСервер()
	...
	//ПРФТ++
	Если НеОчищатьТоварыПриСменеСоглашения ТОгда
	Иначе
		ПриИзмененииСкладаСервер();
	КонецЕсли;
	//ПРФТ--
...
//
&НаСервере
Процедура УстановитьУсловноеОформлениеСтатусаЗаказа()
	
	УО = ЭтаФорма.УсловноеОформление.Элементы.Добавить();
	...
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.СтатусДляЗаказа");
	УО.Оформление.УстановитьЗначениеПараметра("ЦветФона", WebЦвета.СветлоСерый);
	Поле = УО.Поля.Элементы.Добавить();	
	Поле.Поле = Новый ПолеКомпоновкиДанных("Товары");
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСтатусДляЗаказа()
...
//Кожемякин А.Г. agkozhemyakin@gmail.com {
//10/19/2017 3:18:24 PM
//
&НаКлиенте
Процедура ТоварыПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.Товары.ТекущиеДанные;
	дп_ДополнительныеПроцедурыКлиент.УстановитьКодЦС(ТекущиеДанные);

КонецПроцедуры
//}Кожемякин А.Г.

&НаСервере
Процедура fldЗаполнитьКолонкуСвободныйОстаток()
	
	ТаблицаТовары = Объект.Товары.Выгрузить();
	ТаблицаОстатков = fldПолучитьСвободныйОстаток(ТаблицаТовары);
	
	Для Каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
		
		СтрокаОстаток = ТаблицаОстатков.Найти(СтрокаТабличнойЧасти.НомерСтроки, "НомерСтроки");		
		СтрокаТабличнойЧасти.СвободныйОстаток = ?(СтрокаОстаток <> Неопределено, СтрокаОстаток.СвободныйОстаток, 0);
...
&НаСервере
Функция  fldПолучитьСвободныйОстаток(ТаблицаТовары)
	...
	|	СУММА(СвободныйОстаток.ВНаличииОстаток - СвободныйОстаток.ВРезервеСоСкладаОстаток) КАК СвободныйОстаток
	|ИЗ
	...
	|						ИЗ
	|							тзТовары КАК ТЗ)
	|					И Склад = &Склад) КАК СвободныйОстаток
	|		ПО тзТовары.Номенклатура = СвободныйОстаток.Номенклатура
	|			И тзТовары.Характеристика = СвободныйОстаток.Характеристика
	|
	|СГРУППИРОВАТЬ ПО
	|	тзТовары.НомерСтроки";
	Запрос.УстановитьПараметр("тзТовары", ТаблицаТовары);
...
&НаКлиенте
Процедура ИспользуютсяШтрихкодыКоробовПриИзменении(Элемент)
	ПроверитьПоказатьШККоробов();
КонецПроцедуры

Процедура ПроверитьПоказатьШККоробов()
	Элементы.ШтрихкодыКоробовГруппа.Видимость = Объект.ИспользуютсяШтрихкодыКоробов;	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьШККоробов(Команда)
	Элементы.ЗагрузкаШК.Видимость = истина;
	Для ТекСтрока = 1 По 1000 Цикл
	 ТабДок.Область(ТекСтрока,1).Текст = "";
...
&НаКлиенте
Процедура ЗагрузитьШК(Команда)
	Объект.ШтрихкодыКоробов.Очистить();
	Для ТекСтрока = 1 По 1000 Цикл
		Если НЕ ТабДок.Область(ТекСтрока,1).Текст = "" Тогда
			НоваяСтрока = Объект.ШтрихкодыКоробов.Добавить();
			НоваяСтрока.Штрихкод =  ТабДок.Область(ТекСтрока,1).Текст;
		КонецЕсли;
	КонецЦикла;
Элементы.ЗагрузкаШК.Видимость = Ложь;
...
//ПРФТ  ++   Херомантия для доставки
Процедура ДоставкаМенеджерам()
	
	Элементы.АдресДоставки.СписокВыбора.Очистить();
	Если Объект.Контрагент.ИНН = "7704217370" ТОгда
		Элементы.АдресДоставки.Видимость = Истина;
		Для Каждого ТекСтрока Из   Элементы.АдресДоставкиСамовывоз.СписокВыбора Цикл
			НоваСтркоа = Элементы.АдресДоставки.СписокВыбора.Добавить();
			НоваСтркоа.Значение = ТекСтрока.Значение.АдресДоставки;
			//НоваСтркоа.Представление = ТекСтрока.Значение.Представление;
			//ЗаполнитьЗначенияСвойств(НоваСтркоа, ТекСтрока);
		КонецЦикла;
	Иначе
...
&НаСервере
Процедура ЖелаемаяДатаОтгрузкиПриИзмененииНаСервере()
	
	Если Объект.СостояниеПодтвержденияЗаказа = Перечисления.СостоянияПодтвержденияЗаказа.ТребуетПодтверждения ТОгда
		Для Каждого ТекСтрока Из Объект.ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Желаемая менеджером дата отгрузки") ТОгда
				ТекСтрока.Значение = Дата(1,1,1);
			КонецЕсли;
		КонецЦикла;
		СостояниеПодтвержденияЗаказа = Перечисления.СостоянияПодтвержденияЗаказа.ПустаяСсылка();
	КонецЕсли;	
КонецПроцедуры

&НаКлиенте
Процедура ЖелаемаяДатаОтгрузкиПриИзменении(Элемент)
	ЖелаемаяДатаОтгрузкиПриИзмененииНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоSKU(Команда)
	Элементы.ЗагрузкаПоSKU.Видимость = Истина;
	ЗаполнитьПоSKUНаСервере();
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПоSKUНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьSKUИзТабличногоДокумента(Команда)
	
	
	Для ТекСтрока = 1 По 1000 Цикл
		Если НЕ ТабДок.Область(ТекСтрока,1).Текст = "" Тогда
			ЗагрузитьSKUИзТабличногоДокументаНаСервере(ТабДок.Область(ТекСтрока,1).Текст, ТабДок.Область(ТекСтрока,2).Текст);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ЗагрузитьSKUИзТабличногоДокументаНаСервере(СКУ, Количество)
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	КодыЦентральногоСправочника.Номенклатура КАК Номенклатура,
	|	КодыЦентральногоСправочника.Характеристика КАК Характеристика
	|ИЗ
	|	РегистрСведений.КодыЦентральногоСправочника КАК КодыЦентральногоСправочника
	|ГДЕ
	|	КодыЦентральногоСправочника.КодЦС = &КодЦС");	
	Запрос.УстановитьПараметр("КодЦС",СКУ);
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
