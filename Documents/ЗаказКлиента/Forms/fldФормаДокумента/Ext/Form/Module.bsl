...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МногооборотнаяТараСервер.УстановитьЗаголовокИтоговДокументаБезТары(Элементы.ГруппаИтого.Заголовок);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить("ЗаполнитьДубликатыЗависимыхРеквизитов", ПолучитьСтруктуруЗависимыхРеквизитов());
	СтруктураДействий.Вставить("ПроверитьЗаполнитьВариантОбеспечения", ВариантОбеспечения(Объект));
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	КонецЕсли;
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьЗаполнитьВариантОбеспечения", ВариантОбеспечения(Объект));
...
Процедура ПроверкаЗаполненияИРасчетСтатуса(КэшированныеЗначения, РезультатПроверки)
	...
	ТекстВопроса = НСтр("ru= 'Для автоматического расчета статуса требуется'") + ":" + Символы.ПС;
...
Процедура РассчитатьСтатусДокументаСервер(КэшированныеЗначения, СтатусРассчитан)
	...
	Если Объект.ВернутьМногооборотнуюТару Тогда
		...
		Для Каждого ТекСтрока Из Объект.Товары Цикл
			Если ТекСтрока.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара И
				Не ТекСтрока.Отменено Тогда
				СуммаВозвратнойТары = СуммаВозвратнойТары + ТекСтрока.СуммаСНДС;
			КонецЕсли;
		КонецЦикла;
	...
	ПараметрыРасчета.Вставить("СуммаВозвратнойТары", СуммаВозвратнойТары);
...
Процедура ЗаполнитьБанковскийСчетИКассуПоОрганизации(Организация)
	...
	СвойстваАвтозаполнения = Новый Структура("Организация, БанковскийСчет, Касса", Объект.Организация, Объект.БанковскийСчет, Объект.Касса);
...
Процедура ЗаполнитьМногооборотнуюТаруИзХранилищаСервер(АдресТарыВХранилище)
	...
	СтруктураДействийДобавленныеСтроки.Вставить("ПроверитьЗаполнитьВариантОбеспечения", ВариантОбеспечения(Объект));
...
&НаКлиентеНаСервереБезКонтекста
Функция ВариантОбеспечения(Объект)
...
Функция  fldПолучитьСвободныйОстаток(ТаблицаТовары)
	...
	|	СУММА(СвободныйОстаток.ВНаличииОстаток - СвободныйОстаток.ВРезервеОстаток) КАК СвободныйОстаток
	|ИЗ
