...
Функция ЗаполнитьВариантОбеспечения(Объект, Форма, Операция, ДанныеЗаполнения, ПараметрыУказанияСерий = Неопределено, ЗависимыеРеквизиты = Неопределено) Экспорт
	...
	Для Каждого СтрокаОбеспечения Из ДанныеЗаполнения Цикл
		...
		Если Идентификатор <> СтрокаОбеспечения.Идентификатор Тогда
			...
			Если Операция = "СтрокаТовары" Или Операция = "СтрокиТовары" Или Операция = "Заказ" Тогда
				...
				Исключение
				    СтрокаТовары = Объект.Товары.Получить(Идентификатор);
				КонецПопытки;				
				//}Кожемякин А.Г.
	...
	Если Операция = "СтрокаТовары" Или Операция = "СтрокиТовары" Или Операция = "Заказ" Тогда
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//6/27/2017 6:42:15 PM
		//Форма.Модифицированность = Истина;
		Попытка Форма.Модифицированность = Истина; Исключение КонецПопытки;
		//}Кожемякин А.Г.
	КонецЕсли;
...
Функция ПолучитьДанныеДляПечатнойФормыЗаказаНаТоварыУслуги(МассивОбъектов, ПараметрыПечати) Экспорт
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//9/29/2017 9:52:11 AM
	//
	|	ЕСТЬNULL(Товары.Характеристика, Значение(Справочник.ХарактеристикиНоменклатуры.ПустаяСсылка)) 					 КАК ХарактеристикаСсылка,
	//}Кожемякин А.Г.
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	//Кожемякин А.Г. agkozhemyakin@gmail.com {
	//08.07.2016 10:18:13
	//добавим печать документа реализации
	//получим массив подчиненных реализаций и передадим его на печать
	МассивРеализаций = дп_ДополнительныеПроцедуры.ПолучитьРеализацииЗаказов(МассивОбъектов);
	Если МассивРеализаций.Количество() Тогда
		Документы.РеализацияТоваровУслуг.СформироватьКомплектПечатныхФорм(МассивРеализаций, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати);
	КонецЕсли;
	//}Кожемякин А.Г.		
КонецФункции
...
Функция ПолучитьДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	СтруктураДействий.Вставить("УстановитьПартнера", МассивДокументов[0].Партнер);
