#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ЗаполнитьУсловияПродаж(Знач УсловияПродаж) Экспорт
		...
		Иначе
			...
			//ПРФТ++
			ЖелаемаяДатаОтгрузки = ЭтотОбъект.Дата;
		КонецЕсли;
	...
	Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		Если ПроверятьДату Тогда
			если НЕ ЗначениеЗаполнено(ЖелаемаяДатаОтгрузки) Тогда
				Сообщить("Не заполнена желаемая дата отгрузки!!!");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ РольДоступна("ПРФТ_ПодтверждениеЗаказовСкладом") Тогда
			...
			|ГДЕ
			|	ПартнерыДополнительныеРеквизиты.Свойство.Наименование ПОДОБНО ""%"" + ""Индивидуальное время на сборку"" + ""%""
			|	И ПартнерыДополнительныеРеквизиты.Ссылка = &Объект";	
			Запрос.УстановитьПараметр("Объект", Партнер);	
			РезультатЗапроса = Запрос.Выполнить().Выбрать();
			Если РезультатЗапроса.Следующий() Тогда
				ИндивидуальноеВремя = РезультатЗапроса.Значение;	
			КонецЕсли;
			Если ЕстьОрдер() ТОгда
			Иначе
				Если ИндивидуальноеВремя = 0 Тогда
					
					Времяпроверка =(Константы.ВремяЗапретаСозданияЗаказовНаСклад.Получить() -НачалоДня(Константы.ВремяЗапретаСозданияЗаказовНаСклад.Получить())) - (ТекущаяДата() - началоДня(ТекущаяДата()));
					Если НачалоДня(ЖелаемаяДатаОтгрузки) - НачалоДня(ТекущаяДата()) = 86400 И  Времяпроверка < 0  ТОгда
						
						ЖелаемаяДатаЕсть = Ложь;
						Для Каждого ТекСтрока Из ДополнительныеРеквизиты Цикл
							Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Желаемая менеджером дата отгрузки") ТОгда
								ЖелаемаяДатаЕсть = Истина;
								ТекСтрока.Значение = ЖелаемаяДатаОтгрузки;
							КонецЕсли;
						КонецЦикла;
						
						Если Не ЖелаемаяДатаЕсть ТОгда
							НоваяСтркоа = ДополнительныеРеквизиты.Добавить();
							НоваяСтркоа.Значение = ЖелаемаяДатаОтгрузки;
							НоваяСтркоа.Свойство =   ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Желаемая менеджером дата отгрузки");
						КонецЕсли;
						
						//Если (ТекущаяДата() - НачалоДня(ТекущаяДата()))  > ВремяВСекундах Тогда
						СостояниеПодтвержденияЗаказа = Перечисления.СостоянияПодтвержденияЗаказа.ТребуетПодтверждения;
						ЖелаемаяДатаОтгрузки = ЖелаемаяДатаОтгрузки+86400;
						Сообщить("Внимание!!!! Желаемая дата заказа установлена на " + Формат(ЖелаемаяДатаОтгрузки,"ДФ=dd.MM.yyyy") + ", для утверждения даты:"  + Формат(ЖелаемаяДатаОтгрузки-86400,"ДФ=dd.MM.yyyy") + " требуется подтверждение сотрудника склада!!!");
						//КонецЕсли;
						
						
					КонецЕсли;
				Иначе
					Времяпроверка =(ТекущаяДата() + 3600*ИндивидуальноеВремя);
					Если НачалоДня(ЖелаемаяДатаОтгрузки) < Времяпроверка   ТОгда
						//Если (ТекущаяДата() - НачалоДня(ТекущаяДата()))  > ВремяВСекундах Тогда
						ЖелаемаяДатаЕсть = Ложь;
						Для Каждого ТекСтрока Из ДополнительныеРеквизиты Цикл
							Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Желаемая менеджером дата отгрузки") ТОгда
						...
						ЖелаемаяДатаОтгрузки = ЖелаемаяДатаОтгрузки+86400;
						//Сообщить("Внимание!!!! Желаемая дата заказа установлена на " + Формат(ЖелаемаяДатаОтгрузки,"ДФ=dd.MM.yyyy") + ", для утверждения даты:"  + Формат(ЖелаемаяДатаОтгрузки-86400,"ДФ=dd.MM.yyyy") + " требуется подтверждение сотрудника склада!!!");
						Сообщить("Внимание!!!! У данного клиента особые условия сборки. Дата не может быть ранее " + Формат(ЖелаемаяДатаОтгрузки,"ДФ=dd.MM.yyyy") + ", ожидайте подтверждение  склада!!!");
						//КонецЕсли;
					КонецЕсли;
		...
		КонтрольЖелаемаяДатаЕсть = Ложь;
		Для Каждого ТекСтрока Из ДополнительныеРеквизиты Цикл
			Если ТекСтрока.Свойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоНаименованию("Желаемая менеджером дата отгрузки") ТОгда
				КонтрольЖелаемаяДатаЕсть = Истина;
				
			КонецЕсли;
		КонецЦикла;
		
		Если Не КонтрольЖелаемаяДатаЕсть ТОгда
	...
	Функция ЕстьОрдер ()
		...
		Попытка
			...
			|ИЗ
			|	РегистрНакопления.ТоварыКОтгрузке.ОстаткиИОбороты КАК ТоварыКОтгрузкеОстаткиИОбороты
			|ГДЕ
			|	ТоварыКОтгрузкеОстаткиИОбороты.ДокументОтгрузки = &ДокументОтгрузки");
			Запрос.УстановитьПараметр("ДокументОтгрузки", Ссылка);
			Выборка = ЗАпрос.Выполнить().Выбрать();
			Если Выборка.Следующий() ТОгда
				Если Выборка.КОтгрузкеПриход > 0 Тогда
					ОрдерБыл = Истина;
				Иначе
					ОрдерБыл = Ложь;
				КонецЕсли;
	...
	Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
		...
		дп_ДополнительныеПроцедуры.ПроверитьЖелаемуюДатуОтгрузки(Отказ, ЭтотОбъект);
	...
	Процедура ОбработкаПроведения(Отказ, РежимПроведения)
		...
		//////ПРФТ++  Временно
		////Для КАждого ТекСТрока ИЗ ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТоварыКОтгрузке Цикл
