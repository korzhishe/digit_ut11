...
Функция ПолучитьДанныеДляПечатиЭтикеток(МассивДокументов) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ЗаказКлиентаТовары.Ссылка.СкладОтправитель.ТипСклада = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
	...
	|ИЗ
	|	Документ.ЗаказНаПеремещение.Товары КАК ЗаказКлиентаТовары
	...
	|			И (ЦеныНоменклатуры.ВидЦены = ЗаказКлиентаТовары.Ссылка.СкладОтправитель.РозничныйВидЦены)
	|ГДЕ
