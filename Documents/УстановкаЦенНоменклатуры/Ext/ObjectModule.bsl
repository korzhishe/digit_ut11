...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ПРФТ++
	Если СтрНайти(Комментарий,"закупочным ценам") > 0  Тогда
		Движения.ЦеныНоменклатуры.Очистить();
		Движения.ЦеныНоменклатуры.Записывать = Истина;
		Для каждого ТекСтрока ИЗ Товары Цикл
			Если ТекСтрока.Цена = 0 Тогда Продолжить; КонецЕсли;
			НовДвиж = Движения.ЦеныНоменклатуры.Добавить();
			НовДвиж.Период = дата;
			НовДвиж.Регистратор = ССылка;
			НовДвиж.Номенклатура = ТекСтрока.Номенклатура;
			НовДвиж.Характеристика = ТекСтрока.Характеристика;
			НовДвиж.ВидЦены = ТекСтрока.ВидЦены;
			НовДвиж.Цена = ТекСтрока.Цена;
			НовДвиж.Валюта = Константы.ВалютаРегламентированногоУчета.Получить();
		КонецЦикла;
	КонецЕсли;
