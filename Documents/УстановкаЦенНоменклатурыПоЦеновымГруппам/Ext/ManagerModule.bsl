...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	МенежерТаблицЗапроса 			= Новый МенеджерВременныхТаблиц;
	Запрос.МенеджерВременныхТаблиц 	= МенежерТаблицЗапроса;
	
	Запрос.Текст =  
		"ВЫБРАТЬ
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦеноваяГруппа,
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ВидЦены,
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Цена,
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦенаРаспродажа,
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Формула,
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ФормулаРаспродажа,
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Ссылка
		|ПОМЕСТИТЬ табЦеновыеГруппы
		|ИЗ
		|	Документ.УстановкаЦенНоменклатурыПоЦеновымГруппам.ЦеновыеГруппы КАК УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы
		|ГДЕ
		|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Ссылка = &Ссылка
		|	И УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыУстановокЦенНоменклатуры.Согласован)
		|	И (УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Цена > 0
		|			ИЛИ УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦенаРаспродажа > 0)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	НЕ Товары.Номенклатура.ВидНоменклатуры.ИспользоватьХарактеристики
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	Товары.Номенклатура,
		|	Товары.ЦеноваяГруппа,
		...
		|					ТОГДА Товары.Номенклатура.ВидНоменклатуры = ХарактеристикиНоменклатуры.Владелец
		|				ИНАЧЕ Товары.Номенклатура.Ссылка = ХарактеристикиНоменклатуры.Владелец
		|			КОНЕЦ)
		|ГДЕ
		|	Товары.Номенклатура.ВидНоменклатуры.ИспользоватьХарактеристики
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Номенклатура
		|;
		...
		|	ВТТовары.ЦеноваяГруппа КАК ЦеноваяГруппа
		|ПОМЕСТИТЬ ТоварыРаспродажа
		|ИЗ
		...
		|ГДЕ
		|	НоменклатураДополнительныеРеквизиты.Свойство.Заголовок = ""Распродажа""
		...
		|	ВТТовары.ЦеноваяГруппа,
		|	ВЫБОР
		|		КОГДА ТоварыРаспродажа.Номенклатура ЕСТЬ NULL
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК НеРаспродажный
		|ПОМЕСТИТЬ Фильтр
		|ИЗ
		|	ВТТовары КАК ВТТовары
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТоварыРаспродажа КАК ТоварыРаспродажа
		|		ПО ВТТовары.Номенклатура = ТоварыРаспродажа.Номенклатура";
...
Функция ТекстЗапросаТаблицаЦеныНоменклатуры(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ВидЦены,
	               |	ВЫБОР
	               |		КОГДА Фильтр.НеРаспродажный
	               |			ТОГДА УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Цена
	               |		ИНАЧЕ УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦенаРаспродажа
	               |	КОНЕЦ КАК ЦЕНА,
	               |	Фильтр.Номенклатура.ЕдиницаИзмерения КАК Упаковка,
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ВидЦены.ВалютаЦены КАК Валюта,
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Ссылка.Дата КАК ПЕРИОД
	               |ИЗ
	               |	Фильтр КАК Фильтр
	               |		ЛЕВОЕ СОЕДИНЕНИЕ табЦеновыеГруппы КАК УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы
	               |		ПО (УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦеноваяГруппа = Фильтр.ЦеноваяГруппа)
	               |ГДЕ
	               |	ВЫБОР
	               |			КОГДА Фильтр.НеРаспродажный
	               |				ТОГДА УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Цена
	...
	ИмяРегистра = "дп_ЦеныПоЦеновымГруппам";
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦеноваяГруппа КАК ЦеноваяГруппа,
	|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ВидЦены,
	|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Цена,
	|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ЦенаРаспродажа как ЦенаРаспродажа,
	|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ВидЦены.ВалютаЦены КАК Валюта,
	|	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Ссылка.Дата КАК ПЕРИОД
	|ИЗ
	|	табЦеновыеГруппы КАК УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы
	|";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса, ИмяРегистра);
	
	ИмяРегистра = "дп_ФормулыПоЦеновымГруппам";
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ВидЦены,
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Формула,
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ФормулаРаспродажа КАК ФормулаРаспродажа,
	               |	УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Ссылка.Дата КАК ПЕРИОД
	               |ИЗ
	               |	табЦеновыеГруппы КАК УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы
	               |ГДЕ
	               |	(ВЫРАЗИТЬ(УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.Формула как Строка(500)) <> """"
	               |			ИЛИ ВЫРАЗИТЬ(УстановкаЦенНоменклатурыПоЦеновымГруппамЦеновыеГруппы.ФормулаРаспродажа как Строка(500)) <> """")";
