...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	//Если ПравоДоступа("Добавление", Метаданные.Документы.УдалитьАктПриемкиТоваров) Тогда
	//	КомандаСоздатьНаОсновании = КомандыСоздатьНаОсновании.Добавить();
	//	КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.УдалитьАктПриемкиТоваров.ПолноеИмя();
	//	КомандаСоздатьНаОсновании.Представление = ВводНаОсновании.ПредставлениеОбъекта(Метаданные.Документы.УдалитьАктПриемкиТоваров);
	//	КомандаСоздатьНаОсновании.ПроверкаПроведенияПередСозданиемНаОсновании = Истина;
	//	КомандаСоздатьНаОсновании.ФункциональныеОпции = "УдалитьИспользоватьАктыПриемкиТоваров,ИспользоватьАктыРасхожденийПослеОтгрузкиПоРеализациям";
	//	
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.УдалитьАктПриемкиТоваров)
		 И ПолучитьФункциональнуюОпцию("ИспользоватьНоменклатуруПоставщиков") Тогда
		// ТОРГ-1 (в номенклатуре поставщика)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "ТОРГ1";
...
Процедура ЗаполнитьТабличныйДокументТОРГ1(ПечатьВНоменклатуреПоставщика, ТабличныйДокумент, Запрос, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.УдалитьАктПриемкиТоваров.ПФ_MXL_ТОРГ1");
...
Процедура СформироватьТекстЗапросаПоДокументам(ТекстЗапроса)
	...
	|ИЗ
	|	Документ.УдалитьАктПриемкиТоваров КАК АктПриемкиТоваров
	...
	|				И (НЕ ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) = 1)
	|			ТОГДА ПоступлениеТоваровУслугТоварыПоДаннымПоставщика.Упаковка
	|		КОГДА (НЕ ПоступлениеТоваровУслугТовары.Упаковка ЕСТЬ NULL )
	|				И (НЕ ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) = 1)
	|			ТОГДА ПоступлениеТоваровУслугТоварыПоДаннымПоставщика.Упаковка
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Упаковка,
	|	ВЫБОР
	|		КОГДА ПоступлениеТоваровУслугТоварыПоДаннымПоставщика.Упаковка ЕСТЬ НЕ NULL
	|			ТОГДА &ТекстЗапросаЕдиницаИзмерения2
	...
	|			И (АктПриемкиТоваров.ДокументОснование.УдалитьЕстьРасхождения = ИСТИНА)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугТовары
	|		ПО АктПриемкиТоваров.ДокументОснование = ПоступлениеТоваровУслугТовары.Ссылка
	|			И (АктПриемкиТоваров.ДокументОснование.УдалитьЕстьРасхождения = ЛОЖЬ)
	|ГДЕ
	|	АктПриемкиТоваров.Ссылка В(&МассивОбъектов)
	...
	|	ИЗ
	|		Документ.УдалитьАктПриемкиТоваров КАК ДанныеДокумента
...
Функция ДоступныеДляШаблоновПечатныеФормы() Экспорт
	...
	МассивДоступныхПечатныхФорм.Добавить(ШаблоныСообщенийСервер.СтруктураПараметровДоступнойПечатнойФормы( 
	                                     "ТОРГ1", НСтр("ru = 'ТОРГ-1'"), "Документ.УдалитьАктПриемкиТоваров", Неопределено));
