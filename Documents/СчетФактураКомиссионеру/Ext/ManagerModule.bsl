...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	ЗапросСФ = Новый Запрос("ВЫБРАТЬ
	                        ...
	                        |	СчетФактураКомиссионеру.Комиссионер КАК Грузополучатель,
	                        |	СчетФактураКомиссионеру.Комиссионер КАК Контрагент,
	                        |	СчетФактураКомиссионеру.Комиссионер.ИНН КАК ИННПокупателя,
	                        |	СчетФактураКомиссионеру.Комиссионер.КПП КАК КПППокупателя
	                        |ИЗ
	                        |	Документ.СчетФактураКомиссионеру КАК СчетФактураКомиссионеру
	                        |ГДЕ
	                        |	СчетФактураКомиссионеру.Ссылка В(&МассивОбъектов)
	                        |ИТОГИ ПО
	                        |	Ссылка");
	ЗапросСФ.УстановитьПараметр("МассивОбъектов",МассивОбъектов);
	РезультатПоКонтрагентам   = ЗапросСФ.Выполнить();
	//ПРФТ--
