...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//ПРФТ++
	Для Каждого ТекСтрока Из ТаблицаОстатковРасчетов Цикл
		Попытка
			ТекСтрока.ДокЗаказ = ТекСтрока.Заказ.ЗаказКлиента;
			...
			ЗапросСФ.УстановитьПараметр("ДокументОснование",ТекСтрока.Заказ);
			Выборка = ЗапросСФ.Выполнить().Выбрать();
			Если Выборка.Следующий() Тогда
				ТекСтрока.СчетФактура = Выборка.ССылка;	
			КонецЕсли;
		Исключение
		КонецПопытки;
	КонецЦикла;
