...
Процедура ЗаполнитьДоступныеПоля()
	...
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЦенникДляТоваров
		...
		//19.05.2017 15:26:35
		ИменаПолей.Добавить("Номенклатура.Производитель.Описание");	
		//}Кожемякин А.Г.
		...
		//Кожемякин А.Г. agkozhemyakin@gmail.com {
		//18.05.2017 17:45:42
		//добавим поле импортер идет из реквизита дпИмпортер заказа поставщика 
		ИменаПолей.Добавить("Импортер");
		ИменаПолей.Добавить("ШтрихкодПартнера");
		ИменаПолей.Добавить("КодЦС");
		//}Кожемякин А.Г.
		
		//БА_Начало
		ИменаПолей.Добавить("НомерГТДИмпортер");
		ИменаПолей.Добавить("НомерГТДДатаПроизводства");
		//БА_Конец
	ИначеЕсли Объект.Назначение = Перечисления.НазначенияШаблоновЭтикетокИЦенников.ЭтикеткаДляДоставки Тогда
