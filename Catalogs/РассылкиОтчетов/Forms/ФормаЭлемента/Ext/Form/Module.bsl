...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовый Тогда
		...
		Если СозданКопированием Тогда
			...
			Если АвторОснования = ТекущийПользователь Тогда
				...
				Пароли = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(РассылкаОснование, "ПарольАрхива, FTPПароль, SharePointПароль");
				...
				КонецЕсли;
				Если ЗначениеЗаполнено(Пароли.SharePointПароль) Тогда
					SharePointПароль = ПарольСкрыт();
					SharePointПарольИзменен = Истина; // См. обработку этого параметра в ПриЗаписиНаСервере.
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		УстановитьПривилегированныйРежим(Истина);
		Пароли = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(Объект.Ссылка, "ПарольАрхива, FTPПароль, SharePointПароль");
		УстановитьПривилегированныйРежим(Ложь);
		ПарольАрхива = ?(ЗначениеЗаполнено(Пароли.ПарольАрхива), ПарольСкрыт(), "");
		FTPПароль = ?(ЗначениеЗаполнено(Пароли.FTPПароль), ПарольСкрыт(), "");
		SharePointПароль = ?(ЗначениеЗаполнено(Пароли.SharePointПароль), ПарольСкрыт(), "");
	КонецЕсли;
	Пароли = Неопределено;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.Подготовлена Тогда
		...
		Если Объект.ИспользоватьSharePointРесурс Тогда
			Если Не ЗначениеЗаполнено(Объект.SharePointСервер)
				Или Не ЗначениеЗаполнено(Объект.SharePointКаталог) Тогда
				Отказ = Истина;
				ТекстСообщения = НСтр("ru = 'Не введен SharePoint адрес'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "SharePointСерверИКаталог");
			КонецЕсли;
		КонецЕсли;
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	SharePointПарольИзмененНоСкрыт = SharePointПарольИзменен И SharePointПароль = ПарольСкрыт();
	
	Если (ПарольАрхиваИзмененНоСкрыт Или FTPПарольИзмененНоСкрыт или SharePointПарольИзмененНоСкрыт) И ЗначениеЗаполнено(РассылкаОснование) Тогда
		ТекущийПользователь = Пользователи.ТекущийПользователь();
		АвторОснования = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РассылкаОснование, "Автор");
		Если АвторОснования = ТекущийПользователь Тогда
			УстановитьПривилегированныйРежим(Истина);
			...
			КонецЕсли;
			Если SharePointПарольИзмененНоСкрыт Тогда
				ВременнаяПеременная = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(РассылкаОснование, "SharePointПароль");
				ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, ВременнаяПеременная, "SharePointПароль");
				SharePointПарольИзменен = Ложь;
			КонецЕсли;
			УстановитьПривилегированныйРежим(Ложь);
		КонецЕсли;
	...
	Если SharePointПарольИзменен Тогда
		УстановитьПривилегированныйРежим(Истина);
		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(ТекущийОбъект.Ссылка, SharePointПароль, "SharePointПароль");
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура ПроверитьПубликацию(Команда)
	...
	ПараметрыДоставки.Вставить("ИспользоватьSharePointРесурс", Объект.ИспользоватьSharePointРесурс);
	ПараметрыДоставки.Вставить("ИспользоватьOneDriveРесурс",	Объект.ИспользоватьOneDriveРесурс);
...
Процедура ПроверитьЭлектроннуюПочту(Команда)
	...
	ПараметрыДоставки.Вставить("ИспользоватьSharePointРесурс", Ложь);
	ПараметрыДоставки.Вставить("ИспользоватьOneDriveРесурс", Ложь);
...
Процедура ПроверитьРассылкуПослеОтветаНаВопрос(Ответ, ПараметрыДоставки) Экспорт
	...
	// SharePoint.
	Если ПараметрыДоставки.ИспользоватьSharePointРесурс Тогда
		ПараметрыДоставки.Вставить("Владелец",            Объект.Ссылка);
		ПараметрыДоставки.Вставить("Сервер",              Объект.SharePointСервер);
		ПараметрыДоставки.Вставить("Логин",               Объект.SharePointЛогин);
		Если SharePointПарольИзменен Тогда
			ПараметрыДоставки.Вставить("Пароль", SharePointПароль);
		КонецЕсли;
		ПараметрыДоставки.Вставить("Каталог",             Объект.SharePointКаталог);	
		ПараметрыДоставки.Вставить("ИДКлиента",           Объект.SharePointИДКлиента);
		ПараметрыДоставки.Вставить("СекретКлиента",       Объект.SharePointСекретКлиента);
		ПараметрыДоставки.Вставить("ТенантИД",            Объект.SharePointТенантИД);
	КонецЕсли;
	
	// OneDrive.
	Если ПараметрыДоставки.ИспользоватьOneDriveРесурс Тогда
		ПараметрыДоставки.Вставить("Владелец",            Объект.Ссылка);
		ПараметрыДоставки.Вставить("Логин",               Объект.OneDriveЛогин);
		ПараметрыДоставки.Вставить("Каталог",             Объект.OneDriveКаталог);
		ПараметрыДоставки.Вставить("ИДКлиента",           Объект.OneDriveИДКлиента);
		ПараметрыДоставки.Вставить("СекретКлиента",       Объект.OneDriveСекретКлиента);
		ПараметрыДоставки.Вставить("ТенантИД",            Объект.OneDriveТенантИД);
	КонецЕсли;
...
Процедура ВычислитьФлажкиДополнительныхСпособовДоставки()
	...
	Объект.ИспользоватьSharePointРесурс    	= Публиковать И (ДругойСпособДоставки = "ИспользоватьSharePointРесурс");
	Объект.ИспользоватьOneDriveРесурс    	= Публиковать И (ДругойСпособДоставки = "ИспользоватьOneDriveРесурс");
КонецПроцедуры
...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или  Изменения = "OneDriveКаталог" Тогда 
		Форма.OneDriveКаталог = Объект.OneDriveКаталог;
	ИначеЕсли Изменения = "" Или  Изменения = "SharePointСерверИКаталог" Тогда 
		Если ЗначениеЗаполнено(Объект.SharePointСервер) Тогда
			ПредставлениеАдреса = "https://";
			Форма.SharePointСерверИКаталог = ПредставлениеАдреса + Объект.SharePointСервер + Объект.SharePointКаталог;
		Иначе
			Форма.SharePointСерверИКаталог = "";
		КонецЕсли;
	ИначеЕсли Изменения = "" Или Изменения = "FTPСерверИКаталог" Тогда
	...
	Если Изменения = ""
		...
		Элементы.SharePointСерверИКаталог.АвтоОтметкаНезаполненного     = Объект.Подготовлена И Форма.Публиковать;
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Если Не РассылкаОбщая Тогда
			...
			Объект.ИспользоватьSharePointРесурс = Ложь;
			Объект.ИспользоватьOneDriveРесурс 	= Ложь;
			Объект.ИспользоватьЭлектроннуюПочту = Истина;
		КонецЕсли;
		...
		ИначеЕсли Объект.ИспользоватьFTPРесурс Тогда
			...
			Форма.Публиковать = Истина;
		ИначеЕсли Объект.ИспользоватьSharePointРесурс Тогда
			Форма.ДругойСпособДоставки = "ИспользоватьSharePointРесурс";
			Форма.Публиковать = Истина;
		ИначеЕсли Объект.ИспользоватьOneDriveРесурс Тогда
			Форма.ДругойСпособДоставки = "ИспользоватьOneDriveРесурс";
			Форма.Публиковать = Истина;	
		Иначе
	...
	Если Изменения = ""
		...
		КонецЕсли;
		Если Объект.ИспользоватьSharePointРесурс Тогда
			СпособыДоставки = НСтр("ru = 'SharePoint'");
		КонецЕсли;
		Если Объект.ИспользоватьOneDriveРесурс Тогда
			СпособыДоставки = НСтр("ru = 'OneDrive'");
		КонецЕсли;
...
Функция ПроверитьСпособДоставки(Рассылка, Знач ПараметрыДоставки)
	...
	ПараметрыДоставки.Вставить("ВыполненаНаSharePoint",       Ложь);
	ПараметрыДоставки.Вставить("ВыполненаНаOneDrive",       Ложь);
...
&НаКлиенте
Процедура SharePointСерверИКаталогПриИзменении(Элемент)
	ВыбранноеЗначение = РассылкаОтчетовКлиентСервер.РазобратьSharePointАдрес(SharePointСерверИКаталог);
	SharePointСерверИКаталогОбработкаВыбора(Элемент, ВыбранноеЗначение, Истина);
КонецПроцедуры

&НаКлиенте
Процедура SharePointСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ПараметрыПроизвольнойФормы = Новый Структура("Сервер, Каталог, Логин, ИДКлиента, СекретКлиента, ТенантИД");
	Для Каждого КлючИЗначение Из ПараметрыПроизвольнойФормы Цикл
		ПараметрыПроизвольнойФормы[КлючИЗначение.Ключ] = Объект["SharePoint" + КлючИЗначение.Ключ];
	КонецЦикла;
	
	ПараметрыПроизвольнойФормы.Вставить("Пароль", SharePointПароль);
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ru = '<Укажите получателя>'"));
	
	ОткрытьФорму("Справочник.РассылкиОтчетов.Форма.ПараметрыSharePoint", ПараметрыПроизвольнойФормы, Элемент);
КонецПроцедуры

&НаКлиенте
Процедура SharePointСерверИКаталогОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура SharePointСерверИКаталогОткрытие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	ПолныйАдрес = "https://"+ Объект.SharePointСервер + Объект.SharePointКаталог;
	ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(ПолныйАдрес);
КонецПроцедуры

&НаКлиенте
Процедура SharePointСерверИКаталогОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если ВыбранноеЗначение = Неопределено Или ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	Для Каждого КлючИЗначение Из ВыбранноеЗначение Цикл
		Если КлючИЗначение.Ключ = "Пароль" Тогда
			Если КлючИЗначение.Значение <> SharePointПароль И КлючИЗначение.Значение <> ПарольСкрыт() Тогда
				SharePointПароль = КлючИЗначение.Значение;
				SharePointПарольИзменен = Истина;
			КонецЕсли;
		Иначе
			Объект["SharePoint" + КлючИЗначение.Ключ] = КлючИЗначение.Значение;
		КонецЕсли;
	КонецЦикла;
	
	ВидимостьДоступностьКорректность(ЭтотОбъект, "SharePointСерверИКаталог");
	Модифицированность = Истина;
КонецПроцедуры

//Кожемякин А.Г. agkozhemyakin@gmail.com {
//1/30/2020 2:56:33 PM
//Функции и процедуры OneDrive
&НаКлиенте
Процедура OneDriveКаталогПриИзменении(Элемент)
	OneDriveКаталогОбработкаВыбора(Элемент, OneDriveКаталог, Истина);
КонецПроцедуры

&НаКлиенте
Процедура OneDriveНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ПараметрыПроизвольнойФормы = Новый Структура("Каталог, Логин, ИДКлиента, СекретКлиента, ТенантИД");
	Для Каждого КлючИЗначение Из ПараметрыПроизвольнойФормы Цикл
		ПараметрыПроизвольнойФормы[КлючИЗначение.Ключ] = Объект["OneDrive" + КлючИЗначение.Ключ];
	КонецЦикла;
	
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ru = '<Укажите получателя>'"));
	
	ОткрытьФорму("Справочник.РассылкиОтчетов.Форма.ПараметрыOneDrive", ПараметрыПроизвольнойФормы, Элемент);
КонецПроцедуры

&НаКлиенте
Процедура OneDriveКаталогОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура OneDriveКаталогОткрытие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	ПолныйАдрес = "https://digitstyle-my.sharepoint.com/personal/"+Объект.OneDriveЛогин+"/";
	ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(ПолныйАдрес);
КонецПроцедуры

&НаКлиенте
Процедура OneDriveКаталогОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	Если ВыбранноеЗначение = Неопределено Или ТипЗнч(ВыбранноеЗначение) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из ВыбранноеЗначение Цикл
		Объект["OneDrive" + КлючИЗначение.Ключ] = КлючИЗначение.Значение;
	КонецЦикла;
	
	ВидимостьДоступностьКорректность(ЭтотОбъект, "OneDriveКаталог");
	Модифицированность = Истина;
КонецПроцедуры
